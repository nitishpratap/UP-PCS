# Topic 7 — Transport, Tourism & Natural Hazards

### ★ UKPCS state card (Unit 2)

> **High-yield 2025 stems:** Silkyara length (Q51) · airports mismatch (Q103) · climate vulnerability (Q105) · SCCC / UAPCC (Q106).

---

## Quick Revision — Spine Only

| Head | Lock |
|------|------|
| Silkyara tunnel | **4.531 km**; Char Dham all-weather road; shortens Gangotri–Yamunotri link |
| Pantnagar airport | **Udham Singh Nagar** |
| Naini Saini | **Pithoragarh** |
| Chinyalisaur | **Uttarkashi** |
| Gauchar | **Chamoli** — **not Rudraprayag** |
| Climate vulnerability | State highly vulnerable; water stress + biodiversity depletion |
| SCCC | State Council for Climate Change — **2011** |
| UAPCC prep | **Not** State Revenue Department — forest/environment set-up |
| Tourist pillars | Char Dham, Nainital–Mussoorie, Corbett, Auli, Valley of Flowers |
| Hazards | Landslides, cloudbursts, flash floods, earthquakes, glacial lake risk |

---

## Confused Pairs

| Pair | Lock |
|------|------|
| Gauchar–Rudraprayag | **Wrong** — Gauchar is **Chamoli** |
| Silkyara 3.531 / 6.531 | Keyed length **4.531 km** |
| UAPCC by Revenue Dept | **False** — wrong department |
| SCCC year 2011 vs 2009 | **2011** keyed |

---

## 7.1 Transport network

- Road corridors (NH and Char Dham project) dominate connectivity; rail is stronger on the southern fringe.
- **Silkyara–Barkot** tunnel (**4.531 km**) is the length stem for Gangotri–Yamunotri access.
- Airfields: Pantnagar (US Nagar), Naini Saini (Pithoragarh), Chinyalisaur (Uttarkashi), Gauchar (**Chamoli**).

---

## 7.2 Tourism geography

- Pilgrimage (Char Dham, Haridwar–Rishikesh), hill stations, wildlife, and adventure tourism structure the visitor map.
- Festival / expo CA stems may appear in Current Affairs — keep place identity here.

---

## 7.3 Natural hazards and climate action

- Fragile slopes, intense monsoon bursts, and seismic belts make the state highly disaster-prone.
- Climate change worsens water stress and biodiversity pressure — both statements keyed true in 2025.
- **SCCC (2011)** steers climate action; **UAPCC** was not a Revenue Department product.

---

## Must-Score Facts

1. Silkyara = **4.531 km**.
2. Gauchar = **Chamoli**, not Rudraprayag.
3. Vulnerability: both statements correct (Q105).
4. SCCC **2011**; UAPCC ≠ Revenue Dept (Q106 → Only stmt 1).
5. Landslide / cloudburst / flood = core hazard set.

---

## UKPCS Inline PYQs

**Logic:** Precise length on the Char Dham route.

**UKPCS Prelims 2025, Q51**

The Silkyara Tunnel spanning ____ is expected to shorten the distance between Gangotri and Yamunotri by above 20 km.

A. 2.531 km 
B. 3.531 km 
C. 4.531 km 
D. 6.531 km 

<details>
<summary>Show answer</summary>

**Ans: C (Series B provisional key).** Silkyara Bend–Barkot tunnel is **4.531 km**.

</details>

**Logic:** Gauchar’s district is the planted error.

**UKPCS Prelims 2025, Q103**

Which pair is not correctly matched?

A. Naini Saini — Pithoragarh 
B. Gauchar — Rudraprayag 
C. Chinyalisaur — Uttarkashi 
D. Pantnagar — Udham Singh Nagar 

<details>
<summary>Show answer</summary>

**Ans: B (Series B provisional key).** Gauchar airstrip is in **Chamoli**, not Rudraprayag.

</details>

**Logic:** Both statements repeat state climate-plan findings.

**UKPCS Prelims 2025, Q105**

Statement 1: Uttarakhand is highly vulnerable to climate change. 
Statement 2: Climate change is causing water stress and depleting biodiversity in Uttarakhand.

A. Only statement 1 correct 
B. Only statement 1 correct and statement 2 incorrect 
C. Statements 1 and 2 both correct 
D. Both incorrect 

<details>
<summary>Show answer</summary>

**Ans: C (Series B provisional key).** Both vulnerability and water-stress / biodiversity effects are keyed correct.

</details>

**Logic:** Department entrusted with UAPCC is the planted error.

**UKPCS Prelims 2025, Q106**

Statement 1: The State Council for Climate Change (SCCC) of Uttarakhand was established in 2011. 
Statement 2: Responsibility of preparing UAPCC was given to the State Revenue Department.

A. Only statement 1 correct 
B. Only statement 2 correct 
C. Both incorrect 
D. Both correct 

<details>
<summary>Show answer</summary>

**Ans: A (Series B provisional key).** SCCC in 2011 is correct. UAPCC was prepared through the forest/environment set-up, not Revenue.

</details>

---

## Common Traps

1. **Gauchar = Rudraprayag** → Chamoli.
2. **Silkyara = 3.531 km** → 4.531 km.
3. **UAPCC = Revenue Department** → false.
4. **Rejecting climate biodiversity impact** → both Q105 statements true.
