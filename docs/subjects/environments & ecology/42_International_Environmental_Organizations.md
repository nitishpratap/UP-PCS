# Topic 42 — International Environmental Organizations
### ★ UPPCS Revision Sheet — Lucent-style (no repetition · Practice ≥25)

<details>
<summary><strong>Covers syllabus</strong> (click to expand)</summary>

UNEP (संयुक्त राष्ट्र पर्यावरण कार्यक्रम) | UNDP (यूएनडीपी) | FAO | UNESCO | IUCN (आईयूसीएन) | WWF (डब्ल्यूडब्ल्यूएफ) | IPCC (आईपीसीसी) | WMO | GEF | Greenpeace

</details>

> **Sources baked in:** IPCC AR6; UNEP; UNDP HDR; IUCN Red List; WWF LPR; FAO SOFO; UNESCO WH/MAB; UPPCS PYQs
> **Weight:** ★★★ High — org ↔ report; HQ traps; IUCN≠CITES; IPCC≠UNFCCC
> **Last verified:** August 2026

---

## Current Affairs (this topic)

- UNEP / IUCN / IPCC / GEF role headlines refresh institutional CA.
- HQ and mandate traps stay fixed teaching.

---

## Consolidated — 24 Must-Score Facts

1. **UNEP** was created after **Stockholm 1972** (स्टॉकहोम सम्मेलन). Its HQ is **Nairobi**. Flagship products include the **Emissions Gap Report** (उत्सर्जन अंतराल रिपोर्ट) and **GEO**.
2. **UNDP** sits in **New York** and publishes the **Human Development Report / HDI** (from **1990**).
3. **FAO** sits in **Rome** and publishes **SOFO**. It co-hosts Rotterdam work with UNEP.
4. **UNESCO** sits in **Paris** and runs World Heritage (विश्व धरोहर) and the **MAB** Biosphere Reserve (जैवमंडल आरक्षित क्षेत्र) programme.
5. **IUCN** (आईयूसीएन) sits in **Gland** and publishes the **Red List** of extinction risk.
6. **WWF** also sits in **Gland** and publishes the **Living Planet Report / LPI**.
7. **IPCC** sits in **Geneva**. It was created by **WMO + UNEP** in **1988** and shared the **2007** Nobel Peace Prize.
8. Emissions Gap Report = **UNEP**, not IPCC or UNDP. HDR = **UNDP**, not UNEP.
9. IPCC does science assessment. **UNFCCC** (यूएनएफसीसीसी) is the climate treaty. Do not merge the two.
10. IUCN assesses extinction risk. **CITES** (साइट्स) regulates wildlife **trade** (पण्याध्यक्ष). Different jobs.
11. IUCN and WWF share a city but differ: assessment union versus conservation NGO campaigning and Living Planet metrics.
12. UNEP coordinates environment (पर्यावरण). UNDP coordinates development. HQ and report facts separate them.
13. UNESCO heritage–education–MAB is not the same as UNEP environment coordination.
14. The **GEF (1991)** is a multilateral funder. Donors are not “developed countries only” as a rigid fact.
15. IPCC Assessment Reports synthesize climate science. They do not set national NDCs.
16. FAO’s food–forest–agriculture mandate explains why SOFO and pesticide PIC work sit there.
17. Red List categories are IUCN risk ranks, not CITES Appendix numbers.
18. Living Planet Index tracks vertebrate abundance trends for WWF messaging.
19. Nairobi versus Geneva versus New York versus Rome versus Paris versus Gland is a high-yield HQ ladder.
20. Do not credit IPCC with the Emissions Gap Report.
21. Do not credit UNEP with HDI.
22. Treaty secretariats hosted or supported by UNEP (Montreal, Basel, Stockholm POPs, Minamata) are still treaty bodies, not UNEP synonyms.
23. WWF is not the Red List author. IUCN is.
24. FAO HQ is Rome, not Paris or Geneva.

---

## Confused Pairs

| A | B | Distinguishing Fact / Correct Match | Hindi Terminology |
|---|---|-------------------------------------|-------------------|
| **UNEP** | **UNDP** | UN environmental coordinator established post-1972 Stockholm Conference (HQ: Nairobi, Kenya) vs UN development network producing the Human Development Index (HQ: New York) | यूएनईपी (नैरोबी, पर्यावरण) / यूएनडीपी (न्यूयॉर्क, विकास) |
| **IPCC** | **UNFCCC** | Scientific review panel established in 1988 by WMO/UNEP to assess peer-reviewed climate data (HQ: Geneva) vs legally binding treaty secretariat coordinating annual COPs (HQ: Bonn, Germany) | आईपीसीसी (वैज्ञानिक समीक्षा) / यूएनएफसीसीसी (वार्ता सचिवालय) |
| **IUCN** | **WWF** | Oldest global conservation union (1948, Gland) publishing the Red List of Threatened Species vs private international conservation NGO (1961, Gland) famous for Giant Panda logo and Earth Hour | आईयूसीएन (रेड लिस्ट) / डब्ल्यूडब्ल्यूएफ (पांडा प्रतीक) |
| **CITES** | **CMS (Bonn Convention)** | Regulates commercial cross-border trade in endangered species of wild flora and fauna (1973) vs promotes conservation and habitat protection for migratory wild animal species (1979) | साइट्स (वन्यजीव व्यापार) / बॉन कन्वेंशन (प्रवासी प्रजातियां) |
| **Global Environment Facility (GEF)** | **Green Climate Fund (GCF)** | Multilateral financial mechanism (World Bank 1991) serving CBD, UNCCD, Stockholm, Minamata, and UNFCCC vs dedicated operating financial entity of UNFCCC created at COP16 Cancun | जीईएफ (वैश्विक पर्यावरण कोष) / जीसीएफ (हरित जलवायु कोष) |
---

## Must-score facts — UNEP UNDP IUCN IPCC HQs

### Org ↔ HQ / product

| Item | Lock |
|------|------|
| UNEP | Nairobi; Emissions Gap + GEO; post-Stockholm 1972 |
| UNDP | New York; HDR/HDI |
| FAO | Rome; SOFO |
| UNESCO | Paris; WH + MAB |
| IUCN | Gland; Red List |
| WWF | Gland; Living Planet |
| IPCC | Geneva; WMO+UNEP 1988; Nobel 2007 |
| IUCN ≠ CITES | Risk assess / trade regulate |
| GEF | **1991** multilateral funder |

---

## 42.1 UNEP

**Identity:** Stockholm 1972 | Nairobi | Emissions Gap | GEO | Treaty host

- **UNEP (United Nations Environment Programme)** is the UN’s principal environment coordination body, created after the **Stockholm Conference (1972)** (स्टॉकहोम सम्मेलन).
- **HQ: Nairobi, Kenya** — not Geneva or New York.
- Publishes the annual **Emissions Gap Report** (NDC shortfall vs 1.5–2°C pathways) and **GEO (Global Environment Outlook)**.
- Hosts/supports secretariats for treaties such as Montreal, Basel, Stockholm POPs, and Minamata.
- Co-created **IPCC** with WMO, but Emissions Gap is **UNEP’s own** report.
- **GEF (Global Environment Facility, 1991)** finances projects under major environment conventions; UNEP is an implementing agency. Donors are **not** “developed countries only.”
- GEF serves as a financial mechanism for five major conventions: **Minamata**, **Stockholm POPs**, **CBD**, **UNCCD**, and **UNFCCC**.
- Core implementing partners include **UNDP**, **UNEP**, and the **World Bank**.

**PYQ — UPPCS Prelims 2024, Q6**

Emissions Gap Report publisher among org–report match:

<details><summary>Show answer</summary>

**UNEP** — Full key: HDR→UNDP, WEO→IMF, WIR→UNCTAD, Emissions Gap→**UNEP** → **A (3 1 4 2)**.

</details>

**PYQ — UPPCS Prelims 2025, Q145**

GEF: (1) established 1991 (2) only developed countries are donors

Options: A. Only 2 B. Neither C. Both D. Only 1

<details><summary>Show answer</summary>

**Ans: D** — Year true; donor statement false.

</details>

> **Logic:** Emissions Gap ≠ IPCC Assessment Report.

---

## 42.2 UNDP

**Identity:** New York | HDR 1990 | HDI | Mahbub ul Haq + Amartya Sen | SDGs

- **UNDP (United Nations Development Programme)** is the UN’s development network agency.
- **HQ: New York**.
- Publishes the **Human Development Report (HDR)** and **Human Development Index (HDI)** since **1990**.
- HDI was developed by **Mahbub ul Haq** with conceptual contribution from **Amartya Sen** — not a “Priesner-led” first team.
- Supports country-level **SDG** implementation; environment enters via sustainable development programming.
- Not the publisher of Emissions Gap, Red List, or Living Planet Report.

**PYQ — UPPCS Prelims 2025, Q132**

1. UNDP published first HDR in 1990 with HDI.
2. First HDI team led by Mahbub-ul-Haq and S. Priesner.

Options: A. Only 2 B. Neither C. Both D. Only 1

<details><summary>Show answer</summary>

**Ans: D** — Stmt 1 true; stmt 2 false (Haq–Sen lineage, not Priesner).

</details>

**PYQ — UPPCS Prelims 2018, Q35**

HDI was first developed by:

Options: A. UNDP B. IMF C. UNICEF D. UNCTAD

<details><summary>Show answer</summary>

**Ans: A**

</details>

> **Logic:** UNDP ≠ UNEP. HDR ≠ Emissions Gap.

---

## 42.3 FAO

**Identity:** Rome | Food–agriculture–forests | SOFO | Rotterdam with UNEP

- **FAO (Food and Agriculture Organization)** is the UN specialised agency for food and agriculture.
- **HQ: Rome, Italy**.
- Environment role: agriculture–forest–biodiversity–land-use links; pesticide and forestry governance.
- Co-publishes **State of the World’s Forests (SOFO)** with UNEP.
- Jointly administers the **Rotterdam Convention** (PIC for hazardous chemicals in trade) with **UNEP**.
- Not the publisher of HDR, Emissions Gap, or Red List.

> **Logic:** FAO HQ = **Rome**, not Nairobi.

---

## 42.4 UNESCO

**Identity:** Paris | World Heritage | MAB Biosphere Reserves (जैवमंडल आरक्षित क्षेत्र) | ESD

- **UNESCO** leads education, science, and culture — environment enters via heritage and biosphere programmes.
- **HQ: Paris**.
- Designates **UNESCO World Heritage Sites** (natural/cultural/mixed) under the 1972 WH Convention.
- Runs the **Man and the Biosphere (MAB)** programme — World Network of Biosphere Reserves.
- Supports Education for Sustainable Development (ESD).
- **Limits to Growth** is **Club of Rome**, not UNESCO.

**PYQ — UPPCS Prelims 2019, Q35**

Jaipur listed as UNESCO World Heritage Site at the 43rd session (पंगु सत्र) held in:

Options: A. Baku B. Bishkek C. Istanbul D. Marrakesh

<details><summary>Show answer</summary>

**Ans: A (Baku)**

</details>

**PYQ — UPPCS Prelims 2019, Q146**

“Limits to Growth” propounded by:

Options: A. Club of Rome B. UNESCO C. Brundtland D. Agenda 21

<details><summary>Show answer</summary>

**Ans: A**

</details>

> **Logic:** UNESCO = WH/MAB. UNEP = environment programme.

---

## 42.5 IUCN

**Identity:** 1948 | Gland | Red List | Assess risk ≠ regulate trade

- **IUCN (International Union for Conservation of Nature)** is a global conservation union of governments and NGOs, founded **1948**.
- **HQ: Gland, Switzerland**.
- Publishes the **IUCN Red List** of extinction risk (LC, NT, VU, EN, CR, EW, EX).
- **Threatened** categories commonly = **VU + EN + CR**.
- Assesses scientific extinction risk; does **not** regulate wildlife trade — **CITES** does.
- Advises CBD/CITES scientifically but is a separate organisation.

> **Logic:** “IUCN bans trade” = false. **CITES** regulates trade.

---

## 42.6 WWF

**Identity:** Conservation NGO | Gland | Living Planet Report | Panda logo

- **WWF (World Wide Fund for Nature)** is a global conservation **NGO**, not a UN agency.
- Since **1961**, WWF’s logo animal has been the **Giant Panda (*Ailuropoda melanoleuca*)**.
- It publishes the **Living Planet Report** and Living Planet Index.
- **HQ: Gland, Switzerland** — same city as IUCN, different organisation.
- Publishes the **Living Planet Report** and **Living Planet Index (LPI)** on vertebrate population (जनसंख्या) trends.
- Known for the **giant panda** logo and flagship-species campaigns.
- Campaigns and advocates; IUCN assesses via Red List.

> **Logic:** Living Planet = **WWF**. Red List = **IUCN**.

---

## 42.7 IPCC

**Identity:** 1988 WMO+UNEP | Geneva | Assessment Reports | Science ≠ policy

- **IPCC (Intergovernmental Panel on Climate Change (जलवायु परिवर्तन))** was established in **1988** by **WMO** and **UNEP**.
- **HQ: Geneva**.
- Publishes **Assessment Reports** (AR1→AR6; AR6 synthesis **2023**) plus special reports.
- Synthesises existing science; does **not** set NDCs or negotiate treaties.
- Shared the **Nobel Peace Prize 2007** with Al Gore.
- **UNFCCC/COP** is the policy–treaty track; IPCC informs it.

> **Logic:** IPCC ≠ UNFCCC. IPCC ≠ Emissions Gap Report.

---

## 42.8 WMO, Greenpeace & US EPA

**Identity:** WMO Geneva | Greenpeace Amsterdam | US EPA 1970

- The **World Meteorological Organization (WMO)** (डब्लूएमओ) provides the global framework for meteorology and operational hydrology.
- WMO originated from the **International Meteorological Organization (1873)** and became WMO on **17 March 1951**.
- **WMO headquarters is Geneva, Switzerland.** It co-created the **IPCC** with UNEP in **1988**.
- **Greenpeace International** is an environmental NGO headquartered in **Amsterdam, Netherlands**.
- The **US Environment Protection Agency (EPA)** was established on **2 December 1970** to write and enforce environmental health regulations in the United States.
- Do not confuse the **US EPA** with India’s **Environment (Protection) Act, 1986**.

| Body | HQ / year | Job |
|------|-----------|-----|
| WMO | Geneva | Weather/climate cooperation; IPCC parent |
| Greenpeace | Amsterdam | Environmental NGO |
| US EPA | Est. 1970 | US federal environment agency |
| UNEP | Nairobi | UN environment programme |

---

## Org ↔ Report ↔ HQ Card

| Org | HQ | Flagship fact |
|-----|-----|---------------|
| UNEP | Nairobi | Emissions Gap / GEO |
| UNDP | New York | HDR / HDI |
| FAO | Rome | SOFO; Rotterdam (+UNEP) |
| UNESCO | Paris | World Heritage; MAB |
| IUCN | Gland | Red List |
| WWF | Gland | Living Planet |
| IPCC | Geneva | Assessment Reports |
| WMO | Geneva | Meteorology; IPCC co-parent |
| Greenpeace | Amsterdam | Environmental NGO |

---

## Complete PYQ Bank (Topic 42)

**Q1. UPPCS Prelims 2024, Q6**

Match List-I (Organisation) with List-II (Report):

| List-I | List-II |
| --- | --- |
| 1. UNDP | A. Emissions Gap Report |
| 2. IMF | B. Human Development Report |
| 3. UNEP | C. World Economic Outlook |
| 4. UNCTAD | D. World Investment Report |

*Row order is not the answer code.*

A. 3 1 4 2
B. 1 3 2 4
C. 2 4 1 3
D. 4 2 3 1

<details>
<summary>Show answer</summary>

**Logic:** Org–report match — Emissions Gap Report is UNEP’s; HDR is UNDP’s; WEO is IMF’s; WIR is UNCTAD’s.

**Ans: A (3 1 4 2).** HDR→**UNDP**; WEO→**IMF**; WIR→**UNCTAD**; Emissions Gap→**UNEP**. **Trap:** Crediting IPCC or UNDP with Emissions Gap Report.

</details>

**Q2. UPPCS Prelims 2025, Q132**

Consider the following statements:

1. UNDP published the first Human Development Report in 1990 with the HDI.
2. The first HDI team was led by Mahbub-ul-Haq and S. Priesner.

A. Only 2
B. Neither
C. Both
D. Only 1

<details>
<summary>Show answer</summary>

**Logic:** HDR/HDI started 1990 under UNDP with Mahbub ul Haq and Amartya Sen — not “Priesner-led” team.

**Ans: D (Only 1).** Stmt 1 true; stmt 2 false — **Haq–Sen** lineage, not Priesner. **Trap:** Accepting any plausible Western name on stmt 2.

</details>

**Q3. UPPCS Prelims 2025, Q145**

With reference to the Global Environment Facility (GEF):

1. It was established in 1991.
2. Only developed countries are donors to the GEF.

A. Only 2
B. Neither
C. Both
D. Only 1

<details>
<summary>Show answer</summary>

**Logic:** GEF established 1991; donors are not “developed countries only.”

**Ans: D (Only 1).** **1991** true; donor statement **false** — developing countries also contribute. **Trap:** Treating GEF as Annex-I-only club.

</details>

**Q4. UPPCS Prelims 2018, Q35**

HDI was first developed by:

A. UNDP
B. IMF
C. UNICEF
D. UNCTAD

<details>
<summary>Show answer</summary>

**Logic:** HDI is UNDP’s flagship index — not IMF, UNICEF, or UNCTAD.

**Ans: A.** **UNDP** publishes HDR/HDI from **1990**. **Trap:** UNEP because “environment” sounds related.

</details>

**Q5. UPPCS Prelims 2019, Q35**

Jaipur was listed as a UNESCO World Heritage Site at the 43rd session held in:

A. Baku
B. Bishkek
C. Istanbul
D. Marrakesh

<details>
<summary>Show answer</summary>

**Logic:** Jaipur inscribed as UNESCO World Heritage Site at the 43rd session in Baku (2019).

**Ans: A (Baku).** **43rd WHC session — Baku, 2019.** **Trap:** Marrakesh/Istanbul as other recent host cities.

</details>

**Q6. UPPCS Prelims 2019, Q146**

“Limits to Growth” was propounded by:

A. Club of Rome
B. UNESCO
C. Brundtland Commission
D. Agenda 21

<details>
<summary>Show answer</summary>

**Logic:** Limits to Growth (1972) was Club of Rome — not UNESCO, Brundtland, or Agenda 21.

**Ans: A.** **Club of Rome** (1972); Brundtland = Our Common Future; Agenda 21 = Rio 1992. **Trap:** UNESCO because of “education/science” label.

</details>

**Q7. UPPCS Prelims (pattern)**

The Intergovernmental Panel on Climate Change (IPCC) was established in 1988 by:

A. World Meteorological Organization (WMO) and UNEP
B. FAO and WHO alone
C. IUCN and WWF alone
D. UNDP and UNESCO alone

<details>
<summary>Show answer</summary>

**Logic:** IPCC created 1988 by WMO + UNEP; shared Nobel Peace Prize 2007 — does not set NDCs.

**Ans: A.** **WMO + UNEP → IPCC (1988)**; Nobel **2007**. **Trap:** IPCC = UNFCCC treaty body (policy is UNFCCC/COP).

</details>

**Q8. UPPCS Prelims (pattern)**

Which statement is correct?

A. IUCN Red List assesses extinction risk; CITES regulates international trade in listed species
B. IUCN bans all wildlife trade globally
C. CITES publishes the Living Planet Report
D. Red List Appendix I–III sets trade permits

<details>
<summary>Show answer</summary>

**Logic:** IUCN Red List assesses extinction risk; CITES regulates international wildlife trade — different jobs.

**Ans: A.** **IUCN** = risk assessment; **CITES** = trade regulation (Appendices I–III). **Trap:** “IUCN bans trade” or WWF authorship of Red List.

</details>

**Q9. UPPCS Prelims (pattern)**

The **Living Planet Report** is published by:

A. WWF
B. IUCN
C. IPCC
D. FAO

<details>
<summary>Show answer</summary>

**Logic:** WWF publishes Living Planet Report/LPI; IUCN publishes Red List — both HQ in Gland but different roles.

**Ans: A.** **WWF** = Living Planet / LPI; **IUCN** = Red List. **Trap:** Same city (Gland) → same organisation.

</details>

**Q10. UPPCS Prelims (pattern)**

Which pairing of organisation and headquarters is **correct**?

A. UNEP — Nairobi; FAO — Rome; UNESCO — Paris
B. UNEP — Geneva; FAO — Nairobi; UNESCO — New York
C. UNEP — Rome; FAO — Paris; UNESCO — Nairobi
D. UNEP — New York; FAO — Gland; UNESCO — Geneva

<details>
<summary>Show answer</summary>

**Logic:** HQ ladder — UNEP Nairobi; FAO Rome; UNESCO Paris; IPCC/WMO Geneva; UNDP New York.

**Ans: A.** **UNEP Nairobi**, **FAO Rome**, **UNESCO Paris** (IPCC/WMO Geneva; UNDP New York). **Trap:** UNEP in Geneva because “environment = Geneva agencies.”

</details>

---

## Practice Zone — UPPCS Format Drill

> **30 questions** | Original drill | ≥60% multi-statement/application | A/R · Match · chronology · NOT-matched | Keys balanced | **Ans first, then Logic**

**Q1.** With reference to UNEP, which of the following statements is/are correct?

1. Created after Stockholm 1972; HQ Nairobi.
2. UNEP’s flagship products are the Human Development Report and World Development Report.
3. UNEP publishes HDI.

A. Only 1
B. 1 and 2
C. Only 3
D. 2 and 3

<details>
<summary>Show answer</summary>

**Ans: A.** Only 1 is correct.

**Logic:** Statement 1 is true (Stockholm → UNEP Nairobi). Statement 2 confuses UNEP flagships (Emissions Gap, GEO) with UNDP/World Bank reports. Statement 3 is false — HDI is UNDP, not UNEP.

</details>

**Q2.** Match List-I with List-II:

| List-I | List-II |
|--------|--------|
| A. UNDP | 1. Rome; SOFO |
| B. FAO | 2. New York; HDR/HDI |
| C. UNESCO | 3. Paris; WH + MAB |

Code:

A. A-2, B-3, C-1
B. A-1, B-2, C-3
C. A-2, B-1, C-3
D. A-3, B-1, C-2

<details>
<summary>Show answer</summary>

**Ans: C.** A-2, B-1, C-3.

**Logic:** UNDP/FAO/UNESCO.

</details>

**Q3.** Which of the following pairs is/are correctly matched?

1. IUCN — Gland; Red List
2. IUCN — Gland; Living Planet Report / LPI
3. WWF — Red List author

A. 2 and 3
B. Only 1
C. 1 and 2
D. Only 3

<details>
<summary>Show answer</summary>

**Ans: B.** Only 1 is correct.

**Logic:** Only statement 1 remains true among the numbered claims. Statement 1 is true. Statement 2 is false. Statement 3 is false.

</details>

**Q4.** Given below are two statements, one labelled as Assertion (A) and the other as Reason (R).

Assertion (A): Emissions Gap Report is by UNEP.

Reason (R): It is published by the IPCC.

A. Both (A) and (R) are true, but (R) is not the correct explanation of (A)
B. (A) is false, but (R) is true
C. (A) is true, but (R) is false
D. Both (A) and (R) are true and (R) is the correct explanation of (A)

<details>
<summary>Show answer</summary>

**Ans: C.** A true, R false.

**A/R logic:** Do not credit IPCC with Emissions Gap.

</details>

**Q5.** With reference to IPCC, which of the following statements is/are correct?

1. HQ Geneva; created by WMO + UNEP 1988.
2. Shared 2007 Nobel Peace Prize.
3. IPCC sets national NDCs.

A. 1 and 2 only
B. 2 and 3 only
C. 1 and 3 only
D. 1, 2 and 3

<details>
<summary>Show answer</summary>

**Ans: A.** Only 1 and 2.

**Logic:** Science assessment — not NDC setter.

</details>

**Q6.** Arrange HQ ladder teaching cities:

A. All in Nairobi only
B. Nairobi UNEP; Geneva IPCC; New York UNDP; Rome FAO; Paris UNESCO; Gland IUCN/WWF
C. All in Gland only
D. All in Bonn only

<details>
<summary>Show answer</summary>

**Ans: B.** Six-city HQ ladder.

**Logic:** High-yield map.

</details>

**Q7.** Which of the following pairs is/are NOT correctly matched?

1. IUCN — extinction risk assessment
2. CITES — wildlife trade regulation
3. IUCN Red List categories = CITES Appendix numbers

A. 1 and 2 only
B. 2 and 3 only
C. 3 only
D. 1, 2 and 3

<details>
<summary>Show answer</summary>

**Ans: C.** Only pair 3 is not correctly matched.

**Logic:** Risk ranks ≠ Appendices.

</details>

**Q8.** Given below are two statements, one labelled as Assertion (A) and the other as Reason (R).

Assertion (A): IPCC does science assessment; UNFCCC is the climate treaty.

Reason (R): The two should not be merged.

A. Both (A) and (R) are true, but (R) is not the correct explanation of (A)
B. (A) is false, but (R) is true
C. (A) is true, but (R) is false
D. Both (A) and (R) are true and (R) is the correct explanation of (A)

<details>
<summary>Show answer</summary>

**Ans: D.** Both true and R restates the separation.

**A/R logic:** Science body ≠ treaty.

</details>

**Q9.** With reference to GEF, which of the following statements is/are correct?

1. Multilateral funder from 1991.
2. Donors are not “developed countries only” as a rigid fact.
3. GEF publishes the Living Planet Index.

A. 1, 2 and 3
B. 2 and 3 only
C. 1 and 3 only
D. 1 and 2 only

<details>
<summary>Show answer</summary>

**Ans: D.** Only 1 and 2.

**Logic:** LPI = WWF.

</details>

**Q10.** Match List-I with List-II:

| List-I | List-II |
|--------|--------|
| A. Emissions Gap | 1. UNDP |
| B. HDR / HDI | 2. UNEP |
| C. Red List | 3. IUCN |

Code:

A. A-2, B-1, C-3
B. A-1, B-2, C-3
C. A-2, B-3, C-1
D. A-3, B-1, C-2

<details>
<summary>Show answer</summary>

**Ans: A.** A-2, B-1, C-3.

**Logic:** Report–org map.

</details>

**Q11.** Arrange IUCN vs WWF same-city split:

A. Identical jobs always
B. IUCN assessment union; WWF NGO campaigning + Living Planet metrics
C. Both set NDCs
D. Both publish SOFO only

<details>
<summary>Show answer</summary>

**Ans: B.** Assessment vs campaigning NGO.

**Logic:** Shared Gland — different jobs.

</details>

**Q12.** With reference to FAO, which of the following statements is/are correct?

1. HQ Rome — not Paris or Geneva.
2. Publishes SOFO.
3. Co-hosts Rotterdam work with UNEP neighbourhood.

A. 1 and 2 only
B. 2 and 3 only
C. 1, 2 and 3
D. 1 and 3 only

<details>
<summary>Show answer</summary>

**Ans: C.** All three.

**Logic:** FAO mandate.

</details>

**Q13.** Given below are two statements, one labelled as Assertion (A) and the other as Reason (R).

Assertion (A): Do not credit UNEP with HDI.

Reason (R): HDR/HDI is UNDP’s product from 1990 neighbourhood.

A. Both (A) and (R) are true, but (R) is not the correct explanation of (A)
B. (A) is false, but (R) is true
C. (A) is true, but (R) is false
D. Both (A) and (R) are true and (R) is the correct explanation of (A)

<details>
<summary>Show answer</summary>

**Ans: D.** Both true and R explains A.

**A/R logic:** UNEP ≠ HDI.

</details>

**Q14.** Which one of the following correctly states Living Planet Index?

A. FAO SOFO forest cover only
B. IUCN Red List categories
C. CITES Appendix I only
D. Vertebrate abundance trends for WWF messaging

<details>
<summary>Show answer</summary>

**Ans: D.** WWF LPI.

**Logic:** Not Red List.

</details>

**Q15.** How many of the following HQ facts are correct?

UNEP Nairobi; IPCC Geneva; UNDP New York; FAO Rome

A. All four
B. Only two
C. Only three
D. Only one

<details>
<summary>Show answer</summary>

**Ans: A.** All four.

**Logic:** HQ ladder chunk.

</details>

**Q16.** With reference to treaty secretariats, which of the following statements is/are correct?

1. Montreal, Basel, Stockholm POPs, Minamata may be hosted/supported by UNEP.
2. They remain treaty bodies — not “UNEP itself.”
3. They are identical to IPCC Assessment Reports.

A. 2 and 3 only
B. 1 and 2 only
C. 1 and 3 only
D. 1, 2 and 3

<details>
<summary>Show answer</summary>

**Ans: B.** Only 1 and 2.

**Logic:** Treaty bodies ≠ IPCC ARs.

</details>

**Q17.** Match List-I with List-II:

| List-I | List-II |
|--------|--------|
| A. UNESCO | 1. Environment coordination |
| B. UNEP | 2. Heritage / education / MAB |
| C. UNDP | 3. Development coordination |

Code:

A. A-2, B-3, C-1
B. A-1, B-2, C-3
C. A-2, B-1, C-3
D. A-3, B-1, C-2

<details>
<summary>Show answer</summary>

**Ans: C.** A-2, B-1, C-3.

**Logic:** Mandate split.

</details>

**Q18.** Given below are two statements, one labelled as Assertion (A) and the other as Reason (R).

Assertion (A): IPCC Assessment Reports synthesize climate science.

Reason (R): They set each country’s NDC targets.

A. Both (A) and (R) are true, but (R) is not the correct explanation of (A)
B. (A) is false, but (R) is true
C. (A) is true, but (R) is false
D. Both (A) and (R) are true and (R) is the correct explanation of (A)

<details>
<summary>Show answer</summary>

**Ans: C.** A true, R false.

**A/R logic:** Science ≠ NDC setter.

</details>

**Q19.** Which of the following pairs is/are correctly matched?

1. FAO food–forest–agriculture mandate explains SOFO + pesticide PIC neighbourhood
2. UNESCO ≠ UNEP environment coordination
3. FAO HQ is Paris

A. 1, 2 and 3
B. 2 and 3 only
C. 1 and 3 only
D. 1 and 2 only

<details>
<summary>Show answer</summary>

**Ans: D.** Only 1 and 2.

**Logic:** FAO = Rome.

</details>

**Q20.** Arrange IPCC creation:

A. 1988 by WMO + UNEP
B. 1972 UNEP alone
C. 1992 CBD alone
D. 2015 Paris alone

<details>
<summary>Show answer</summary>

**Ans: A.** 1988 WMO+UNEP.

**Logic:** IPCC birth.

</details>

**Q21.** With reference to Gland twins, which of the following statements is/are correct?

1. IUCN and WWF both sit in Gland.
2. Different products: Red List vs Living Planet.
3. Both are identical to CITES Appendices.

A. 2 and 3 only
B. 1 and 2 only
C. 1 and 3 only
D. 1, 2 and 3

<details>
<summary>Show answer</summary>

**Ans: B.** Only 1 and 2.

**Logic:** Not CITES.

</details>

**Q22.** Given below are two statements, one labelled as Assertion (A) and the other as Reason (R).

Assertion (A): IUCN assesses extinction risk; CITES regulates wildlife trade.

Reason (R): Different institutions, different jobs.

A. Both (A) and (R) are true, but (R) is not the correct explanation of (A)
B. (A) is false, but (R) is true
C. (A) is true, but (R) is false
D. Both (A) and (R) are true and (R) is the correct explanation of (A)

<details>
<summary>Show answer</summary>

**Ans: D.** Both true and R explains the separation.

**A/R logic:** Risk vs trade.

</details>

**Q23.** How many of the following report–org pairs are correct?

Emissions Gap–UNEP; HDR–UNDP; Red List–IUCN; LPI–WWF

A. Only one
B. Only two
C. All four
D. Only three

<details>
<summary>Show answer</summary>

**Ans: C.** All four.

**Logic:** Four classic pairs.

</details>

**Q24.** Which one of the following correctly states UNEP vs UNDP?

A. Both set CITES Appendices
B. Both only publish Red List
C. Both HQ only in Gland
D. Environment coordination vs development coordination — different HQ/reports

<details>
<summary>Show answer</summary>

**Ans: D.** Environment vs development.

**Logic:** Separate lanes.

</details>

**Q25.** With reference to GEO, which of the following statements is/are correct?

1. UNEP flagship neighbourhood with Emissions Gap.
2. Not the UNDP Human Development Report.
3. Identical to IUCN Red List categories.

A. 1 and 2 only
B. 2 and 3 only
C. 1 and 3 only
D. 1, 2 and 3

<details>
<summary>Show answer</summary>

**Ans: A.** Only 1 and 2.

**Logic:** GEO ≠ Red List.

</details>

**Q26.** Match List-I with List-II:

| List-I | List-II |
|--------|--------|
| A. IUCN | 1. Living Planet |
| B. WWF | 2. Red List |
| C. IPCC | 3. Geneva; science ARs |

Code:

A. A-1, B-2, C-3
B. A-2, B-1, C-3
C. A-2, B-3, C-1
D. A-3, B-1, C-2

<details>
<summary>Show answer</summary>

**Ans: B.** A-2, B-1, C-3.

**Logic:** IUCN/WWF/IPCC.

</details>

**Q27.** Given below are two statements, one labelled as Assertion (A) and the other as Reason (R).

Assertion (A): Do not credit IPCC with the Emissions Gap Report.

Reason (R): Emissions Gap Report is UNEP’s annual product.

A. Both (A) and (R) are true, but (R) is not the correct explanation of (A)
B. (A) is false, but (R) is true
C. (A) is true, but (R) is false
D. Both (A) and (R) are true and (R) is the correct explanation of (A)

<details>
<summary>Show answer</summary>

**Ans: D.** Both true and R explains A.

**A/R logic:** UNEP not IPCC.

</details>

**Q28.** Arrange UNESCO MAB neighbourhood:

A. Only Kyoto Annex-I
B. Only CGWB groundwater
C. Biosphere Reserve programme under UNESCO Paris
D. Only BS-VI fuels

<details>
<summary>Show answer</summary>

**Ans: C.** MAB / Biosphere.

**Logic:** UNESCO tool.

</details>

**Q29.** With reference to HDR start, which of the following statements is/are correct?

1. From 1990 neighbourhood.
2. UNDP New York.
3. Published by UNEP Nairobi as HDI.

A. 1, 2 and 3
B. 2 and 3 only
C. 1 and 3 only
D. 1 and 2 only

<details>
<summary>Show answer</summary>

**Ans: D.** Only 1 and 2.

**Logic:** Not UNEP.

</details>

**Q30.** Which one of the following correctly states FAO HQ?

A. Rome
B. Paris
C. Geneva
D. Nairobi

<details>
<summary>Show answer</summary>

**Ans: A.** Rome.

**Logic:** Not Paris/Geneva.

</details>

---

## Common Traps

| Trap | Correct |
|------|------|
| Emissions Gap = IPCC/UNDP | **UNEP** |
| HDR = UNEP | **UNDP** |
| UNEP HQ Geneva/NY | **Nairobi** |
| IPCC sets NDCs | **UNFCCC/COP** |
| IUCN regulates trade | **CITES** |
| WWF = IUCN | Same city, **different roles** |
| FAO HQ Nairobi | **Rome** |
| GEF donors = developed only | **False** |
| HDI team = Priesner | **Haq–Sen** lineage |
| Limits to Growth = UNESCO | **Club of Rome** |

---

## 42.8 OPCW & UN-Habitat (bank add-ons)

**Identity:** Chemical Weapons Convention | The Hague | Habitat (वास स्थान) Nairobi

- **OPCW (Organisation for the Prohibition (प्रतिषेध) of Chemical Weapons)** implements the **Chemical Weapons Convention** (force **29 April 1997**); HQ **The Hague**, Netherlands. It verifies destruction/non-re-emergence of chemical weapons and assists states against chemical threats — **not** an EU/NATO/WHO body. Nobel Peace Prize **2013**.
- **UN-Habitat** (UN Human Settlements Programme; HQ **Nairobi**, from **1978**) is mandated to promote socially and environmentally sustainable towns/cities and adequate shelter; partners include governments, local authorities, and non-state actors; contributes to poverty reduction and safe water/sanitation access. Partners are **not** governments-only.

> **Logic:** OPCW ≠ EU organ. UN-Habitat partners ≠ governments only.
