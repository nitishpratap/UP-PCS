# Topic 36 — Ozone Layer
### ★ UPPCS Revision Sheet — Lucent-style (no repetition · Practice ≥25)

<details>
<summary><strong>Covers syllabus</strong> (click to expand)</summary>

Ozone Layer | Ozone Depletion | Ozone Hole | Ozone Depleting Substances (ODS) | Montreal Protocol | Vienna Convention

</details>

> **Sources baked in:** NCERT; UNEP Ozone Secretariat; Vienna 1985; Montreal 1987 (force 1989); Kigali 2016; MoEFCC Ozone Cell; UPPCS/UPSC PYQs
> **Weight:** ★★★ High — Montreal ≠ Kyoto; ozone = stratosphere; Ozone Day 16 Sept; CFC link
> **Last verified:** September 2026

---

## Current Affairs (this topic)

- Twin of Ozone Layer core — keep Montreal / Kigali as ozone-family CA only.
- Prefer one consistent ODS story across both ozone chapters.

---

## Consolidated — 34 Must-Score Facts

1. **Ozone (O₃)** is triatomic oxygen. **Good ozone** sits in the **stratosphere** (about **15–30/35 km**). Ground-level ozone is a harmful pollutant.
2. About **90%** of atmospheric ozone is in the **stratosphere**; about **10%** is in the **troposphere**.
3. Stratospheric ozone forms when **UV** acts on **O₂** (Chapman cycle). Ozone absorbs **ultraviolet**, not IR as its main protective role.
4. Ozone amount is measured in **Dobson Units**. **1 DU ≈ 0.01 mm** of pure ozone at **0°C and 1 atm**. Average column ≈ **300 DU**. An ozone hole is often taken as below **220 DU**.
5. Stratospheric ozone is naturally regulated in part by **nitrogen dioxide (NO₂)**. Column thickness is often highest in **spring** and lowest in the **rainy season** (general seasonal pattern — distinct from the Antarctic spring hole).
6. Ozone depletion is driven by **ODS** such as CFCs through chlorine and bromine catalytic cycles.
7. One chlorine atom can destroy on the order of **100,000** ozone molecules before it is removed.
8. The classic **ozone hole** forms over **Antarctica** in spring (**September–October**) with polar stratospheric clouds, polar front/vortex, and CFCs.
9. Joe **Farman**, Brian **Gardiner**, and Jonathan **Shanklin** (British Antarctic Survey) reported the Antarctic ozone hole in **Nature (May 1985)**; TOMS mapping was used in that era of monitoring.
10. ODS include **CFCs, Halon-1211/1301, carbon tetrachloride, methyl chloroform, methyl bromide, HCFCs**, and related chlorine/bromine carriers. **HFCs** are **not** classic ODS.
11. Refrigerant trade name **Mafron** (halocarbon coolants) is common in domestic refrigerators; **ammonia** is used in many large plants. CFCs are **not** primarily “automobile exhaust” gases.
12. CFCs are used in refrigerators/AC, foam blowing, aerosol propellants, and cleaning some electronic components — **not** tubeless-tyre production as a classic CFC use.
13. **HFCs** are high-GWP climate gases under **Kigali**. They are **not** classic ozone-depleting substances.
14. The **Vienna Convention (1985)** is the framework treaty. The **Montreal Protocol (16 September 1987; force 1 January 1989)** is the binding ODS phase-out.
15. World Ozone Day is **16 September** (Montreal signing; UN observance from **1994**).
16. The **Kigali Amendment (2016; force 1 January 2019)** phases down **HFCs** for climate reasons under the Montreal family.
17. **Montreal = ozone. Kyoto = climate.** Carbon trading ≠ Montreal.
18. ODP measures ozone-damage potential. GWP measures heat-trapping potential.
19. Stratospheric ozone absorbs harmful UV (**~93–99%** of the most damaging wavelengths). Tropospheric ozone is a pollutant and greenhouse gas.
20. **UV-A (315–400 nm)** largely reaches the surface; **UV-B (280–315 nm)** is mostly blocked by ozone; **UV-C (100–280 nm)** is blocked and does not reach the surface under normal conditions.
21. Extra UV-B raises **skin cancer**, cataracts, crop damage, and plankton stress — not “liver/brain/oral cancer” as the classic stem key.
22. India’s Ozone Cell under MoEFCC tracks HCFC phase-out and cooling-sector compliance.
23. Do not place the protective ozone layer in the troposphere.
24. CFCs are both ODS and strong greenhouse gases — “CFC not a GHG” is false.
25. Polar spring timing matters: the deepest Antarctic loss is not midsummer.
26. Vienna does not itself list every control measure; Montreal carries the binding phase-out schedules.
27. **G.W. Kent Moore** (University of Toronto) led the **2005** report of an ozone “halo” / low-ozone feature over the **Tibetan Plateau**.
28. Homosphere (uniform composition) ≈ troposphere + stratosphere + mesosphere; **ionosphere** is **not** part of the homosphere set in UKPCS-style stems.
29. Troposphere is deepest in the tropics (~**18–20 km**) and shallower near the poles.
30. Dobson Unit is a column measure, not a surface concentration in ppm.
31. A stem pairing Montreal with climate mitigation is usually the Kyoto/Paris trap.
32. Ozone as a gas near the surface is poisonous to lungs/respiratory systems; as a stratospheric layer it is essential for biotic life via UV shielding.
33. Formation of ozone is slower over polar regions, so depletion effects are strongest there.
34. Acid rain is driven by **SO₂ / NOx**, not “nitric acid alone” as a matched pair with ozone–CFC style keys.

---

## Confused Pairs

| Term | One-line fact |
|------|---------------|
| **Stratospheric / Tropospheric O₃** | UV shield / pollutant + GHG |
| **Ozone depletion / Ozone hole** | Column loss / Antarctic region < 220 DU |
| **ODS / HFC** | Ozone destroyers / climate warmers (Kigali) |
| **Vienna / Montreal** | 1985 framework / 1987 binding phase-out |
| **ODP / GWP** | Ozone damage potential / heat-trapping potential |
| **Dobson Unit** | Column ozone measure |

---

## Must-score facts — DU, ODS, Vienna–Montreal–Kigali

### Science / treaties

| Item | Lock |
|------|------|
| Good / bad | Stratospheric UV shield / tropospheric pollutant |
| Share | ~90% / ~10% |
| 1 DU | ≈0.01 mm pure O₃; hole often <**220 DU** |
| Antarctic season | Sep–Oct spring |
| 1985 discovery | Farman–Gardiner–Shanklin (BAS) |
| ODS set | CFCs, Halons, CCl₄, CH₃CCl₃, CH₃Br, HCFCs… |
| HFCs | High GWP; **not** classic ODS → **Kigali** |
| Vienna / Montreal / Kigali | **1985** / **1987** (force 1989) / **2016** |
| Ozone Day | **16 Sep** |
| Montreal ≠ Kyoto | Ozone / climate |

---

## 36.1 Ozone Layer

**Identity:** Stratosphere ~15–30 km | UV shield | Dobson Unit | Chapman cycle | NO₂ regulation

- **Ozone (O₃)** is triatomic oxygen that absorbs harmful solar ultraviolet radiation.
- The protective **ozone layer** sits mainly in the lower **stratosphere** (~**15–30 km**, sometimes cited to ~**35 km**; peak near ~**25 km**). The stratosphere itself spans roughly **10–50 km**.
- About **90%** of atmospheric ozone is stratospheric; about **10%** is tropospheric.
- Natural formation/destruction follows the **Chapman cycle**: UV splits O₂ → O; O + O₂ → O₃; UV also breaks O₃.
- Stratospheric ozone is naturally regulated in part by **nitrogen dioxide (NO₂)**.
- Column thickness varies seasonally — often **highest in spring** and **lowest in the rainy season** (general pattern; the Antarctic “hole” is a separate spring-extreme story).
- **UV-A (315–400 nm)** largely reaches the surface; **UV-B (280–315 nm)** is mostly blocked by ozone; **UV-C (100–280 nm)** is blocked by O₂/O₃ and does not reach the surface under normal conditions.
- Stratospheric ozone blocks roughly **93–99%** of the most damaging solar UV.
- Near the surface, ozone is a **poisonous pollutant** (lungs/respiratory harm). In the stratosphere it is essential because it shields life from UV.
- Ozone absorption heats the stratosphere, so temperature **rises with height** there.
- Column ozone is measured in **Dobson Units (DU)**; **1 DU ≈ 0.01 mm** of pure ozone at **0°C and 1 atm**. Global average ≈ **300 DU**. Total atmospheric ozone mass is on the order of **~3 billion tonnes**.

**PYQ — UPPCS Prelims 2025, Q45**

1. Ozone layer protects Earth’s surface from solar UV.
2. Ozone depletion has been linked to CFCs.

Options: A. Only 2 B. Neither C. Both 1 and 2 D. Only 1

<details><summary>Show answer</summary>

**Ans: C**

</details>

**PYQ — UPPCS Prelims 2018, Q52 / Q57**

Maximum ozone concentration / wrong pair:

<details><summary>Show answer</summary>

**Stratosphere.** Pair **Ozone layer — Troposphere** is NOT correctly matched.

</details>

**PYQ — UPPCS Prelims 2018, Q55**

Ultra-violet radiation from sunlight causes the reaction that produces:

Options: A. CO B. SO₂ C. O₃ D. Fluorides

<details><summary>Show answer</summary>

**Ans: C**

</details>

> **Logic:** UV-shield ozone = **stratosphere**. Ground O₃ = bad. Ozone absorbs **UV**, not IR as its protective role.

---

## 36.2 Ozone Depletion

**Identity:** ODS → Cl·/Br· | Catalytic destruction | Rowland–Molina 1974 | CFC uses

- **Ozone depletion** is a sustained fall in stratospheric ozone below natural levels.
- Stable **ODS** (especially CFCs) reach the stratosphere, where UV releases **chlorine** (or bromine) radicals.
- One chlorine atom can destroy on the order of **~100,000** ozone molecules in a catalytic cycle.
- **Rowland–Molina (1974)** linked CFCs to ozone loss; Nobel Prize in Chemistry **1995**.
- CFCs are non-toxic, non-flammable, long-lived (~**80–100 years**) coolants/solvents — also strong **greenhouse gases**. Saying “CFC is not responsible for the greenhouse effect” is **false**.
- Domestic fridge coolants are often sold under names like **Mafron** (halocarbon refrigerants). Large industrial plants may use **ammonia**.
- Classic CFC uses: refrigerators/AC, foam blowing, aerosol pressurising agents, cleaning some electronic components — **not** tubeless-tyre manufacture as a standard CFC use, and **not** automobile exhaust as the primary CFC source.
- Extra UV-B raises risks of **skin cancer**, cataracts, crop damage, and marine plankton stress.
- Depletion is not the same as the greenhouse effect, though some substitutes (HFCs) are strong climate gases.

> **Logic:** Depletion link to **CFCs** is a direct UPPCS fact. Skin cancer is the classic UV-harm key — not liver/brain/oral.

---

## 36.3 Ozone Hole

**Identity:** < 220 DU | Antarctic spring | PSCs + polar vortex | Farman 1985 | Tibet 2005

- The **ozone hole** is a region of severely thinned column ozone, defined as **below 220 DU**.
- It is **not** a literal hole in the sky.
- Classic location: **Antarctic spring** (**September–October**, Southern Hemisphere). Ozone forms slowly over poles, so depletion effects are strongest there.
- Extreme cold forms **Polar Stratospheric Clouds (PSCs)**; nitric-acid chemistry on PSC surfaces activates chlorine from CFCs; the **polar front/vortex** isolates air; chlorine becomes highly active when sunlight returns.
- Discovered in **1985** by **Farman, Gardiner, and Shanklin** (British Antarctic Survey) in *Nature*; total-ozone mapping (TOMS-era) supported monitoring.
- In **2005**, **G.W. Kent Moore** (University of Toronto) and colleagues reported a low-ozone / “ozone halo” feature over the **Tibetan Plateau**.
- Arctic thinning occurs but is usually weaker/less persistent than the Antarctic hole.
- There is **no ozone hole over Uttar Pradesh / India** as an Antarctic-style seasonal feature.

> **Logic:** Hole = Antarctic spring + DU threshold + PSC/CFC chemistry — not “everywhere CFCs are used.”

---

## 36.4 Ozone Depleting Substances (ODS)

**Identity:** CFCs / Halon-1211·1301 / CCl₄ / CH₃Br / HCFCs | ODP scale

- **ODS** are chemicals controlled under the Montreal Protocol because they destroy stratospheric ozone.
- **CFCs:** refrigerants, aerosol propellants, foam blowing, electronics cleaning — long-lived chlorine sources.
- **Halons (e.g. Halon-1211, Halon-1301):** fire suppressants — bromine; often higher ozone-depleting impact per molecule.
- Also controlled: **carbon tetrachloride (CCl₄)**, **methyl chloroform (1,1,1-trichloroethane)**, **methyl bromide (CH₃Br)**, and transitional **HCFCs**.
- **ODP (Ozone Depleting Potential)** ranks destructiveness relative to **CFC-11 = 1**.
- **HFCs** were ozone-friendly replacements but have high **GWP**; controlled later by the **Kigali Amendment** (climate, not classic ODS).
- **Nitrous oxide (N₂O)** also contributes to ozone chemistry / long-term ozone loss discussions, but CFCs/halons dominate classic “ozone hole” stems.

| ODS family | Typical use | Fact |
|------------|-------------|------|
| CFCs | Cooling, aerosols, foam, electronics cleaners | Main Cl source |
| Halons (1211, 1301) | Fire fighting | Br; high ODP |
| CCl₄ / methyl chloroform / CH₃Br | Solvent / fumigant | Controlled ODS |
| HCFCs | Transitional coolants | Lower ODP; being phased out |
| HFCs | Coolants | Not ODS; Kigali / GWP |

> **Logic:** HFC ≠ ODS, but still relevant under Montreal’s Kigali track. Methyl chloroform **is** an ODS under Montreal (ignore any bank key that treats it as non-ODS).

---

## 36.5 Montreal Protocol

**Identity:** Signed 16 Sep 1987 | Force 1 Jan 1989 | Binding ODS phase-out | Ozone Day

- The **Montreal Protocol on Substances that Deplete the Ozone Layer (1987)** is the binding global treaty to phase out ODS (CFCs, halons, CCl₄, etc.).
- It operates under the **Vienna Convention** framework.
- Signed on **16 September 1987**; entered into force on **1 January 1989**.
- **16 September** is observed as **International Day for the Preservation of the Ozone Layer (World Ozone Day)** — UN observance from **1994**.
- Key amendments: London 1990, Copenhagen 1992, Montreal 1997, Beijing 1999, **Kigali 2016 (HFCs; force 1 January 2019)**.
- Often called the most successful environmental treaty — near-universal ratification; ODS production/consumption cut dramatically from peak.
- India has an **Ozone Cell under MoEFCC**; CFC phase-out completed on developing-country schedule; HCFC phase-out continues; Kigali HFC schedule applies later.

**PYQ — UPPCS Prelims 2024, Q35**

Which agreement is related to climate change mitigation?

Options: A. Montreal Protocol B. Ramsar C. Kyoto Protocol D. CITES

<details><summary>Show answer</summary>

**Ans: C** — Montreal = **ozone**, not climate mitigation.

</details>

**PYQ — UPPCS Prelims 2019, Q132**

Which is NOT correctly matched?

Options: A. Cartagena — Biosafety B. Stockholm — POPs C. Montreal — Ozone layer D. Kyoto — Save water

<details><summary>Show answer</summary>

**Ans: D** — Kyoto = climate/GHG, not “save water.” Montreal–ozone is correct.

</details>

**PYQ — UPPCS Prelims 2021, Q114 / 2022, Q82 / 2025, Q62**

World / International Ozone Day date:

<details><summary>Show answer</summary>

**16 September** (Montreal signing date).

</details>

> **Logic:** Montreal = ozone. Kyoto = climate. Never swap. Carbon trading ≠ Montreal.

---

## 36.6 Vienna Convention

**Identity:** 1985 | Framework treaty | Research + cooperation | Parent of Montreal

- The **Vienna Convention for the Protection of the Ozone Layer (1985)** is the **framework** treaty.
- It commits parties to protect the ozone layer through research, monitoring, information exchange, and cooperation.
- It does **not** itself set the detailed binding ODS phase-out schedules — that is Montreal’s job.
- Sequence fact: **Vienna 1985 → Montreal 1987**.
- Together they form the global ozone regime administered with UNEP support (Ozone Secretariat).

| Treaty | Year | Role |
|--------|------|------|
| Vienna Convention | **1985** | Framework / cooperation |
| Montreal Protocol | **1987** | Binding ODS controls |
| Kigali Amendment | **2016** (force **1 Jan 2019**) | HFC phase-down under Montreal |

> **Logic:** Vienna alone ≠ CFC ban list; Montreal adds the teeth.

---

## Complete PYQ Bank (Topic 36)

**Q1. UPPCS Prelims 2025, Q45**

With reference to the ozone layer:

1. It protects Earth's surface from solar ultraviolet radiation.
2. Ozone depletion has been linked to CFCs.

A. Only 2

B. Neither

C. Both 1 and 2

D. Only 1

<details>
<summary>Show answer</summary>

**Logic:** Both UV-shield role and CFC–depletion link are standard facts — answer both statements.

**Ans: C.** Stratospheric ozone absorbs **UV-B/C**; **CFCs** release stratospheric chlorine radicals. **Trap:** Treating ground-level O3 smog as the protective layer.

</details>

---

**Q2. UPPCS Prelims 2025, Q62 — Ozone Day pair**

International Day for the Preservation of the Ozone Layer is observed on:

A. 5 June

B. 16 September

C. 22 March

D. 2 February

<details>
<summary>Show answer</summary>

**Logic:** International Ozone Day = Montreal signing date, 16 September.

**Ans: B.** **16 September** marks Montreal Protocol signature (**1987**). **Trap:** 5 June (WED) or 22 March (Water Day).

</details>

---

**Q3. UPPCS Prelims 2024, Q35**

Which international agreement is related to climate change mitigation?

A. Montreal Protocol

B. Ramsar Convention

C. Kyoto Protocol

D. CITES

<details>
<summary>Show answer</summary>

**Logic:** Montreal = ozone/ODS; Kyoto = climate — 2024 Q35 mitigation stem.

**Ans: C.** **Kyoto** mitigates GHGs; **Montreal** phases out **ODS**. **Trap:** Montreal because it is the headline ozone treaty in this chapter.

</details>

---

**Q4. UPPCS Prelims 2023, Q52**

The ozone layer exists mainly in which atmospheric layer?

A. Troposphere

B. Mesosphere

C. Stratosphere

D. Thermosphere

<details>
<summary>Show answer</summary>

**Logic:** Ozone layer stratosphere location — overlaps Topic 35 but anchored here for ozone science.

**Ans: C.** ~**90%** of ozone is stratospheric. **Trap:** Troposphere pair in NOT-matched lists.

</details>

---

**Q5. UPPCS Prelims 2022, Q82**

On which date is Ozone Day observed?

A. October 2

B. September 16

C. April 29

D. January 30

<details>
<summary>Show answer</summary>

**Logic:** Ozone Day date recurrence — 2022 wording variant.

**Ans: B.** **16 September** — Montreal Protocol date. **Trap:** Gandhi Jayanti or Earth Day month swaps.

</details>

---

**Q6. UPPCS Prelims 2022, Q46**

Photochemical smog always contains—

A. Phosphorus

B. Ozone

C. Methane

D. Aluminium ion

<details>
<summary>Show answer</summary>

**Logic:** Photochemical smog always contains tropospheric ozone — secondary pollutant, distinct from stratospheric shield.

**Ans: B.** **NOx + VOCs + sunlight → O3 + PAN** in photochemical smog. **Trap:** Confusing this bad ground O3 with stratospheric good ozone location.

</details>

---

**Q7. UPPCS Prelims 2021, Q114**

'World Ozone Day' is celebrated on

A. 25 December

B. 21 April

C. 16 September

D. 30 January

<details>
<summary>Show answer</summary>

**Logic:** 2021 Ozone Day stem — same 16 September key.

**Ans: C.** **16 September** worldwide. **Trap:** 21 April (Earth Day).

</details>

---

**Q8. UPPCS Prelims 2019, Q132**

Which is **NOT** correctly matched?

A. Cartagena — Biosafety

B. Stockholm — POPs

C. Montreal — Ozone layer

D. Kyoto — Save water

<details>
<summary>Show answer</summary>

**Logic:** Treaty match trap — Kyoto is climate/GHG, not save water; Montreal–ozone is correct.

**Ans: D.** **Kyoto = climate/GHG mitigation**, not a water treaty. Montreal–ozone is correct. **Trap:** Kyoto paired with ozone or water because of generic environment label.

</details>

---

**Q9. UPPCS Prelims 2018, Q52 / Q55 / Q57 — set**

Consider the following:

1. Ultra-violet radiation from sunlight helps produce ozone (O3).
2. The ozone layer is located in the troposphere.

A. Only 1

B. Only 2

C. Both 1 and 2

D. Neither 1 nor 2

<details>
<summary>Show answer</summary>

**Logic:** UV radiation produces ozone photochemically; wrong pair = ozone layer in troposphere.

**Ans: A.** Statement 1 is true (UV-driven O3 chemistry). Statement 2 is false — ozone **layer = stratosphere**. **Trap:** Accepting pair ozone layer — troposphere on match lists (2018 Q57).

</details>

---

**Q10. Pattern**

The global ozone treaty sequence is:

A. Vienna Convention 1985 (framework) → Montreal Protocol 1987 (ODS phase-out; in force 1 January 1989)

B. Montreal 1985 → Vienna 1987

C. Kyoto 1987 → Montreal 1992

D. Vienna alone banned all CFCs without Montreal schedules

<details>
<summary>Show answer</summary>

**Logic:** Vienna 1985 framework → Montreal 1987 binding ODS phase-out; force date 1 Jan 1989.

**Ans: A.** **Vienna = framework/cooperation**; **Montreal = binding ODS cuts** (signed 16 Sep 1987, force **1 Jan 1989**). **Trap:** Vienna alone as full CFC ban, or reversing treaty order.

</details>

---

## Extra Drill — Ghatnachakra-pattern (Q31–Q40)

**Logic (set):** Location, measurement, ODS identity, Antarctic chemistry, treaties, and UV harm.

**Q31.** Ozone layer absorbing UV exists mainly in the:

A. Mesosphere 
B. Troposphere 
C. Stratosphere 
D. Thermosphere 

<details>
<summary>Show answer</summary>

**Ans: C.** About **90%** of ozone is stratospheric.

</details>

**Q32.** Stratospheric ozone is naturally regulated in large part by:

A. Nitrous oxide only 
B. Nitrogen dioxide 
C. CFC alone as a natural regulator 
D. Water vapour alone 

<details>
<summary>Show answer</summary>

**Ans: B.** **NO₂** helps regulate stratospheric ozone chemically.

</details>

**Q33.** Which statement about Dobson Unit is correct?

A. It measures ground-level ppm of ozone pollution only 
B. 1 DU ≈ 0.01 mm pure ozone at 0°C and 1 atm 
C. Hole threshold is always 500 DU 
D. It measures only UV-A 

<details>
<summary>Show answer</summary>

**Ans: B.** Column measure; hole often **< 220 DU**.

</details>

**Q34.** Which is **NOT** correctly matched?

A. Renneting — Cheese 
B. Golden rice — Vitamin A 
C. Genetic engineering — Plasmids 
D. Ozone layer — Troposphere 

<details>
<summary>Show answer</summary>

**Ans: D.** Ozone layer = **stratosphere**.

</details>

**Q35.** Antarctic ozone-hole chemistry is best linked to:

A. Tropospheric turbulence only 
B. Polar front + stratospheric clouds + CFCs 
C. Absence of polar front and PSCs 
D. Warm polar midsummer only 

<details>
<summary>Show answer</summary>

**Ans: B.** PSC chlorine activation + vortex isolation.

</details>

**Q36.** Who discovered the Antarctic ozone hole reported in 1985?

A. Russian team only 
B. German team only 
C. American team only 
D. British team (Farman et al.) 

<details>
<summary>Show answer</summary>

**Ans: D.** Farman, Gardiner, Shanklin — *Nature* 1985.

</details>

**Q37.** ‘Ozone halo’ over the Tibetan Plateau (2005) is linked to work led by:

A. M. Molina alone 
B. Joseph Farman alone 
C. G.W. Kent Moore 
D. Marcus Rex alone 

<details>
<summary>Show answer</summary>

**Ans: C.** University of Toronto team led by **Kent Moore**.

</details>

**Q38.** Montreal Protocol entered into force on:

A. 16 September 1987 only as force date 
B. 1 January 1989 
C. 1 January 1992 
D. 16 February 2005 

<details>
<summary>Show answer</summary>

**Ans: B.** Signed **16 Sep 1987**; force **1 Jan 1989**.

</details>

**Q39.** Harmful UV reaching the surface mainly raises risk of:

A. Liver cancer 
B. Brain cancer 
C. Oral cancer 
D. Skin cancer 

<details>
<summary>Show answer</summary>

**Ans: D.** UV-B exposure → **skin cancer** (classic key).

</details>

**Q40.** Assertion (A): Stratospheric ozone is essential for biotic life. 
Reason (R): The ozone layer protects Earth’s surface from high-energy UV radiation.

A. Both true; R explains A 
B. Both true; R does not explain A 
C. A true; R false 
D. A false; R true 

<details>
<summary>Show answer</summary>

**A/R logic:** A tests “good ozone” necessity; R tests the UV-shield mechanism (not ground-level toxicity).

**Ans: A**

</details>


## Practice Zone — UPPCS Format Drill

> **30 questions** | Original drill | ≥60% multi-statement/application | A/R · Match · chronology · NOT-matched | Keys balanced | **Ans first, then Logic**

**Q1.** With reference to ozone, which of the following statements is/are correct?

1. O₃ is diatomic oxygen identical to O₂.
2. About 90% is stratospheric; about 10% tropospheric.
3. Protective ozone layer sits mainly in the troposphere.

A. Only 1
B. Only 2
C. 1 and 2
D. 2 and 3

<details>
<summary>Show answer</summary>

**Ans: B.** Only 2 is correct.

**Logic:** Statement 2 is true (~90% stratospheric / ~10% tropospheric). Statement 1 is false — ozone is triatomic oxygen. Statement 3 is false — protective ozone is mainly stratospheric.

</details>
**Q2.** Match List-I with List-II:

| List-I | List-II |
|--------|--------|
| A. Dobson Unit | 1. 16 September |
| B. World Ozone Day | 2. Column measure (~300 DU avg) |
| C. Ozone hole season | 3. Antarctic spring Sep–Oct |

Code:

A. A-2, B-3, C-1
B. A-1, B-2, C-3
C. A-2, B-1, C-3
D. A-3, B-1, C-2

<details>
<summary>Show answer</summary>

**Ans: C.** A-2, B-1, C-3.

**Logic:** DU / Ozone Day / spring hole.

</details>

**Q3.** Which of the following pairs is/are correctly matched?

1. Vienna Convention — 1985 framework
2. Kyoto Protocol — 16 Sep 1987; force 1 Jan 1989
3. Kigali Amendment — HFC phase-down for climate (2008; force 2019)

A. 2 and 3
B. Only 1
C. 1 and 2
D. Only 3

<details>
<summary>Show answer</summary>

**Ans: B.** Only 1 is correct.

**Logic:** Only statement 1 remains true among the numbered claims. Statement 1 is true. Statement 2 is false. Statement 3 is false.

</details>

**Q4.** Given below are two statements, one labelled as Assertion (A) and the other as Reason (R).

Assertion (A): Montreal = ozone; Kyoto = climate.

Reason (R): Carbon trading belongs to the Montreal Protocol.

A. Both (A) and (R) are true, but (R) is not the correct explanation of (A)
B. (A) is false, but (R) is true
C. (A) is true, but (R) is false
D. Both (A) and (R) are true and (R) is the correct explanation of (A)

<details>
<summary>Show answer</summary>

**Ans: C.** A true, R false.

**A/R logic:** Carbon trading ≠ Montreal.

</details>

**Q5.** With reference to ODS, which of the following statements is/are correct?

1. CFCs, Halons, CCl₄, methyl chloroform, methyl bromide, HCFCs…
2. HFCs are classic ODS identical to CFCs.
3. One Cl atom can destroy ~100,000 O₃ molecules teaching.

A. 1 and 3 only
B. 1 and 2 only
C. 2 and 3 only
D. 1, 2 and 3

<details>
<summary>Show answer</summary>

**Ans: A.** Only 1 and 3.

**Logic:** HFCs = Kigali climate gases — not classic ODS.

</details>

**Q6.** Arrange Chapman / UV role:

A. Ozone absorbs mainly IR as UV shield
B. Stratospheric O₃ forms via UV on O₂; absorbs UV (not IR as main shield role)
C. Only tropospheric formation
D. Only argon catalysis

<details>
<summary>Show answer</summary>

**Ans: B.** UV formation + UV absorption.

**Logic:** Not IR as main protective role.

</details>

**Q7.** Which of the following pairs is/are NOT correctly matched?

1. Farman, Gardiner, Shanklin — Antarctic ozone hole Nature 1985
2. ODP — ozone-damage potential; GWP — heat-trapping
3. CFCs — never greenhouse gases

A. 1 and 2 only
B. 2 and 3 only
C. 3 only
D. 1, 2 and 3

<details>
<summary>Show answer</summary>

**Ans: C.** Only pair 3 is not correctly matched.

**Logic:** CFCs are also strong GHGs.

</details>

**Q8.** Given below are two statements, one labelled as Assertion (A) and the other as Reason (R).

Assertion (A): Antarctic ozone hole needs PSCs, polar vortex, and spring sunlight chemistry neighbourhood.

Reason (R): Deepest Antarctic loss is midsummer.

A. Both (A) and (R) are true, but (R) is not the correct explanation of (A)
B. (A) is false, but (R) is true
C. (A) is true, but (R) is false
D. Both (A) and (R) are true and (R) is the correct explanation of (A)

<details>
<summary>Show answer</summary>

**Ans: C.** A true, R false.

**A/R logic:** Spring — not midsummer.

</details>

**Q9.** With reference to UV bands, which of the following statements is/are correct?

1. UV-A largely reaches surface.
2. UV-B mostly blocked by ozone.
3. UV-C blocked / does not reach surface teaching.

A. 1 and 2 only
B. 2 and 3 only
C. 1 and 3 only
D. 1, 2 and 3

<details>
<summary>Show answer</summary>

**Ans: D.** All three.

**Logic:** UV-A/B/C map.

</details>

**Q10.** Match List-I with List-II:

| List-I | List-II |
|--------|--------|
| A. Stratospheric O₃ | 1. Pollutant / GHG |
| B. Tropospheric O₃ | 2. UV shield |
| C. India’s Ozone Cell | 3. MoEFCC HCFC/cooling compliance |

Code:

A. A-2, B-1, C-3
B. A-1, B-2, C-3
C. A-2, B-3, C-1
D. A-3, B-1, C-2

<details>
<summary>Show answer</summary>

**Ans: A.** A-2, B-1, C-3.

**Logic:** Good/bad ozone + Ozone Cell.

</details>

**Q11.** Arrange 1 DU meaning:

A. Surface ppm only
B. ≈0.01 mm pure ozone at 0°C and 1 atm (column)
C. Only river BOD
D. Only AQI band

<details>
<summary>Show answer</summary>

**Ans: B.** Column thickness unit.

**Logic:** Not surface ppm.

</details>

**Q12.** With reference to health/ecology of extra UV-B, which of the following statements is/are correct?

1. Skin cancer, cataracts, crop damage, plankton stress.
2. Classic stem does not centre “liver/brain/oral cancer.”
3. Extra UV-B only cools the planet.

A. 1 and 3 only
B. 2 and 3 only
C. 1 and 2 only
D. 1, 2 and 3

<details>
<summary>Show answer</summary>

**Ans: C.** Only 1 and 2.

**Logic:** Statement 3 false.

</details>

**Q13.** Given below are two statements, one labelled as Assertion (A) and the other as Reason (R).

Assertion (A): Vienna does not itself list every control measure.

Reason (R): Montreal carries the binding phase-out schedules.

A. Both (A) and (R) are true, but (R) is not the correct explanation of (A)
B. (A) is false, but (R) is true
C. (A) is true, but (R) is false
D. Both (A) and (R) are true and (R) is the correct explanation of (A)

<details>
<summary>Show answer</summary>

**Ans: D.** Both true and R explains A.

**A/R logic:** Framework vs controls.

</details>

**Q14.** Which one of the following correctly states Mafron / ammonia teaching?

A. Both banned under UNFCCC only
B. Both only used in tubeless tyres
C. Both identical to methane hydrates
D. Mafron common in domestic fridges; ammonia in many large plants

<details>
<summary>Show answer</summary>

**Ans: D.** Domestic vs large-plant coolants.

**Logic:** Refrigerant facts.

</details>

**Q15.** How many of the following CFC use facts are correct?

Refrigerators/AC; foam blowing; aerosol propellants; cleaning some electronics

A. All four
B. Only two
C. Only three
D. Only one

<details>
<summary>Show answer</summary>

**Ans: A.** All four.

**Logic:** Not tubeless-tyre centre.

</details>

**Q16.** With reference to Tibetan ozone feature, which of the following statements is/are correct?

1. G.W. Kent Moore led 2005 ozone “halo” / low-ozone report over Tibetan Plateau.
2. Formation of ozone is slower over polar regions — depletion strongest there.
3. Protective ozone sits only at ground level in cities.

A. 2 and 3 only
B. 1 and 2 only
C. 1 and 3 only
D. 1, 2 and 3

<details>
<summary>Show answer</summary>

**Ans: B.** Only 1 and 2.

**Logic:** Ground-level O₃ is pollutant.

</details>

**Q17.** Match List-I with List-II:

| List-I | List-II |
|--------|--------|
| A. ODP | 1. Heat-trapping |
| B. GWP | 2. Ozone-damage potential |
| C. NO₂ | 3. Helps regulate stratospheric ozone naturally |

Code:

A. A-2, B-3, C-1
B. A-1, B-2, C-3
C. A-2, B-1, C-3
D. A-3, B-1, C-2

<details>
<summary>Show answer</summary>

**Ans: C.** A-2, B-1, C-3.

**Logic:** ODP/GWP/NO₂.

</details>

**Q18.** Given below are two statements, one labelled as Assertion (A) and the other as Reason (R).

Assertion (A): Surface ozone is poisonous to lungs/respiratory systems.

Reason (R): Stratospheric ozone is essential as a UV shield for biotic life.

A. Both (A) and (R) are true, but (R) is not the correct explanation of (A)
B. (A) is false, but (R) is true
C. (A) is true, but (R) is false
D. Both (A) and (R) are true and (R) is the correct explanation of (A)

<details>
<summary>Show answer</summary>

**Ans: A.** Both true; stratospheric benefit does not explain surface toxicity.

**A/R logic:** Good vs bad ozone.

</details>

**Q19.** Which of the following pairs is/are correctly matched?

1. Homosphere ≈ troposphere + stratosphere + mesosphere teaching
2. Ionosphere is part of that homosphere set in usual prelims framing
3. Troposphere deepest in tropics (~18–20 km)

A. 1, 2 and 3
B. 1 and 2 only
C. 2 and 3 only
D. 1 and 3 only

<details>
<summary>Show answer</summary>

**Ans: D.** Only 1 and 3.

**Logic:** Ionosphere not in that homosphere set.

</details>

**Q20.** Arrange Kigali vs Montreal jobs:

A. Montreal ODS phase-out; Kigali HFC phase-down for climate under Montreal family
B. Kigali phases out ODS chlorine only as Montreal clone
C. Both are Kyoto carbon markets
D. Both are acid-rain treaties

<details>
<summary>Show answer</summary>

**Ans: A.** ODS vs HFC climate.

**Logic:** Family but different targets.

</details>

**Q21.** With reference to column thickness seasonality, which of the following statements is/are correct?

1. Often highest in spring and lowest in autumn teaching neighbourhood.
2. DU is a column measure.
3. DU equals river BOD mg/L.

A. 2 and 3 only
B. 1 and 2 only
C. 1 and 3 only
D. 1, 2 and 3

<details>
<summary>Show answer</summary>

**Ans: B.** Only 1 and 2.

**Logic:** DU ≠ BOD.

</details>

**Q22.** Given below are two statements, one labelled as Assertion (A) and the other as Reason (R).

Assertion (A): A stem pairing Montreal with climate mitigation is usually the Kyoto/Paris trap.

Reason (R): Montreal’s core job is ozone ODS control.

A. Both (A) and (R) are true, but (R) is not the correct explanation of (A)
B. (A) is false, but (R) is true
C. (A) is true, but (R) is false
D. Both (A) and (R) are true and (R) is the correct explanation of (A)

<details>
<summary>Show answer</summary>

**Ans: D.** Both true and R explains the trap.

**A/R logic:** Montreal ≠ climate treaty.

</details>

**Q23.** How many of the following are ODS neighbourhood substances?

CFCs, Halon-1211/1301, methyl bromide, HFCs as classic ODS

A. Only one
B. Only two
C. Only three
D. All four

<details>
<summary>Show answer</summary>

**Ans: C.** Only three.

**Logic:** HFCs not classic ODS.

</details>

**Q24.** Which one of the following correctly states acid rain vs ozone keys?

A. Acid rain = only Chapman cycle
B. Acid rain = only Montreal ODS
C. Acid rain = only Dobson Units
D. Acid rain driven by SO₂/NOx — not nitric-acid-alone matched to CFC–ozone keys

<details>
<summary>Show answer</summary>

**Ans: D.** SO₂/NOx pathway.

**Logic:** Different problem.

</details>

**Q25.** With reference to refrigerants, which of the following statements is/are correct?

1. CFCs used in foam/aerosols/AC.
2. HFCs high-GWP under Kigali.
3. HFCs are the main Antarctic PSC crystals.

A. 1 and 2 only
B. 2 and 3 only
C. 1 and 3 only
D. 1, 2 and 3

<details>
<summary>Show answer</summary>

**Ans: A.** Only 1 and 2.

**Logic:** PSCs = polar stratospheric clouds.

</details>

**Q26.** Match List-I with List-II:

| List-I | List-II |
|--------|--------|
| A. Vienna | 1. 2016 |
| B. Montreal signing | 2. 1985 |
| C. Kigali | 3. 1987 |

Code:

A. A-1, B-2, C-3
B. A-2, B-3, C-1
C. A-2, B-1, C-3
D. A-3, B-1, C-2

<details>
<summary>Show answer</summary>

**Ans: B.** A-2, B-3, C-1.

**Logic:** 1985 / 1987 / 2016.

</details>

**Q27.** Given below are two statements, one labelled as Assertion (A) and the other as Reason (R).

Assertion (A): Stratospheric ozone absorbs ~93–99% of the most damaging UV wavelengths teaching.

Reason (R): Tropospheric ozone is a pollutant and greenhouse gas.

A. Both (A) and (R) are true, but (R) is not the correct explanation of (A)
B. (A) is false, but (R) is true
C. (A) is true, but (R) is false
D. Both (A) and (R) are true and (R) is the correct explanation of (A)

<details>
<summary>Show answer</summary>

**Ans: A.** Both true; tropospheric role does not explain stratospheric UV absorption.

**A/R logic:** Two ozone roles.

</details>

**Q28.** Arrange chlorine catalytic idea:

A. One Cl harmless always
B. Only argon destroys ozone
C. One Cl can destroy ~10⁵ O₃ molecules before removal
D. Only CO₂ destroys ozone

<details>
<summary>Show answer</summary>

**Ans: C.** ~100,000 molecules.

**Logic:** Catalytic cycle.

</details>

**Q29.** With reference to World Ozone Day, which of the following statements is/are correct?

1. 16 September (Montreal signing).
2. UN observance from 1994.
3. Identical to World Environment Day 5 June.

A. 1, 2 and 3
B. 2 and 3 only
C. 1 and 3 only
D. 1 and 2 only

<details>
<summary>Show answer</summary>

**Ans: D.** Only 1 and 2.

**Logic:** Not WED.

</details>

**Q30.** Which one of the following correctly states polar depletion strength?

A. Ozone formation slower over poles — depletion effects strongest there
B. Poles always have thickest ozone with no loss
C. Only tropics have ozone holes
D. Only oceans form ozone holes

<details>
<summary>Show answer</summary>

**Ans: A.** Slower formation → strongest depletion effects.

**Logic:** Polar vulnerability.

</details>

---

## Common Traps

| Trap | Correct |
|------|------|
| Montreal = climate mitigation | **Ozone / ODS**; Kyoto = climate |
| Ozone layer in troposphere | **Stratosphere** (~90% of O₃) |
| Ozone hole over India like Antarctica | **Antarctic spring** feature |
| Vienna = detailed CFC ban alone | **Framework**; Montreal = phase-out |
| HFC = classic ODS | **High GWP**; Kigali track |
| Ozone Day = 5 June | **16 September** |
| Hole = physical hole | Column O₃ **< 220 DU** |
| Good ozone at ground level | Ground O₃ = **bad** pollutant |
| CFC not a greenhouse gas | CFCs **are** GHGs and ODS |
| CFC main source = automobiles | Refrigerants / foam / aerosols / electronics cleaners |
| CFC use = tubeless tyres | **Not** a classic CFC use |
| Fridge gas = methane | **Mafron**/halocarbon; large plants may use **ammonia** |
| Ozone absorbs IR as UV-shield role | Protective role = **UV** |
| UV harm key = liver/brain cancer | Classic key = **skin cancer** |
| Carbon trading = Montreal | **Kyoto** |
| Agenda 21 = ozone treaty | **Sustainable Development** |
| Kyoto = save water / ozone | **GHG / climate** |
| Ionosphere in homosphere | Homosphere ≈ tropo + strato + meso; **ionosphere not included** |
| Methyl chloroform not an ODS | It **is** Montreal-controlled ODS |

---

