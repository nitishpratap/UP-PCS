# Topic 6 — Electricity, Magnetism and Electromagnetism

### ★ UPPCS Revision Sheet — Lucent & Physics Wallah Advanced Notes (Zero Gaps · High-Yield Focus)

> **MANDATORY EXAM SCOPE:** Covers all dimensions of Electric Current, Potential Difference, Ohm's Law, Resistance and Resistivity, Circuits (Series vs Parallel), Electric Power and Commercial Energy Units, Safety Devices (Fuse wires, Earthing, 3-pin plug), AC vs DC Generation, Transformers and Rectifiers, Lighting Technologies (Incandescent, Fluorescent, CFL, LED, Sodium lamps), Magnetism & Electromagnetism, Renewable Solar/Wind Energy, EV Battery Cathode Chemistries, and Distributed Energy Resources. Fully synchronized with Ghatna Chakra Physics, NCERT Class 10–12, and latest 2021–2025 State PCS exam papers.

---

## Current Affairs & Exam Locks (2024–2026 Focus)

1. **Electric Vehicle (EV) Battery Cathode Chemistries (IAS Pre 2025 Lock):**
   - Lithium-ion batteries used in EVs comprise four primary parts: Cathode, Anode, Electrolyte, and Separator.
   - **Cathode (Positive Electrode):** Typically composed of **Cobalt, Lithium, and Nickel** (as NMC: $\text{LiNiMnCoO}_2$) or Lithium Iron Phosphate (LFP: $\text{LiFePO}_4$), coated on aluminium foil.
   - **Anode (Negative Electrode):** Constructed of **Graphite (Carbon)** coated on copper foil.
   - **Exam Trap Lock:** Graphite is strictly an **ANODE** material, NOT a cathode component!

2. **Alternative Powertrain Vehicles (IAS Pre 2025 Lock):**
   - Refers to propulsion systems replacing conventional internal combustion engines (petrol/diesel).
   - Classified into: (i) Full Battery Electric Vehicles (BEVs), (ii) Hydrogen Fuel Cell Vehicles (FCEVs), and (iii) Fuel Cell-Electric Hybrids. All three represent alternative powertrains.

3. **Distributed Energy Resources (DER: IAS Pre 2024 Lock):**
   - Decentralized, small-scale power generation situated near consumption sites:
     1. Rooftop solar photovoltaic units
     2. Battery storage systems
     3. Biomass generators (utilizing agricultural/waste gas)
     4. Fuel cells and mini-hydro schemes

4. **Pumped-Storage Hydropower (PSH) in Uttar Pradesh (IAS Pre 2024 Lock):**
   - MoEFCC approved massive PSH projects ($11.98\text{ GW}$), including Greenko's **$3.66\text{ GW}$ project in Sonbhadra District, Uttar Pradesh**. PSH functions as a giant grid-scale battery by pumping water to upper reservoirs during low-tariff hours and releasing it through turbines during peak demand.

5. **Silkyara Tunnel Lighting Calculation (UPPCS RO/ARO Pre 2023 Lock):**
   - $15$ bulbs of $200\text{ W}$ burning continuously for $24\text{ hours}$ consume:
     $$\text{Total Energy} = \frac{15 \times 200\text{ W} \times 24\text{ h}}{1000} = 72\text{ kWh (units)}$$

6. **Wind Power Installed Capacity in India (CGPSC / MPPSC Lock):**
   - India ranks **4th globally** in wind power capacity (after China, USA, Germany).
   - Total capacity crossed **$56\text{ GW}$**.
   - State rankings: **Gujarat is #1** ($\sim 27.9\%$, over $15.6\text{ GW}$), having overtaken **Tamil Nadu (#2**, $\sim 21.7\%$, $12.1\text{ GW}$). Madhya Pradesh ranks 7th.

7. **Global Solar Silicon Wafer Production (IAS Pre Lock):**
   - Global fab capacity: **China ranks #1**, followed by Taiwan (#2) and South Korea (#3).
   - In India, solar tariffs are regulated by **CERC and SERCs** via competitive bidding (SECI is a trading licensee, not tariff setter).

---

## Consolidated — 50 Must-Score Facts

1. Electric current is defined as the rate of flow of electric charge: $I = Q/t$; its SI unit is the **Ampere ($1\text{ A} = 1\text{ C/s}$)**.
2. By international convention, the direction of electric current is the direction of flow of positive charge, which is **opposite to the drift of electrons**.
3. **Current density ($\vec{j} = I/A$) is a VECTOR quantity** (pointing in the direction of positive charge motion), whereas electric current ($I$) and electric potential ($V$) are **scalars**.
4. The **dielectric constant** (relative permittivity $\varepsilon_r$) is a dimensionless quantity ($M^0 L^0 T^0$).
5. The kinetic energy acquired by an electron of charge $e$ falling through potential difference $V$ is **$E = eV\text{ Joules} = 1.602 \times 10^{-19}\text{ J}$** for $1\text{ volt}$.
6. The commercial unit of electrical energy is the **kilowatt-hour (kWh)**, commonly called one **'unit'**: $1\text{ kWh} = 1000\text{ W} \times 3600\text{ s} = 3.6 \times 10^6\text{ Joules}$.
7. Household electrical consumption of $250\text{ units} = 250 \times 3.6 \times 10^6 = \mathbf{9 \times 10^8\text{ Joules}}$ (71st BPSC 2025).
8. Electric power equations in simple ohmic circuits: $P = VI = I^2 R = V^2 / R$. The expression **$IR^2$ DOES NOT represent electric power** (71st BPSC 2025).
9. Resistance of a conductor: $R = \rho L / A$, where $\rho$ is specific resistance (electrical resistivity), measured in **Ohm-metre ($\Omega\cdot\text{m}$)**.
10. If a wire is stretched so that its length doubles ($L' = 2L$) at constant volume, its area halves ($A' = A/2$), causing its resistance to quadruple ($R' = 4R$).
11. Of two domestic light bulbs connected to the same voltage supply, **the dimmer bulb has a LARGER electrical resistance** ($P = V^2 / R$).
12. Connected across the mains in parallel, two identical bulbs glow significantly brighter than when connected in series ($P_{\text{parallel}} = 2V^2/R$ vs $P_{\text{series}} = V^2/2R$).
13. Resistor colour code memory key **BBROYGBVGW**: Brown = 1, Green = 5, Black = 0 (multiplier $10^0 = 1$). A resistor with bands Brown-Green-Black has resistance $(15) \times 10^0 = 15\,\Omega$; under $15\text{ V}$, it draws **$1\text{ Ampere}$** (UPPCS RO/ARO 2023).
14. Capacitive reactance is inversely proportional to frequency: $X_C = \frac{1}{2\pi f C} \propto \frac{1}{f}$.
15. Inductive reactance is directly proportional to frequency: $X_L = 2\pi f L \propto f$.
16. A **dynamo (DC generator)** converts mechanical energy into direct electrical energy using a **commutator (split rings)**.
17. An **AC generator (alternator)** converts mechanical energy into alternating current using **continuous slip rings**.
18. An **electric motor** converts electrical energy into mechanical rotational energy based on the Lorentz force on a current-carrying loop.
19. Electric motors operating at severely depressed voltages burn out because they **draw excessively high current** ($I = P/V$) to deliver rated mechanical power, generating excessive Joule heating ($I^2 R$).
20. The **Faraday constant ($F \approx 96,485\text{ C/mol}$)** represents the magnitude of electric charge per mole of electrons and is a universal physical constant.
21. Motor car lead-acid batteries have six $2\text{ V}$ cells connected in series giving a nominal **$12\text{ V}$**; capacity is rated in **ampere-hours (Ah)**; electrolyte is approx $35\%$ sulphuric acid ($\text{H}_2\text{SO}_4$) in water; positive electrode is lead dioxide ($\text{PbO}_2$) and negative electrode is spongy lead ($\text{Pb}$).
22. **Nickel-Cadmium (Ni-Cd)** rechargeable batteries use nickel oxide hydroxide and metallic cadmium electrodes with potassium hydroxide ($\text{KOH}$) alkaline electrolyte; used in calculators, cordless tools, and transistors.
23. The filament of an incandescent light bulb is made of **Tungsten** (Wolfram, W), discovered by Juan José and Fausto Elhuyar; melting point $\approx 3422^\circ\text{C}$, operating temperature $1500 - 2500^\circ\text{C}$.
24. Ordinary incandescent bulbs have short lifespans because: (i) filament thickness is non-uniform, (ii) complete vacuum cannot be maintained, (iii) support wires degrade. They are filled with inert **Argon and Nitrogen** to inhibit tungsten evaporation.
25. Incandescent bulbs convert only **$5\% - 10\%$** of electrical energy into visible light; the remaining $90\% - 95\%$ is dissipated as infrared heat.
26. **Lightning** contains tremendous electrical energy ($10^8 - 10^9\text{ Joules}$), sufficient to vaporize moisture inside a tree and cause explosive combustion.
27. A **lightning conductor** is a copper rod mounted at the apex of a building, bonded to ground through thick conductor wire, directing atmospheric surge harmlessly into Earth.
28. In a **three-pin electrical plug, the Earth pin is LONGER and THICKER** than the live and neutral pins so that the chassis is grounded first upon insertion and disconnected last upon removal.
29. Electric current **cannot flow** between two connected conductors if both are maintained at the **SAME electrical potential** ($I = \Delta V / R = 0$).
30. An **electric fuse** is a critical safety device connected in **SERIES** with the live line; it operates on the **heating effect of electric current** ($H = I^2 R t$).
31. A fuse wire must possess **HIGH RESISTIVITY and a LOW MELTING POINT**.
32. Domestic fuse wires are made of an alloy of **$63\%$ Tin (Sn) and $37\%$ Lead (Pb)**, with a eutectic melting point of only **$183^\circ\text{C}$** (vastly lower than copper at $1085^\circ\text{C}$ or aluminium at $660^\circ\text{C}$).
33. For high-current industrial circuits ($> 15\text{ A}$), tin-coated copper fuse wires are standard.
34. During a **short circuit**, line resistance drops to near zero, causing electric current to **increase drastically / surge massively** (71st BPSC 2025).
35. The filament of a **halogen lamp** is an alloy of **Tungsten and Sodium**, producing intense yellowish-white light.
36. When electric current flows through a metal wire, temperature rises because moving drift electrons collide continuously with vibrating lattice atoms, transferring kinetic energy into thermal vibrations.
37. Domestic electrical distribution wiring is connected in **PARALLEL**; every appliance receives the full rated voltage ($220\text{ V}$), and turning off one appliance does not disrupt others.
38. Standard Indian household insulation colour coding: **Red/Brown = Live**, **Black/Blue = Neutral**, **Green/Yellow-Green = Earth**.
39. The electrical resistance of a dry human body is on the order of **$10^4\,\Omega$ ($10,000\,\Omega$)** to $10^5\,\Omega$; wet or broken skin drops resistance to below $1000\,\Omega$.
40. **CFL (Compact Fluorescent Lamp)** contains mercury vapour and argon; electric discharge excites mercury atoms to emit ultraviolet radiation, which strikes inner phosphor coating to fluoresce visible white light ($22\%$ energy efficiency; lasts $6,000 - 15,000\text{ hours}$).
41. **Light Emitting Diode (LED)** operates on the principle of **ELECTROLUMINESCENCE**; electrons recombine with electron holes in a direct-bandgap semiconductor (e.g. Gallium Arsenide/Phosphide), releasing energy directly as photons.
42. LEDs consume $\sim 85\%$ less power than incandescent bulbs, emit directional light ($180^\circ$), and have an enormous operational lifespan of **$50,000 - 100,000\text{ hours}$**.
43. **Sodium vapour street lamps** are omnidirectional ($360^\circ$) and emit monochromatic yellow light ($589\text{ nm}$), producing shadowy-black colour rendering compared to high-CRI multi-spectrum LEDs.
44. The **choke coil** in a fluorescent tubelight acts as an inductor that produces a high-voltage inductive kick to ignite gas discharge, and subsequently limits and stabilizes alternating current through the tube.
45. If potential difference ($V$) and current ($I$) across a load are each doubled, power increases **fourfold** ($P' = 2V \times 2I = 4P$).
46. If $n$ small identical spherical drops charged to potential $V$ coalesce into a single large drop, the potential of the composite drop becomes **$V' = n^{2/3} V$** (UPPCS Mains).
47. Magnetic field at the centre of a flat circular coil of $N$ turns carrying current $I$: $B = \frac{\mu_0 N I}{2r}$. For $N = 100$, $r = 0.1\text{ m}$, $I = 1\text{ A} \implies B = \mathbf{6.28 \times 10^{-4}\text{ Tesla}}$ (UKPCS 2024).
48. A **transformer** functions exclusively on alternating current (AC) via **mutual electromagnetic induction**; it cannot operate on steady direct current (DC).
49. A **mobile phone charger** incorporates a **step-down transformer** (stepping down $220\text{ V AC} \to 5-10\text{ V AC}$) followed by a **rectifier** (converting AC to DC).
50. Earth's geomagnetic field is generated by convective electric currents in the molten, iron-rich outer liquid core (**geodynamo effect**).
...
| Physical Quantity | SI Unit | Dimension | Scalar / Vector |
| :--- | :--- | :---: | :---: |
| **Electric Charge ($Q$)** | Coulomb ($\text{C} = \text{A}\cdot\text{s}$) | $[A T]$ | Scalar |
| **Electric Current ($I$)** | Ampere ($\text{A}$) | $[A]$ | Scalar |
| **Current Density ($\vec{j}$)** | $\text{A/m}^2$ | $[A L^{-2}]$ | **Vector** |
| **Electric Potential / EMF ($V$)** | Volt ($\text{V} = \text{J/C}$) | $[M L^2 T^{-3} A^{-1}]$ | Scalar |
| **Electrical Resistance ($R$)** | Ohm ($\Omega = \text{V/A}$) | $[M L^2 T^{-3} A^{-2}]$ | Scalar |
| **Electrical Resistivity ($\rho$)** | Ohm-metre ($\Omega\cdot\text{m}$) | $[M L^3 T^{-3} A^{-2}]$ | Scalar |
| **Electrical Conductivity ($\sigma$)** | Siemens/metre ($\text{S/m}$) | $[M^{-1} L^{-3} T^3 A^2]$ | Scalar |
| **Capacitance ($C$)** | Farad ($\text{F} = \text{C/V}$) | $[M^{-1} L^{-2} T^4 A^2]$ | Scalar |
| **Magnetic Flux ($\Phi_B$)** | Weber ($\text{Wb}$) | $[M L^2 T^{-2} A^{-1}]$ | Scalar |
| **Magnetic Field ($B$)** | Tesla ($\text{T} = \text{Wb/m}^2$) | $[M T^{-2} A^{-1}]$ | **Vector** |
| **Dielectric Constant ($\varepsilon_r$)**| Dimensionless | $[M^0 L^0 T^0]$ | Scalar |

### Table 2: Comparative Analysis of Lighting Technologies

| Parameter | Incandescent Bulb | Fluorescent Tube | CFL | LED |
| :--- | :--- | :--- | :--- | :--- |
| **Light Generation** | Thermal incandescence of tungsten | Gas discharge of mercury vapour | Gas discharge in compact tube | Solid-state electroluminescence |
| **Efficiency** | Very low ($5 - 10\%$) | Moderate ($\sim 22\%$) | High ($\sim 25\%$) | Superior ($\sim 85 - 90\%$) |
| **Operating Temp** | $1500 - 2500^\circ\text{C}$ | Low ($\sim 40^\circ\text{C}$) | Low ($\sim 60^\circ\text{C}$) | Very low (cool running) |
...
### 6.1 Charge, Current, Potential and Circuit Laws
1. **Current and Current Density:**
   - Electric current is the macroscopic charge flow rate: $I = dq/dt$.
   - Current density $\vec{j}$ represents the microscopic distribution: $I = \int \vec{j} \cdot d\vec{A}$. In terms of carrier charge $e$, number density $n$, and drift velocity $\vec{v}_d$: $\vec{j} = n e \vec{v}_d$.
2. **Ohm's Law & Resistance:**
   - Under constant physical conditions, $V \propto I \implies V = IR$.
   - Resistivity $\rho = m / (n e^2 \tau)$, where $m$ is electron mass and $\tau$ is relaxation time.
   - Temperature dependence in metals: $\rho_T = \rho_0 [1 + \alpha(T - T_0)]$, where $\alpha > 0$ (resistance rises with temperature due to increased electron-phonon scattering).
3. **Power and Joule Heating:**
   - Heating effect: $H = I^2 R t = \frac{V^2}{R} t = V I t$.
   - Power: $P = VI = I^2 R = V^2 / R$.
   - For household appliances in parallel ($V = \text{constant}$), $P \propto 1/R$. Thus, a $40\text{ W}$ bulb has higher resistance than a $100\text{ W}$ bulb.

### 6.2 Household Safety, Earthing and Safety Engineering
1. **Electric Fuse Wire:**
   - Placed in the live wire before appliances.
   - Operating requirement: Must melt and isolate the circuit prior to current damaging appliance wiring or starting a fire.
   - Made of eutectic Tin-Lead alloy ($63\%\text{ Sn} + 37\%\text{ Pb}$), melting point $183^\circ\text{C}$. High resistivity ensures rapid local heating ($I^2 R$) during fault.
2. **Three-Pin Plug & Earthing:**
   - The earth pin is longer to ensure protective grounding is established prior to live contact. It has a larger diameter to prevent accidental insertion into the live or neutral socket.
3. **Short Circuit Physics:**
   - Direct connection between live and neutral wires reduces circuit resistance to almost zero, causing current $I = V/R \to \infty$, generating extreme instantaneous heat.

### 6.3 Electromagnetic Induction, AC vs DC and Transformers
1. **Faraday's Laws of Induction:**
   - Induced EMF $\varepsilon = -N \frac{d\Phi_B}{dt}$.
   - Negative sign represents Lenz's Law (induced current opposes the flux change causing it).
2. **AC vs DC Machinery:**
   - AC Generator uses slip rings to output alternating sinusoidal current $V = V_0 \sin(\omega t)$.
   - DC Dynamo uses a split-ring commutator to invert every negative half-cycle into unidirectional pulsing DC.
3. **Transformers:**
   - Operates on mutual induction across iron core: $\frac{V_s}{V_p} = \frac{N_s}{N_p} = \frac{I_p}{I_s}$.
   - **Direct current (DC) produces zero time-varying magnetic flux** ($d\Phi/dt = 0$), so transformers CANNOT function on DC. Connecting DC will overheat and burn the primary winding.

### 6.4 Clean Energy Transitions, Battery Storage & Electric Mobility
1. **Photovoltaic Solar Energy:**
   - Silicon solar cells exploit the photovoltaic effect: incident photons with energy $h\nu > E_g$ generate electron-hole pairs across a p-n junction, creating a DC voltage ($0.5 - 0.6\text{ V}$ per cell).
   - Net metering enables bidirectional grid exchange.
2. **Distributed Energy Resources (DER):**
   - Incorporates rooftop solar, battery energy storage systems (BESS), biomass generation, and fuel cells situated adjacent to consumer load centres.
3. **Electric Vehicle Batteries:**
   - Lithium-ion cells use intercalation chemistry:
     - Cathode: Transition metal oxides (Cobalt, Lithium, Nickel, Manganese) on aluminium.
     - Anode: Graphitic carbon on copper.
...
(a) 71 units (b) 72 units
(c) 73 units (d) None of the above
...
brown, green and black colours. What is the current
flowing through the resistor?
(a) 3 ampere (b) 2 ampere
(c) 1 ampere (d) None of the above
...
(b) They draw more current which is inversely
...
from damages in the event of lightning
strike.
Reason (R) : The electric charge conduct to ground
through the wire, instead of passing
...
connected to
(a) Ground terminal (b) Live terminal
(c) Neutral terminal (d) Any terminal
...
(a) Same charge (b) Same capacity
(c) Same resistivity (d) Same potential
...
Assertion (A): Generally an alloy of lead and tin is used
as a material of the fuse wire.
Reason (R): Alloy of lead and tin has a higher melting
point than copper and aluminium.
...
(a) It has greater external outer surface to radiate/emit the
light.
(b) There is no effect of voltage fluctuation on it.
(c) In the tube, electric energy almost converted into light
energy.
(d) None of these.
...
(c) A step up transformer (d) A step down transformer
...
(c) By Transformer (d) By Rectifier
...
(c) dimensionless (d) None of the above
...
final energy in joules would be :
(a) eV (b)
(c) (d) MeV
...
kilo-ohm, then the other resistor will be
(a) 1.403 kilo-ohm (b) 2.0 kilo-ohm
(c) 3.403 kilo-ohm (d) 4.70 kilo-ohm
...
(b) Inversely proportional to sq. root of frequency
...
energy is called –
(a) Battery (b) Motor
(c) Generator (d) Moving coil meter
...
(c) depends on the volume of the solvent in which the
...
through it.
Reason (R) : Collision of metal atoms with each other
...
(c) Photoelectric code (d) Electroluminiescence
...
Reason (R) :Light Emitting Diodes (LED) lamps are
more energy efficient as compared to the
Compact Fluorescent Lamps (CFL).
Select the correct answer using the codes given below:
...
(a) Will remain unaffected (b) Will increase four times
(c) Will increase two times (d) Will decrease half
(e) None of these
...
wire is 10 ohm, the resistance of the thicker wire is :
(a) 5 ohm (b) 10 ohm
(c) 20 ohm (d) 40 ohm
...
Assertion (A) : Transformer is useful for stepping up
or stepping down voltages.
Reason (R) : Transformer is a device used in D.C.
circuits.
...
Join @Apnapdfs
General Science General Studies G–134
...
(b) AC generator has slip rings while the DC generator
has a commutator
...
Reason (R) : Electrical system in India works at a
frequency of 60 Hz.
Code :
(a) both (A) and (R) are individually true, and (R) is the
correct explanation of (A).
(b) both (A) are (R) are individually true, but (R) is not a
correct explanation of (A).
(c) (A) is true, but (R) is false.
(d) (A) is false, but (R) is true.
...
Statement : Should all diesel engines be replaced by
electric engines in trains?
Arguments :
I. Yes, diesel engine causes a lot of pollution.
II. No, India does not produce enough electricity to
fulfil even the domestic needs.
(a) Neither argument I nor argument II is strong.
 (b) Both of the arguments are strong.
CLICK HERE - JOIN @APNAPDFS
Join @Apnapdfs
General Science General Studies G–135
...
(c) a billing mechanism for solar energy by consumers
for the electricity they add to the grid
 (d) the use of piped natural gas in the kitchens of
households
...
ranks fourth in the world
 (b) The total wind power installed capacity of India is
about 400 GW
(c) Among Indian States, maximum wind power potential
(at 100 m above ground level) is in Gujarat
(d) None of the above
...
increased.
(b) Gujarat has around 22% of the total wind power
capacity of the country.
(c) Tamil Nadu's wind power capacity was about 24% of
the country in the year 2021.
...
- **Exam Lock:** $\text{Units} = \frac{3 \times 100\text{ W} \times 10\text{ h/day} \times 30\text{ days}}{1000} = 90\text{ kWh (units)}$.
</details>
...
- **Exam Lock:** $R = V^2 / P$. The $25\text{ W}$ bulb has $4\times$ higher resistance ($R_{25} = 1936\,\Omega$) than the $100\text{ W}$ bulb ($R_{100} = 484\,\Omega$). In series, current $I$ is identical, so power dissipated $P = I^2 R$ is four times larger in the $25\text{ W}$ bulb.
</details>
...
- **Exam Lock:** By Preece's Law for fuse wires, the fusing current $I \propto r^{3/2}$. To carry a higher current without melting, the wire must have a larger cross-sectional radius.
</details>
...
- **Exam Lock:** Transformers operate strictly on alternating current via electromagnetic induction; they cannot function on direct current.
</details>
...
- **Exam Lock:** The anode is made of graphite (carbon) coated on copper foil, which intercalates lithium ions during charging. Cobalt, lithium, and nickel compose the cathode.
</details>
