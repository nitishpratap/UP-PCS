# Chemistry Topic 7 — Chemistry in Daily Life, Industry and Agriculture
### ★ UPPCS Revision Sheet — Lucent-style Comprehensive Standard (No Repetition · Practice ≥25)

<details>
<summary><strong>Covers syllabus</strong> (click to expand)</summary>

Saponification & Detergent Chemistry (Micelle Dynamics, Hard Water Scum vs Synthetic Surfactants) | Pharmaceutical Chemistry (Analgesics, Antipyretics, Antiseptics vs Disinfectants, Antibiotics, Antacids) | Food Chemistry (Preservatives, Antioxidants, Sweeteners) | Industrial Glass Chemistry (Crookes UV Glass, Pyrex Borosilicate, Optical Flint Glass, Annealing) | Portland Cement Manufacture & Setting Chemistry | Agricultural Fertilizers (Urea 46% N, Neem-Coated Urea, DAP, Superphosphates, MOP) | Soil Chemistry (Liming Acid Soils, Gypsum for Alkaline Sodic Soils) | Agrochemicals (Herbicides, Insecticides, DDT Biomagnification, Zinc Phosphide Rat Poison) | Photography Chemistry (Silver Bromide, Hypo Fixer) & Artificial Rain (Silver Iodide)

</details>

> **Sources baked in:** NCERT Class 10 & 12 (Chemistry in Everyday Life), ICAR Agriculture Chemistry Fundamentals, Lucent General Science (Chemistry), Physics Wallah (PW) State PCS Foundation, Ghatna Chakra General Science (1990–2025 PYQs).
> **Weight:** ★★★ High — Recurring questions on Urea nitrogen percentage (46%), Crookes glass UV absorption, Silver Bromide in photography vs Silver Iodide in artificial rain, Sodium chlorate as herbicide, Zinc phosphide as rodenticide, DDT as non-biodegradable pollutant, and Dettol components.
...
3. **Hard Soap vs. Soft Soap**: Soaps made with **Sodium hydroxide ($NaOH$)** are hard soaps (laundry washing bars); soaps made with **Potassium hydroxide ($KOH$)** are soft soaps (toilet luxury soaps, shaving creams, liquid handwashes).
4. **Salting Out of Soap**: Common salt (**$NaCl$**) is added to the saponification mixture to decrease the solubility product of soap, precipitating it out as a curdled solid through the common-ion effect.
5. **Micelle Cleansing Action**: A soap molecule consists of a long non-polar hydrophobic (water-repelling / oil-soluble) hydrocarbon tail and a polar hydrophilic (water-loving) ionic carboxylate head ($-COO^-Na^+$). In water, soap molecules assemble into spherical aggregates called **micelles**: the hydrophobic hydrocarbon tails point inward to trap non-polar oil/grease dirt droplets, while the hydrophilic polar heads face outward into water. Agitation forms an emulsion that is rinsed away.
6. **Why Soap Fails in Hard Water**: Hard water contains dissolved calcium ($Ca^{2+}$) and magnesium ($Mg^{2+}$) ions. These cations react with soluble sodium soap molecules to precipitate an insoluble, sticky white curd called **Scum** (calcium/magnesium stearate):
   $$2C_{17}H_{35}COONa + Ca^{2+} \to (C_{17}H_{35}COO)_2Ca\downarrow\text{ (Insoluble Scum)} + 2Na^+$$
   Scum adheres to fabrics, dulls garments, and wastes large amounts of soap before lathering can begin.
...
9. **Analgesics**: Pharmaceutical drugs that relieve or eliminate bodily pain without causing loss of consciousness. Classified into:
   - *Non-narcotic (non-opioid) analgesics*: **Aspirin** (Acetylsalicylic acid) and **Paracetamol** (Acetaminophen). Aspirin also inhibits platelet blood clotting (anti-thrombotic) and is administered to prevent heart attacks.
   - *Narcotic (opioid) analgesics*: **Morphine**, **Codeine**, and **Heroin**; derived from opium poppy (*Papaver somniferum*); relieve severe post-operative and cancer pain, but induce addiction and respiratory depression.
...
24. **Food Preservatives**: Chemical additives that prevent food spoilage caused by microbial fermentation, mold growth, or oxidative rancidity:
    - **Sodium Benzoate ($C_6H_5COONa$)**: The most widely utilized chemical preservative in commercial packaged foods (fruit squashes, tomato ketchup, jams, soft drinks). It metabolizes safely in the human liver into hippuric acid and is excreted in urine.
    - **Sodium Metabisulphite / Potassium Metabisulphite ($K_2S_2O_5$)**: Releases sulfur dioxide gas ($SO_2$), which inhibits bacterial and fungal enzymes in wines, fruit juices, and cut dried fruits.
    - **Table Salt ($NaCl$), Sugar Syrup, and Vinegar ($CH_3COOH$)**: Natural osmotic and acidic food preservatives used in traditional pickling and fruit preservation.
25. **Food Antioxidants**: Sacrificial reducing agents added to fats, oils, and fried snacks (potato chips) to retard oxidative rancidity: **BHA (Butylated Hydroxyanisole)** and **BHT (Butylated Hydroxytoluene)**. Flushing snack bags with **Nitrogen gas ($N_2$)** prevents oxygen from contacting edible oils.
26. **Glass**: An amorphous, non-crystalline, transparent supercooled inorganic liquid of high viscosity, manufactured by fusing silica (sand, $SiO_2$) with metal carbonates and cooling the melt rapidly without crystallization.
27. **Soda-Lime Glass (Soft Glass)**: Approximate composition $Na_2O \cdot CaO \cdot 6SiO_2$. Low melting point, inexpensive; used for window panes, beverage bottles, mirror plates, and cheap glassware.
28. **Potash Glass (Hard Glass)**: Made using potassium carbonate ($K_2CO_3$), calcium carbonate, and silica ($K_2O \cdot CaO \cdot 6SiO_2$). Highly resistant to chemical reagents and acids; used for high-temperature chemical laboratory equipment (flasks, combustion tubes).
29. **Pyrex Glass (Borosilicate Glass)**: Formed by substituting boron trioxide ($B_2O_3, \approx 13\%$) and alumina for alkali. Possesses an exceptionally **low thermal expansion coefficient** and high thermal shock resistance; does not crack upon rapid heating/cooling; used for standard laboratory glassware (beakers, burettes) and microwave oven cookware.
30. **Flint Glass (Optical / Lead Crystal Glass)**: Contains **Lead Oxide ($PbO, 20–30\%$)**. Possesses a high density, high refractive index, and high optical dispersion; used for manufacturing achromatic camera lenses, optical prisms, telescope objectives, and brilliant crystal chandeliers.
31. **Crookes Glass**: A specialty glass containing **Cerium Oxide ($CeO_2$)**. Cerium atoms possess electronic energy levels that selectively and sharply **absorb harmful ultraviolet (UV) rays** while transmitting visible light; used for high-protection sunglasses and optical UV shields.
32. **Photochromic Glass**: Specialty glass containing microscopic dispersed crystals of **Silver Chloride ($AgCl$) or Silver Bromide ($AgBr$)**. When exposed to sunlight (UV photons), silver halide decomposes into tiny clusters of metallic silver ($AgCl \xrightarrow{h\nu} Ag + Cl$), turning the lens dark. In dim indoor light, copper sensitizers catalyze recombination, restoring optical clarity.
33. **Annealing of Glass**: The process of slowly, uniformly cooling newly shaped hot glass articles inside a temperature-controlled lehr tunnel oven. Rapid cooling produces severe internal structural stress, causing glass to shatter spontaneously upon minor contact. Annealing removes internal stress.
34. **Glass Colorants**:
    - **Cobalt Oxide ($CoO$)**: Deep Royal Blue
    - **Ferrous Oxide ($FeO$) / Chromium Oxide ($Cr_2O_3$)**: Emerald Green
    - **Ferric Oxide ($Fe_2O_3$)**: Yellowish-Brown / Amber
    - **Cuprous Oxide ($Cu_2O$) / Gold Chloride**: Rich Ruby Red
    - **Manganese Dioxide ($MnO_2$)**: Purple / Amethyst
    - **Cadmium Sulphide ($CdS$)**: Brilliant Lemon Yellow
35. **Portland Cement**:
    - Patented in 1824 by British bricklayer **Joseph Aspdin**, named for its visual resemblance to Portland limestone building stone in Dorset, England.
    - Raw materials: Calcareous materials (Limestone, chalk, $CaCO_3 \to \mathbf{CaO, 60–65\%}$) and Argillaceous materials (Clay, shale, $\mathbf{SiO_2, 20–25\%}$; $\mathbf{Al_2O_3, 5–10\%}$; $\mathbf{Fe_2O_3, 2–3\%}$).
36. **Role of Gypsum ($CaSO_4 \cdot 2H_2O$) in Cement**:
    - Tricalcium aluminate ($C_3A, 3CaO \cdot Al_2O_3$) reacts rapidly with water, causing premature flash setting within minutes.
    - Intergrinding **$2–3\%\text{ Gypsum}$** with clinker reacts with $C_3A$ to form an insoluble coating of calcium sulphoaluminate (ettringite), which **retards the initial setting time of cement** to 30–45 minutes, providing masons sufficient working time.
37. **Essential Plant Nutrients**: Higher plants require 17 essential chemical elements:
    - Structural elements (from air/water): $C, H, O$.
    - Primary Macronutrients: **Nitrogen (N)**, **Phosphorus (P)**, **Potassium (K)**.
    - Secondary Macronutrients: Calcium (Ca), Magnesium (Mg), Sulphur (S).
    - Micronutrients: Zinc (Zn), Iron (Fe), Manganese (Mn), Copper (Cu), Boron (B), Molybdenum (Mo), Chlorine (Cl), Nickel (Ni).
38. **Urea ($NH_2CONH_2$)**:
    - The most widely utilized solid nitrogenous fertilizer in India.
    - Contains **$46\%\text{ Nitrogen (N)}$** by weight (the highest nitrogen concentration among all solid nitrogen fertilizers).
    - Synthesized by reacting anhydrous ammonia with carbon dioxide under high pressure ($2NH_3 + CO_2 \to NH_2CONH_2 + H_2O$).
    - In soil, urea is hydrolyzed by the enzyme **urease** into ammonium carbamate, which converts into ammonium ($NH_4^+$) and nitrate ($NO_3^-$) ions that are absorbed by plant roots.
39. **Neem-Coated Urea (NCU)**:
    - Mandatory in India since 2015. Urea prills are coated with a thin film of natural **Neem oil ($0.05\%$)**.
    - **Nitrification Inhibition**: Active triterpenoids in neem (nimbin, azadirachtin) inhibit soil nitrifying bacteria (*Nitrosomonas*, *Nitrobacter*), slowing the conversion of ammonium to nitrate.
    - **Benefits**: Reduces nitrogen leaching into groundwater, prevents nitrous oxide greenhouse gas emissions, boosts Nitrogen Use Efficiency (NUE) by $10–15\%$, and completely halts the illegal diversion of subsidized agricultural urea to chemical resin factories.
40. **Phosphatic Fertilizers**:
    - **Single Superphosphate (SSP)**: Contains **$16\%\text{ available } P_2O_5$**, plus $11\%$ Sulphur and $19\%$ Calcium; manufactured by treating rock phosphate with sulphuric acid ($Ca_3(PO_4)_2 + 2H_2SO_4 \to Ca(H_2PO_4)_2 + 2CaSO_4$).
    - **Triple Superphosphate (TSP)**: Contains **$46–48\%\text{ } P_2O_5$**; made by treating rock phosphate with phosphoric acid.
    - **Diammonium Phosphate (DAP)**: High-analysis complex fertilizer containing **$18\%\text{ Nitrogen (N)}$** and **$46\%\text{ Phosphorus (P}_2\text{O}_5\text{)}$**.
41. **Potassic Fertilizers**:
    - **Muriate of Potash (MOP)**: Potassium chloride ($KCl$); contains **$60\%\text{ } K_2O$**. India lacks indigenous commercial potash mineral reserves and **imports $100\%$ of its MOP consumption**.
    - **Sulphate of Potash (SOP)**: Potassium sulphate ($K_2SO_4, 50\% K_2O$); preferred for chloride-sensitive crops (tobacco, potato, grapes).
42. **Soil pH Remediation**:
    - **Acidic Soils ($\text{pH} < 6.0$)**: Contain excess $H^+$ and toxic soluble $Al^{3+}$ and $Mn^{2+}$. Corrected by **Liming**: applying agricultural **Slaked Lime ($Ca(OH)_2$)**, Quicklime ($CaO$), or powdered **Limestone ($CaCO_3$)**, which neutralizes soil acidity.
    - **Alkaline / Sodic / Usar Soils ($\text{pH} > 8.5$)**: Contain high exchangeable sodium ($Na^+$) that deflocculates soil structure. Corrected by applying **Gypsum ($CaSO_4 \cdot 2H_2O$)** or **Iron Pyrites ($FeS_2$)**:
      $$2\text{Na-Clay} + CaSO_4 \to \text{Ca-Clay} + Na_2SO_4\text{ (leached out by irrigation)}$$
43. **DDT (Dichlorodiphenyltrichloroethane)**:
    - A chlorinated organic insecticide synthesized in 1874 by Othmar Zeidler; its insecticidal properties were discovered in 1939 by Swiss chemist Paul Hermann Müller (Nobel Prize in Medicine, 1948).
    - Extremely persistent, lipophilic, and **non-biodegradable**.
...
44. **BHC / Gammaxene / Lindane**:
    - Benzene Hexachloride / Hexachlorocyclohexane ($C_6H_6Cl_6$); formed by the addition of chlorine to benzene under UV light. The $\gamma$-isomer (**Lindane**) is an active agricultural and anti-lice insecticide.
45. **Herbicides / Weedicides**: Chemical agents used to selectively or non-selectively destroy unwanted invasive weeds:
    - **2,4-D (2,4-Dichlorophenoxyacetic acid)**: A synthetic plant auxin; selectively kills broad-leaved dicotyledonous weeds without harming narrow-leaved monocot cereal crops (wheat, rice).
    - **Sodium Chlorate ($NaClO_3$)**: A powerful, non-selective chemical herbicide that desiccates and destroys all plant vegetation.
    - **Glyphosate**: Widely used systemic non-selective broad-spectrum herbicide that inhibits the plant EPSPS shikimate enzyme pathway.
46. **Rodenticides (Rat Poisons)**: Chemical poisons deployed to eliminate rodent agricultural pests:
    - **Zinc Phosphide ($Zn_3P_2$)**: Ingested rat poison; reacts with gastric hydrochloric acid in the rodent's stomach to liberate lethal, toxic **Phosphine gas ($PH_3$)**, causing heart failure and organ collapse:
      $$Zn_3P_2 + 6HCl \to 3ZnCl_2 + 2PH_3 \uparrow$$
    - **Warfarin**: Anticoagulant rodenticide; inhibits vitamin K epoxide reductase, triggering internal hemorrhaging.
47. **Artificial Rain (Cloud Seeding)**:
    - Discovered in 1946 by Bernard Vonnegut and Vincent Schaefer.
    - Seeded into subcooled moisture clouds using aircraft or ground generators.
    - Primary chemical agent: **Silver Iodide ($AgI$)**, which possesses a hexagonal crystal lattice almost identical to natural ice crystals, serving as an effective ice-nucleating template that triggers rapid ice crystal growth and rainfall.
    - Auxiliary seeding agents: **Dry Ice (solid $CO_2$)** and hygroscopic common salt ($NaCl$).
48. **Photographic Chemistry**:
    - **Light-Sensitive Salt**: Photographic film is coated with a gelatin emulsion containing microscopic crystals of **Silver Bromide ($AgBr$)** (and some $AgI$). When exposed to light, photons reduce trace silver ions to metallic silver nuclei, creating a latent image ($Ag^+ + e^- \xrightarrow{h\nu} Ag$).
    - **Developer**: Hydroquinone (quinol) reduces light-exposed silver bromide grains into black metallic silver.
    - **Fixer (Hypo Solution)**: Sodium Thiosulphate Pentahydrate (**$Na_2S_2O_3 \cdot 5H_2O$**). Dissolves unexposed, unreduced $AgBr$ crystals as a soluble thiosulphato-argentate complex, preventing the film from darkening further in sunlight:
      $$AgBr + 2Na_2S_2O_3 \to Na_3[Ag(S_2O_3)_2]\text{ (Soluble complex)} + NaBr$$
...
| **Pyrex / Borosilicate Glass** | $SiO_2\text{ (80\%)}, B_2O_3\text{ (13\%)}, Al_2O_3\text{ (3\%)}, Na_2O\text{ (4\%)}$ | **Low thermal expansion coefficient**, resists thermal shock | Chemistry beakers, flasks, pipettes, microwave oven bakeware |
| **Flint Glass (Optical / Lead Glass)** | $K_2O \cdot PbO \cdot 6SiO_2$ (contains $20–30\% PbO$) | High density, **high refractive index & high dispersion** | Optical camera lenses, microscope objectives, prisms, crystal chandeliers |
| **Crookes Glass** | Soda glass containing **Cerium Oxide ($CeO_2$)** | **Selectively absorbs harmful ultraviolet (UV) rays** | High-grade UV-absorbing sunglasses, protective industrial welding goggles |
| **Photochromic Glass** | Glass containing dispersed microcrystals of **$AgCl$ or $AgBr$** | Darkens in bright sunlight (UV photolysis); clears indoors | Variable-tint prescription eyeglasses, solar sunglasses |
| **Safety / Laminated Glass** | Two glass sheets sandwiching a layer of **polyvinyl butyral (PVB)** | Shatterproof; shards remain adhered to plastic interlayer | Automotive windshields, bullet-resistant security bank windows |
...
| **Calcium Ammonium Nitrate (CAN)** | $CaCO_3 \cdot NH_4NO_3$ | $25\%\text{ N}$ | **Strictly Neutral** | Known as **"Kisan Khad"**; safe for all soils; non-acidifying. |
| **Single Superphosphate (SSP)**| $Ca(H_2PO_4)_2 + 2CaSO_4$ | **$16\%\text{ P}_2\text{O}_5, 11\%\text{ S}, 19\%\text{ Ca}$** | Neutral / Slightly acidic | Ideal for oilseeds and pulses needing phosphorus and sulphur. |
| **Triple Superphosphate (TSP)**| $3Ca(H_2PO_4)_2$ | **$46–48\%\text{ P}_2\text{O}_5$** | Slightly acidic | High-analysis phosphatic fertilizer without gypsum byproduct. |
| **Diammonium Phosphate (DAP)** | $(NH_4)_2HPO_4$ | **$18\%\text{ N}, 46\%\text{ P}_2\text{O}_5$** | Alkaline initially, then acidic | Second most widely consumed fertilizer in India after urea. |
| **Muriate of Potash (MOP)** | $KCl$ | **$60\%\text{ K}_2\text{O}$** | Neutral | **$100\%$ imported into India**; unsuitable for chlorine-sensitive tobacco/potatoes. |
| **Sulphate of Potash (SOP)** | $K_2SO_4$ | $50\%\text{ K}_2\text{O}, 17.5\%\text{ S}$ | Neutral | Premium fertilizer for chloride-sensitive crops (tobacco, grapes). |

---

### Table 3: Agrochemical Classes & Target Pests

| Agrochemical Type | Classic Active Chemical Compounds | Target Organisms / Ecological Action | Regulatory & Environmental Status |
|---|---|---|---|
| **Insecticides (Organochlorines)** | **DDT**, **BHC / Lindane**, Endosulfan, Aldrin | Controls mosquitoes, agricultural insect pests | **Non-biodegradable; biomagnifies in raptors**; banned globally under Stockholm POPs. |
| **Insecticides (Organophosphates)** | Malathion, Parathion, Chlorpyrifos | Acetylcholinesterase inhibitors in insect nerves | Biodegradable but high acute mammalian toxicity. |
| **Selective Herbicides** | **2,4-D (2,4-dichlorophenoxyacetic acid)** | Destroys broad-leaved dicot weeds in monocot cereals | Synthetic auxin mimicking growth hormones. |
| **Non-Selective Herbicides** | **Sodium Chlorate ($NaClO_3$)**, Glyphosate | Kills all green plant vegetation indiscriminately | Used for clearing railway tracks, industrial yards, weed knockdown. |
| **Rodenticides (Rat Poisons)** | **Zinc Phosphide ($Zn_3P_2$)**, Warfarin | Stomach poison releasing lethal **Phosphine ($PH_3$)** | Standard acute poison for burrowing field rats. |
...
### 7.1 Cleansing Chemistry: Saponification, Micelles & Synthetic Surfactants

#### 1. Saponification Reaction
Soaps are manufactured by heating natural fats and oils (esters of glycerol with long-chain fatty acids) with aqueous alkali:
```
CH₂ - O - CO - C₁₇H₃₅                      CH₂ - OH
|                                          |
...
CH₂ - O - CO - C₁₇H₃₅                      CH₂ - OH
(Glyceryl Stearate / Fat)                 (Glycerol)
```
- The byproduct **Glycerol** is separated by fractional vacuum distillation and used in pharmaceuticals, cosmetics, and nitroglycerin explosives.

#### 2. Micelle Mechanics
- The hydrophobic hydrocarbon tail is lipophilic (attracted to fats, repelled by water).
- The hydrophilic carboxylate head is ionic and solvated by water molecules.
- When concentration exceeds the **Critical Micelle Concentration (CMC)**, molecules self-assemble into a spherical cluster of 100–200 molecules.
- The non-polar dirt/grease is entrapped within the hydrophobic interior core. The charged carboxylate heads on the outer shell repel other micelles, preventing coalescence and keeping the oil droplets suspended as an emulsion that washes away with water.

---

### 7.2 Pharmaceutical Chemistry & Antiseptic Physics

#### 1. Analgesics & Antipyretics
- **Aspirin (Acetylsalicylic Acid)**:
  - Synthesized by acetylating salicylic acid with acetic anhydride in the presence of sulphuric acid:
    $$C_6H_4(OH)COOH + (CH_3CO)_2O \to C_6H_4(OCOCH_3)COOH + CH_3COOH$$
  - Inhibits the enzyme cyclooxygenase (COX), blocking the biosynthesis of **prostaglandins** (pain and fever mediators) and **thromboxanes** (blood clotting agents).
  - Used in low doses ($75–150\text{ mg/day}$) as an antiplatelet prophylactic to prevent coronary thrombosis and secondary strokes.

#### 2. Antiseptic vs. Disinfectant Chemistry
- **Dettol**: The active germicide is **Chloroxylenol (4-chloro-3,5-dimethylphenol)**, blended with $\alpha$-terpineol in pine oil and soap solution. It disrupts bacterial cell membranes without irritating human skin.
- **Phenol Dichotomy**: Phenol ($C_6H_5OH$) was the historic antiseptic introduced by Joseph Lister (1865). At dilute concentrations ($0.2\%$), it denatures bacterial enzymes safely on skin. At higher concentrations ($1.0\%$), it damages human epithelial tissue and is restricted to disinfecting non-living floors and drains.

---

### 7.3 Industrial Glass Chemistry & Portland Cement Mechanics

#### 1. Specialty Glasses
- **Crookes Glass**: Doping molten silicate glass with $CeO_2$ introduces localized electron transitions that absorb electromagnetic radiation with wavelengths below $400\text{ nm}$ (ultraviolet spectrum) while transmitting visible light ($400–700\text{ nm}$).
- **Pyrex (Borosilicate)**: Incorporating $B_2O_3$ reduces the coefficient of thermal expansion from $9 \times 10^{-6}\text{ K}^{-1}$ (soda glass) to $3.3 \times 10^{-6}\text{ K}^{-1}$. When heated unevenly, thermal expansion differentials are minimal, preventing mechanical stress cracks.

#### 2. Portland Cement Hydration & Setting
- When cement is mixed with water, hydration reactions form interlocking needle-like crystals of calcium silicate hydrate (C-S-H gel) and calcium hydroxide ($Ca(OH)_2$):
  $$2(3CaO \cdot SiO_2) + 6H_2O \to 3CaO \cdot 2SiO_2 \cdot 3H_2O\text{ (C-S-H gel)} + 3Ca(OH)_2$$
- Gypsum ($CaSO_4 \cdot 2H_2O$) retards hydration of tricalcium aluminate ($C_3A$), ensuring a controlled setting profile.

---

### 7.4 Agricultural Fertilizers & Soil Remediation Chemistry

#### 1. Urea and Nitrogen Dynamics
- Urea ($NH_2CONH_2$, $M_w = 60\text{ g/mol}$) contains two nitrogen atoms:
  $$\% N = \frac{2 \times 14}{60} \times 100 = 46.67\% \approx 46\%$$
- **Enzymatic Soil Hydrolysis**:
  $$NH_2CONH_2 + 2H_2O \xrightarrow{\text{urease}} (NH_4)_2CO_3 \rightleftharpoons 2NH_4^+ + CO_3^{2-}$$
- **Nitrification by Soil Microbes**:
  $$2NH_4^+ + 3O_2 \xrightarrow{\text{Nitrosomonas}} 2NO_2^- + 4H^+ + 2H_2O$$
  $$2NO_2^- + O_2 \xrightarrow{\text{Nitrobacter}} 2NO_3^-\text{ (absorbed by roots)}$$

#### 2. Remediation of Problem Soils
- **Acidic Soils**: High rainfall leaches basic cations ($Ca^{2+}, Mg^{2+}, K^+$), leaving soil dominated by $H^+$ and $Al^{3+}$. Corrected by liming with $CaCO_3$ or $Ca(OH)_2$:
  $$Al^{3+}\text{-Clay} + 3Ca(OH)_2 \to Ca^{2+}\text{-Clay} + Al(OH)_3\downarrow$$
- **Alkaline / Sodic (Usar) Soils**: Arid evaporation concentrates exchangeable sodium ($Na^+$) on clay particles, causing soil swelling and poor drainage ($\text{pH} > 8.5$). Corrected with Gypsum ($CaSO_4 \cdot 2H_2O$):
  $$2Na^+\text{-Clay} + CaSO_4 \to Ca^{2+}\text{-Clay} + Na_2SO_4\text{ (soluble, flushed out by irrigation)}$$
...
(a) Sodium Chloride (b) Caustic Soda
...
(d) Is grown without the use of artificial fertilizers and
...
table sugar, it is not readily oxidized in the human
...
(c) Aspartame is as sweet as sugar but after ingestion into
...
(a) Pharmacological effect (b) Molecular targets
...
(c) Breaking down fat particles to the microscopic size
