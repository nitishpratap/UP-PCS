# Economy Topic 8 — Employment, Poverty, Human Capital and Social Economy
### ★ UPPCS Revision Sheet — Lucent-style (no repetition · Practice ≥26)

<details>
<summary><strong>Covers syllabus</strong> (click to expand)</summary>

Poverty line and MPI | Poverty programmes | Unemployment types | LFPR / informal / gig | Human capital | HDI | Demographic dividend | Inequality and inclusive growth | Social security

</details>

> **Sources baked in:** NCERT Class 11 Indian Economic Development (poverty, human capital), UNDP HDI/MPI notes, PLFS labour concepts, UPPCS Prelims PYQs, UKPCS Prelims
> **Weight:** ★★★ High — HDI trio, MPI 2010, Alagh nutrition line, demographic dividend 15–59, and “size of house” trap decide stems
> **Last verified:** September 2026 — Lucent remodel (History/Polity section order); Extra Drill rebuilt from mixed stems

Banking-side financial inclusion stays in **Topic 3**. This chapter owns poverty measurement, labour, human capital, HDI/MPI and inclusive-growth teaching.

---

## Current Affairs


| Year | Fact | Why it matters |
|------|------|----------------|
| **2025** | MPI introduced **2010**; it is **not** the $1/day money line. | Income poverty ≠ MPI. |
| **2025** | Social indicators of poverty include illiteracy, water, jobs — **not size of house**. | Quality of dwelling ≠ floor area. |
| **2025** | First HDR/HDI **1990** (Mahbub ul Haq); “Haq + S. Priesner” team tag fails. | Sen/capability circle, not Priesner. |
| **2025** | Human-capital investment yields future returns because education and health raise productivity. | A/R both true, R explains A. |
| **2024** | HDI uses **income, education, life expectancy** — not environment condition. | Three-pillar trap. |
| **2022** | Demographic dividend keyed to working-age **15–59** (Indian convention). | Not 60+, 0–6, or 14–50. |
| **UKPCS 2025** | **Dadabhai Naoroji** used **jail cost of living** for an early poverty-line estimate. | Pre-Independence estimate tag. |

---

## Consolidated — 42 Must-Score Facts


1. **Poverty** is the inability to meet a minimum standard of living — measured by income/consumption lines or by multidimensional deprivations.
2. India’s official poverty estimation has mainly used **household consumption** surveys, not per capita income alone.
3. **Alagh Committee (1979)** recommended a poverty line based on **nutritional / calorie requirements** exclusively in the classic key.
4. Later lines: **Lakdawala**, **Tendulkar**, **Rangarajan** — each revised basket, prices and methodology.
5. **Head Count Ratio** is the share of people below the poverty line.
6. **Dadabhai Naoroji** used the **jail cost of living** to sketch an early subsistence poverty estimate for India.
7. **Social indicators of poverty** include illiteracy, malnutrition, lack of safe drinking water / sanitation, and lack of employment opportunities.
8. **Size of house (floor area)** is **not** the standard social indicator — housing quality (materials) matters in MPI living-standard measures.
9. **Multidimensional Poverty Index (MPI)** was introduced in the **Human Development Report 2010**, replacing the **Human Poverty Index (HPI)** of **1997**.
10. Global MPI dimensions: **Health, Education, Standard of Living** (ten indicators beneath).
11. MPI is a **non-money** deprivation index — it does **not** count people by the World Bank $1/day line.
12. NITI Aayog publishes India’s **National MPI** using NFHS-based indicators.
13. Poverty causes: low productivity, unemployment / underemployment, inequality, social exclusion, shocks (health, climate, debt).
14. Alleviation mix: growth, targeted programmes, food security (Topic 6), employment guarantees, human-capital investment.
15. **MGNREGA** guarantees rural wage employment (100 days teaching) and builds local assets — a demand-side safety net.
16. **Unemployment** means willing workers cannot find work at the going wage / conditions.
17. **Disguised unemployment**: surplus labour with near-zero marginal product (classic farm surplus-labour teaching).
18. **Seasonal unemployment** follows agricultural / tourism seasons.
19. **Structural unemployment** comes from skill / sector mismatch as the economy’s structure changes.
20. **Frictional unemployment** is short job-search / transition unemployment.
21. **Cyclical unemployment** rises in demand slowdowns.
22. Indian labour-status measures: **usual status**, **weekly status**, **daily status** (NSS/PLFS teaching).
23. **Labour-force participation rate (LFPR)** is labour force as a share of population in the reference age group.
24. Female LFPR in India has been a debated low / fluctuating series — education and income effects are competing explanations in A/R stems.
25. **Informal sector** dominates employment share — low social security, high flexibility.
26. **Gig / platform work** is a growing flexible-work form with social-security design questions.
27. **Human capital** is the stock of skills, knowledge and health embodied in people.
28. Investment in education and health yields **future returns** by raising productivity.
29. **Human Development Index (HDI)** (UNDP, first HDR **1990**) combines **long and healthy life**, **knowledge**, and **decent standard of living**.
30. HDI indicators teaching: **life expectancy**, **education** (mean / expected years of schooling), **GNI per capita (PPP)** — not environmental quality.
31. **Mahbub ul Haq** led the first Human Development Report; Amartya Sen’s capabilities approach underpins the philosophy.
32. **Demographic dividend**: growth opportunity when the share of **working-age population (15–59 in Indian keys)** is high relative to dependents.
33. Reaping the dividend needs education, skills, health and jobs — age structure alone is not enough.
34. **Inclusive growth** spreads benefits across regions, castes, genders and income groups — growth plus equity.
35. Income and wealth inequality are tracked by Gini and related measures; high inequality can blunt poverty reduction.
36. Social security / protection: pensions, insurance, maternity, disability and employment guarantees.
37. Skill missions (e.g. PMKVY teaching) aim to cut structural unemployment by matching skills to demand.
38. Migration (rural–urban, inter-State) reallocates labour but can leave social-protection gaps — One Nation One Ration Card (Topic 6) helps food portability.
39. UNDP also publishes **Inequality-adjusted HDI** and **Gender Inequality Index (GII)** — GII appeared in later HDR editions (2010 teaching for GII introduction in many keys).
40. Extreme money poverty ($/day lines) and MPI answer different questions — do not merge them in stems.
41. Poverty programmes evolve (IRDP → SGSY → NRLM teaching) toward livelihood groups and financial inclusion.
42. Topic border: NFSA food entitlement → Topic 6; bank inclusion toolkit → Topic 3; external remittances → Topic 9.

---

## Confused Pairs


| A | B | Distinguishing fact |
|---|---|---|
| **Income / consumption poverty** | **MPI** | Money line vs deprivation score |
| **HPI (1997)** | **MPI (2010)** | Older UNDP poverty index vs household MPI |
| **Alagh** | **Tendulkar / Rangarajan** | Nutrition-exclusive classic vs later revisions |
| **HDI trio** | **Environment as HDI pillar** | Health–education–income; environment not in HDI |
| **Demographic dividend 15–59** | **15–64 UN band / wrong ages** | Indian key vs distractors |
| **Disguised** | **Seasonal** | Surplus labour year-round vs season-linked |
| **LFPR** | **Unemployment rate** | Participation vs joblessness among labour force |
| **House size** | **Housing quality** | Floor area ≠ MPI living-standard materials |
| **Naoroji jail-cost line** | **Tendulkar line** | Early nationalist estimate vs modern NSS line |
| **Human capital** | **Physical capital** | Skills/health in people vs machines/buildings |

## Must-score drill — HDI, MPI, labour


| Item | Tag |
|---|---|
| First HDR / HDI | 1990; Mahbub ul Haq |
| HDI pillars | Life expectancy, education, income |
| HPI → MPI | 1997 → 2010 |
| MPI dimensions | Health, education, living standard |
| Alagh | Nutrition-based poverty line |
| Demographic dividend | Working age 15–59 (UPPCS key) |
| Disguised unemployment | Near-zero marginal product surplus labour |
| Social indicator odd one | Size of house |
| Naoroji | Jail cost of living |

---

## 8.1 Poverty — lines, MPI, indicators


**Poverty** is the inability to meet a minimum accepted standard of living.

- Absolute poverty compares living standards against a fixed poverty line (income or consumption).
- Relative poverty compares a household with the living standards of others in the same society.
- Indian official estimation has mainly used **household consumption** surveys, not income alone.
- The **Head Count Ratio** is the share of people below the poverty line.
- The **poverty gap** measures how far the poor lie below the line on average (depth teaching).

### Poverty-line committees

- **Dadabhai Naoroji** used the **jail cost of living** to sketch an early subsistence poverty estimate for India.
- The **Alagh Committee (1979)** recommended a poverty line based on **nutritional / calorie requirements** exclusively in the classic key.
- The **Lakdawala Committee** updated state-specific prices and continued the calorie-linked tradition in teaching.
- The **Tendulkar Committee** shifted toward a broader consumption basket and mixed urban–rural methodology revisions.
- The **Rangarajan Committee** proposed a higher poverty line and a revised basket relative to Tendulkar.
- Do not treat Alagh, Tendulkar and Rangarajan as interchangeable — each revised method and level.

### Social indicators of poverty

- Social indicators include illiteracy, malnutrition, lack of safe drinking water, poor sanitation and lack of regular employment opportunities.
- **Size of house (floor area)** is **not** the standard social indicator in the keyed UPPCS stem.
- Housing **quality** (materials, flooring) appears in MPI living-standard indicators — that is not the same as “size of house.”

### Multidimensional Poverty Index (MPI)

- The **Human Poverty Index (HPI)** entered the Human Development Report in **1997**.
- The **Multidimensional Poverty Index (MPI)** entered the HDR in **2010** and replaced HPI in UNDP teaching.
- Global MPI uses three dimensions: **Health**, **Education** and **Standard of Living**.
- Ten indicators sit under those three dimensions in the standard Alkire–Foster teaching map.
- MPI is a **non-money** deprivation index — it does **not** count people by the World Bank $/day income line.
- **NITI Aayog** publishes India’s **National MPI** using NFHS-based indicators.
- Income poverty and MPI answer different questions; never merge them in a stem.

### Causes and alleviation (teaching map)

- Causes include low productivity, unemployment and underemployment, inequality, social exclusion, and shocks such as illness, climate stress or debt.
- Alleviation mixes growth, targeted programmes, food security (Topic 6), employment guarantees and human-capital investment.
- **MGNREGA** guarantees rural wage employment (100 days in teaching) and builds local assets as a demand-side safety net.
- Older programme lineage teaching runs IRDP → SGSY → NRLM toward livelihood groups and financial inclusion.

**UPPCS (Pre) 2020**

Which of the following Committees recommended a poverty line based on nutritional requirements exclusively?

A. Alagh Committee
B. Lakdawala Committee
C. Tendulkar Committee
D. Rangarajan Committee

<details>
<summary>Show answer</summary>

**Logic:** Alagh (1979) is the classic nutrition / calorie-exclusive poverty-line committee in this key set.

**Ans: A.** Alagh Committee.

</details>

**UPPCS (Pre) 2020**

Human Development Report 1997 introduced the concept of Human Poverty Index (HPI) but Human Development Report replaced it with Multidimensional Poverty Index (MPI) in the year

A. 1999
B. 2005
C. 2010
D. 2015

<details>
<summary>Show answer</summary>

**Logic:** MPI enters the HDR in 2010 and replaces HPI.

**Ans: C.** 2010.

</details>

**UPPCS (Pre) 2025**

With reference to the Multidimensional Poverty Index (MPI), which of the following statements is/are correct?

1. It was developed and introduced in 2010.
2. It measures extreme poverty and includes people living on less than one dollar per day.

Select the correct answer from the code given below:

A. Only 2
B. Neither 1 nor 2
C. Both 1 and 2
D. Only 1

<details>
<summary>Show answer</summary>

**Logic:** Statement 1 is true. Statement 2 confuses MPI with the World Bank money-metric extreme-poverty line — MPI is non-monetary.

**Ans: D.** Only 1.

</details>

**UPPCS (Pre) 2019**

Multidimensional poverty index includes:

i. Health
ii. Education
iii. Living Standard

Select correct answer from the codes given below:

A. Only i is correct
B. Only ii and iii are correct
C. Only i and ii are correct
D. i, ii and iii are correct

<details>
<summary>Show answer</summary>

**Logic:** All three dimensions are in the MPI.

**Ans: D.** i, ii and iii are correct.

</details>

**UPPCS (Pre) 2025**

Which of the following is NOT a social indicator of poverty?

1. Illiteracy
2. Safe drinking water
3. Employment opportunities
4. Size of house

Select the correct answer from the code given below:

A. 1 and 2
B. 3 and 4
C. Only 4
D. Only 1

<details>
<summary>Show answer</summary>

**Logic:** Illiteracy, water access and jobs are social/deprivation indicators. Floor-area “size of house” is the odd one out (MPI looks at housing materials, not size).

**Ans: C.** Only 4.

</details>

**UKPCS (Pre) 2025**

Who used the 'jail cost of living' to estimate the poverty line in India?

A. Dadabhai Naoroji
B. Mahatma Gandhi
C. C. D. Deshmukh
D. Vallabh Bhai Patel

<details>
<summary>Show answer</summary>

**Logic:** Naoroji’s early subsistence estimate used jail cost of living.

**Ans: A.** Dadabhai Naoroji.

</details>

---

## 8.2 Employment and labour


**Unemployment** means willing workers cannot find work at the going wage and conditions.

### Types of unemployment

- **Disguised unemployment** means surplus labour with near-zero marginal product — classic farm surplus-labour teaching.
- **Seasonal unemployment** follows agricultural seasons, tourism seasons or similar calendar patterns.
- **Structural unemployment** comes from skill or sector mismatch as the economy’s structure changes.
- **Frictional unemployment** is short job-search or transition unemployment between jobs.
- **Cyclical unemployment** rises when aggregate demand slows in a downturn.
- Do not swap disguised (year-round surplus labour) with seasonal (season-linked).

### Indian labour-force measures

- Indian statistics use **usual status**, **weekly status** and **daily status** reference periods (NSS / PLFS teaching).
- The **labour force** includes the employed plus the unemployed who are seeking / available for work.
- **Labour-force participation rate (LFPR)** is the labour force as a share of population in the reference age group.
- The **unemployment rate** is unemployment among the labour force — not the same as LFPR.
- Female LFPR in India has been a debated low / fluctuating series; education and income effects compete in A/R stems.

### Informal work, gig and migration

- The **informal sector** dominates employment share — low social security with high flexibility.
- **Gig / platform work** is a growing flexible-work form with social-security design questions.
- Rural–urban and inter-State migration reallocates labour but can leave social-protection gaps.
- Topic 6 neighbour: One Nation One Ration Card helps food-entitlement portability for migrants.

### Demographic dividend

- The **demographic dividend** is the growth opportunity when the share of **working-age population** is high relative to dependents.
- UPPCS keys the Indian working-age band as **15–59** for this stem set.
- UN often uses 15–64 — that band is a distractor here, not the keyed answer.
- Age structure alone is not enough; education, skills, health and jobs are needed to reap the dividend.
- Youth unemployment wastes human capital just as education investments mature.

**UPPCS (Pre) 2022**

The demographic dividend is—

A. Working population of above 60 years
B. Working population of 15 to 59 years
C. 0–6 years population
D. Working population of 14 to 50 years

<details>
<summary>Show answer</summary>

**Logic:** UPPCS keys the Indian working-age band **15–59** as the demographic-dividend population tag. (UN often uses 15–64 — not printed here.)

**Ans: B.** Working population of 15 to 59 years.

</details>

---

## 8.3 Human capital, HDI and inclusive growth


**Human capital** is the stock of skills, knowledge and health embodied in people.

- Investment in education and health yields **future returns** by raising productivity.
- Human capital differs from physical capital (machines and buildings).
- Skill missions (for example PMKVY teaching) aim to cut structural unemployment by matching skills to demand.

### Human Development Index (HDI)

- The first **Human Development Report** with an HDI appeared in **1990** under UNDP.
- **Mahbub ul Haq** led the first HDR; Amartya Sen’s capabilities approach underpins the philosophy.
- A stem pairing Haq with “S. Priesner” as co-lead fails the keyed team tag.
- HDI combines three pillars: a **long and healthy life**, **knowledge**, and a **decent standard of living**.
- Teaching indicators: **life expectancy**, education (mean / expected years of schooling), and **GNI per capita (PPP)**.
- **Condition of the environment is not an HDI pillar** — a recurring multi-statement trap.
- UNDP also publishes Inequality-adjusted HDI and the **Gender Inequality Index (GII)**.
- GII was introduced in HDR **2010** in many keys (Topic 12 neighbour).

### Inclusive growth and social security

- **Inclusive growth** spreads benefits across regions, castes, genders and income groups — growth plus equity.
- Income and wealth inequality are tracked by Gini and related measures.
- High inequality can blunt poverty reduction even when average income rises.
- Social security / protection covers pensions, insurance, maternity, disability and employment guarantees.
- Extreme money poverty ($/day lines) and MPI still answer different questions — keep them separate.

**UPPCS (Pre) 2025**

Given below are two statements, one is labelled as Assertion (A) and the other as Reason (R).

Assertion (A): Investment in human capital yields returns in the future.

Reason (R): Education and health make people more productive.

Select the correct answer from the code given below:

A. Both (A) and (R) are true, but (R) is not the correct explanation of (A)
B. (A) is false, but (R) is true
C. (A) is true, but (R) is false
D. Both (A) and (R) are true and (R) is the correct explanation of (A)

<details>
<summary>Show answer</summary>

**A/R logic:** Future returns exist because education and health raise productivity — R explains A.

**Ans: D.** Both (A) and (R) are true and (R) is the correct explanation of (A).

</details>

**UPPCS (Pre) 2024**

Which indicators are used in Human Development Index (HDI)?

1. Income level
2. Education
3. Condition of environment
4. Life expectancy

Select the correct answer from the codes given below:

A. Only 1, 2 and 3
B. Only 1 and 4
C. Only 1, 2 and 4
D. Only 1 and 2

<details>
<summary>Show answer</summary>

**Logic:** HDI = income + education + life expectancy. Environment is not an HDI pillar.

**Ans: C.** Only 1, 2 and 4.

</details>

**UPPCS (Pre) 2025**

With reference to Human Development Index, which of the following statements is/are correct?

1. The UNDP published its first Human Development Report in 1990, which had a Human Development Index.
2. The first team that developed the Human Development Index was led by Mahbub-ul-Haq and S. Priesner.

Select the correct answer using the code given below:

A. Only 2
B. Neither 1 nor 2
C. Both 1 and 2
D. Only 1

<details>
<summary>Show answer</summary>

**Logic:** Statement 1 is true. Statement 2 fails — Haq led the first HDR; S. Priesner is not the standard co-lead tag (Sen’s capabilities work is the intellectual partner usually named).

**Ans: D.** Only 1.

</details>

---

## Complete PYQ Bank (UPPCS)


**Q1. UPPCS (Pre) 2020** — Nutrition-exclusive poverty line committee

Which of the following Committees recommended a poverty line based on nutritional requirements exclusively?

A. Alagh
B. Lakdawala
C. Tendulkar
D. Rangarajan

<details>
<summary>Show answer</summary>

**Logic:** Alagh.

**Ans: A.**

</details>

**Q2. UPPCS (Pre) 2020** — HPI replaced by MPI in

Human Development Report 1997 introduced the concept of Human Poverty Index (HPI) but Human Development Report replaced it with Multidimensional Poverty Index (MPI) in the year

A. 1999
B. 2005
C. 2010
D. 2015

<details>
<summary>Show answer</summary>

**Logic:** 2010.

**Ans: C.**

</details>

**Q3. UPPCS (Pre) 2025** — MPI statements (2010 / $1 day)

With reference to the Multidimensional Poverty Index (MPI), which of the following statements is/are correct?

1. It was developed and introduced in 2010.
2. It measures extreme poverty and includes people living on less than one dollar per day.

Select the correct answer from the code given below:

A. Only 2
B. Neither
C. Both
D. Only 1

<details>
<summary>Show answer</summary>

**Logic:** Only the 2010 introduction is correct.

**Ans: D.**

</details>

**Q4. UPPCS (Pre) 2025** — NOT a social indicator of poverty

Which of the following is NOT a social indicator of poverty?

1. Illiteracy
2. Safe drinking water
3. Employment opportunities
4. Size of house

Select the correct answer from the code given below:

A. 1 and 2
B. 3 and 4
C. Only 4
D. Only 1

<details>
<summary>Show answer</summary>

**Logic:** Size of house is the odd one.

**Ans: C.**

</details>

**Q5. UPPCS (Pre) 2024** — HDI indicators

Which indicators are used in Human Development Index (HDI)?

1. Income level
2. Education
3. Condition of environment
4. Life expectancy

Select the correct answer from the codes given below:

A. 1, 2 and 3
B. 1 and 4
C. 1, 2 and 4
D. 1 and 2

<details>
<summary>Show answer</summary>

**Logic:** Drop environment.

**Ans: C.**

</details>

**Q6. UPPCS (Pre) 2025** — Human capital A/R

Given below are two statements, one is labelled as Assertion (A) and the other as Reason (R).

Assertion (A): Investment in human capital yields returns in the future.

Reason (R): Education and health make people more productive.

Select the correct answer from the code given below:

A. Both true, R does not explain
B. A false, R true
C. A true, R false
D. Both true and R explains

<details>
<summary>Show answer</summary>

**A/R logic:** Productivity channel.

**Ans: D.**

</details>

**Q7. UPPCS (Pre) 2025** — HDI 1990 / Haq–Priesner

With reference to the Multidimensional Poverty Index (MPI), which of the following statements is/are correct?

1. It was developed and introduced in 2010.
2. It measures extreme poverty and includes people living on less than one dollar per day.

Select the correct answer from the code given below:

A. Only 2
B. Neither
C. Both
D. Only 1

<details>
<summary>Show answer</summary>

**Logic:** Only the 1990 HDR/HDI statement stands.

**Ans: D.**

</details>

**Q8. UPPCS (Pre) 2022** — Demographic dividend

The demographic dividend is—

A. Above 60 years
B. 15 to 59 years
C. 0–6 years
D. 14 to 50 years

<details>
<summary>Show answer</summary>

**Logic:** 15–59 working-age band.

**Ans: B.**

</details>

**Q9. UPPCS (Pre) 2019** — MPI dimensions

Multidimensional poverty index includes:

i. Health
ii. Education
iii. Living Standard

Select correct answer from the codes given below:

A. Only health
B. Only education and living standard
C. Only health and education
D. Health, education and living standard

<details>
<summary>Show answer</summary>

**Logic:** All three.

**Ans: D.**

</details>

---

## Ghatnachakra Extra Drill — Employment, Poverty and Human Capital

> Extra Drill rebuilt from mixed RO/ARO–coaching stems (paste full Ghatnachakra Purvalokan later to expand).

UPPCS-tagged stems live in **Complete PYQ Bank (UPPCS)** above. UKPCS-tagged stems live in **UKPCS** below. Use Practice Zone for fresh multi-statement drills.

## UKPCS


**Q1. UKPCS (Pre) 2025** — Jail cost of living poverty estimate

Who used the 'jail cost of living' to estimate the poverty line in India?

A. Dadabhai Naoroji
B. Mahatma Gandhi
C. C. D. Deshmukh
D. Vallabh Bhai Patel

<details>
<summary>Show answer</summary>

**Logic:** Naoroji.

**Ans: A.**

</details>

---

## Practice Zone


**Q1.** With reference to disguised unemployment, which of the following statements is/are correct?

1. Marginal product of extra labour can be near zero.
2. It is common in overcrowded agriculture teaching.
3. It means the worker is always above 60 years by definition.

Select the correct answer from the code given below:

A. Only 3
B. 1 and 2
C. 2 and 3
D. Only 1

<details>
<summary>Show answer</summary>

**Ans: B.** Statements 1 and 2 are correct.

**Logic:** Statement 3 is false.

</details>

**Q2.** Which pair is **not** correctly matched?

A. Seasonal unemployment — agri season link
B. Structural unemployment — skill / sector mismatch
C. Frictional unemployment — short job-search transition
D. Demographic dividend — only 0–6 child population

<details>
<summary>Show answer</summary>

**Ans: D.** Dividend is working-age share, not infants.

**Logic:** A–C are standard.

</details>

**Q3.** MGNREGA mainly provides

A. Only urban IT seats
B. Rural wage employment guarantee and local asset creation
C. Only GST slabs
D. Only WPI compilation

<details>
<summary>Show answer</summary>

**Ans: B.** Rural employment guarantee.

**Logic:** Poverty / social protection tool.

</details>

**Q4.** With reference to HDI, which of the following statements is/are correct?

1. Life expectancy enters the index.
2. Education enters the index.
3. Condition of environment is a core HDI pillar.

Select the correct answer from the code given below:

A. Only 3
B. 1 and 2
C. 2 and 3
D. Only 1

<details>
<summary>Show answer</summary>

**Ans: B.** Statements 1 and 2 are correct.

**Logic:** Environment is the 2024 distractor. Statement 3 is false.

</details>

**Q5.** Given below are two statements, one labelled as Assertion (A) and the other as Reason (R).

Assertion (A): Age structure alone does not automatically deliver a demographic dividend.

Reason (R): Education, skills, health and jobs are needed to turn a working-age bulge into growth.

Select the correct answer from the code given below:

A. Both (A) and (R) are true, but (R) is not the correct explanation of (A)
B. (A) is false, but (R) is true
C. (A) is true, but (R) is false
D. Both (A) and (R) are true and (R) is the correct explanation of (A)

<details>
<summary>Show answer</summary>

**Ans: D.** Both true; human-capital and jobs explain why structure is not enough.

**A/R logic:** Dividend policy card.

</details>

**Q6.** Match List-I with List-II.

| List-I | List-II |
|---|---|
| A. Alagh | 1. 2010 MPI introduction |
| B. HDR 1990 | 2. Nutrition-based poverty line committee |
| C. HPI | 3. First HDI report |
| D. MPI replaces HPI | 4. Introduced in HDR 1997 |

Select the correct answer from the code given below:

A. 2 3 4 1
B. 3 2 4 1
C. 2 4 3 1
D. 4 3 2 1

*Row order is not the answer code.*

<details>
<summary>Show answer</summary>

**Ans: A.** Alagh; first HDR; HPI 1997; MPI 2010.

**Logic:** Date hygiene.

</details>

**Q7.** Inclusive growth means

A. Growth only for the top 1% by definition
B. Growth whose benefits reach wider sections of society
C. Only raising CRR
D. Only compiling IIP

<details>
<summary>Show answer</summary>

**Ans: B.** Broad-based sharing of growth gains.

**Logic:** Equity + growth.

</details>

**Q8.** With reference to informal employment, which of the following statements is/are correct?

1. It is a large share of India’s workforce.
2. Social security coverage is often weak.
3. Informal means the worker must be above 60 only.

Select the correct answer from the code given below:

A. Only 3
B. 1 and 2
C. 2 and 3
D. Only 1

<details>
<summary>Show answer</summary>

**Ans: B.** Statements 1 and 2 are correct.

**Logic:** Statement 3 is false.

</details>

**Q9.** Which of the following pairs is **not** correctly matched?

A. MPI — health, education, living standard
B. HDI — life expectancy, education, income
C. Naoroji — jail cost of living estimate
D. MPI — identical to people below one dollar a day

<details>
<summary>Show answer</summary>

**Ans: D.** MPI is not the $1/day count.

**Logic:** A–C are standard.

</details>

**Q10.** Head Count Ratio measures

A. Only forest cover
B. Share of population below the poverty line
C. Only repo rate
D. Only GST collections

<details>
<summary>Show answer</summary>

**Ans: B.** Poverty incidence share.

**Logic:** Basic poverty statistic.

</details>

**Q11.** Given below are two statements, one labelled as Assertion (A) and the other as Reason (R).

Assertion (A): Female LFPR trends in India attract special attention.

Reason (R): Women’s participation shapes labour supply, care economy and demographic-dividend outcomes.

Select the correct answer from the code given below:

A. Both (A) and (R) are true, but (R) is not the correct explanation of (A)
B. (A) is false, but (R) is true
C. (A) is true, but (R) is false
D. Both (A) and (R) are true and (R) is the correct explanation of (A)

<details>
<summary>Show answer</summary>

**Ans: D.** Both true; the stakes explain the attention.

**A/R logic:** LFPR policy importance.

</details>

**Q12.** Skill development mainly attacks which unemployment type?

A. Only frictional five-minute gaps forever
B. Structural mismatch between skills and jobs
C. Only seasonal mango harvests by law
D. Only cyclical boom phases

<details>
<summary>Show answer</summary>

**Ans: B.** Structural / skill mismatch.

**Logic:** PMKVY neighbour.

</details>

**Q13.** With reference to social security, which of the following statements is/are correct?

1. It includes pensions, insurance and employment guarantees in teaching.
2. It reduces vulnerability of workers and the poor.
3. It replaces HDI with WPI.

Select the correct answer from the code given below:

A. Only 3
B. 1 and 2
C. 2 and 3
D. Only 1

<details>
<summary>Show answer</summary>

**Ans: B.** Statements 1 and 2 are correct.

**Logic:** Statement 3 is false.

</details>

**Q14.** Match List-I with List-II.

| List-I | List-II |
|---|---|
| A. Disguised unemployment | 1. Job-search transition |
| B. Frictional unemployment | 2. Near-zero marginal product surplus labour |
| C. Cyclical unemployment | 3. Demand slowdown |
| D. Seasonal unemployment | 4. Agri / season calendar |

Select the correct answer from the code given below:

A. 2 1 3 4
B. 1 2 3 4
C. 2 3 1 4
D. 4 1 2 3

*Row order is not the answer code.*

<details>
<summary>Show answer</summary>

**Ans: A.** Disguised; frictional; cyclical; seasonal.

**Logic:** Typology hygiene.

</details>

**Q15.** Inequality measures such as the Gini coefficient help track

A. Only monsoon onset
B. How unevenly income or wealth is distributed
C. Only Article 280 membership
D. Only CRR vault cash

<details>
<summary>Show answer</summary>

**Ans: B.** Distribution unevenness.

**Logic:** Inclusive-growth companion.

</details>

**Q16.** Given below are two statements, one labelled as Assertion (A) and the other as Reason (R).

Assertion (A): Consumption surveys matter for India’s official poverty estimates.

Reason (R): Official lines have been anchored to household consumption, not a single income-per-head shortcut.

Select the correct answer from the code given below:

A. Both (A) and (R) are true, but (R) is not the correct explanation of (A)
B. (A) is false, but (R) is true
C. (A) is true, but (R) is false
D. Both (A) and (R) are true and (R) is the correct explanation of (A)

<details>
<summary>Show answer</summary>

**Ans: D.** Both true; the method explains why surveys matter.

**A/R logic:** Indian poverty measurement.

</details>

**Q17.** Gig economy work is best described as

A. Only permanent government cadre posts
B. Flexible / platform-mediated task work with evolving social-security design
C. Only zamindari tenure
D. Only MPI living-standard walls

<details>
<summary>Show answer</summary>

**Ans: B.** Flexible platform work.

**Logic:** Modern labour form.

</details>

**Q18.** Which statement about MPI is correct?

A. It averages only national income
B. It identifies overlapping household deprivations across dimensions
C. It is identical to the repo rate
D. It measures only forest density

<details>
<summary>Show answer</summary>

**Ans: B.** Overlapping deprivations at household level.

**Logic:** Why MPI replaced HPI’s country-average weakness.

</details>

**Q19.** With reference to human capital formation, which of the following statements is/are correct?

1. Education builds human capital.
2. Health builds human capital.
3. Human capital is identical to steel plants.

Select the correct answer from the code given below:

A. Only 3
B. 1 and 2
C. 2 and 3
D. Only 1

<details>
<summary>Show answer</summary>

**Ans: B.** Statements 1 and 2 are correct.

**Logic:** Statement 3 is false.

</details>

**Q20.** Which of the following pairs is **not** correctly matched?

A. Tendulkar Committee — poverty-line revision
B. Rangarajan Committee — poverty-line revision
C. Alagh Committee — nutrition-based line
D. Alagh Committee — first Human Development Report 1990 author

<details>
<summary>Show answer</summary>

**Ans: D.** HDR 1990 is Haq/UNDP, not Alagh.

**Logic:** A–C are standard.

</details>

**Q21.** Usual status / weekly status / daily status are

A. Only GST return types
B. Labour-force measurement reference periods in Indian statistics teaching
C. Only HDI pillars
D. Only Gati Shakti engines

<details>
<summary>Show answer</summary>

**Ans: B.** PLFS/NSS unemployment measurement concepts.

**Logic:** Employment statistics card.

</details>

**Q22.** Given below are two statements, one labelled as Assertion (A) and the other as Reason (R).

Assertion (A): Safe drinking water access is used as a social / living-standard poverty indicator.

Reason (R): Lack of safe water is a direct deprivation affecting health and dignity.

Select the correct answer from the code given below:

A. Both (A) and (R) are true, but (R) is not the correct explanation of (A)
B. (A) is false, but (R) is true
C. (A) is true, but (R) is false
D. Both (A) and (R) are true and (R) is the correct explanation of (A)

<details>
<summary>Show answer</summary>

**Ans: D.** Both true; deprivation logic explains the indicator.

**A/R logic:** 2025 social-indicator neighbour.

</details>

**Q23.** Migration can raise incomes yet still raise vulnerability because

A. Social protection may not travel with the worker
B. Workers stop needing food forever
C. HDI abolishes States
D. MPI becomes a currency

<details>
<summary>Show answer</summary>

**Ans: A.** Portability gaps in entitlements / security.

**Logic:** Topic 6 ONORC neighbour.

</details>

**Q24.** Match List-I with List-II.

| List-I | List-II |
|---|---|
| A. Gini | 1. Rural employment guarantee |
| B. MGNREGA | 2. Inequality measure |
| C. LFPR | 3. Labour force ÷ population share |
| D. Capability approach | 4. Sen’s human-development philosophy |

Select the correct answer from the code given below:

A. 2 1 3 4
B. 1 2 3 4
C. 2 3 1 4
D. 4 1 2 3

*Row order is not the answer code.*

<details>
<summary>Show answer</summary>

**Ans: A.** Gini; MGNREGA; LFPR; Sen.

**Logic:** Social-economy toolkit.

</details>

**Q25.** Youth unemployment is especially costly because

A. It wastes human capital just as education investments mature
B. It raises forest cover automatically
C. It sets the repo rate
D. It compiles WPI

<details>
<summary>Show answer</summary>

**Ans: A.** Idle educated / young workers.

**Logic:** Dividend risk card.

</details>

**Q26.** With reference to Tendulkar Committee, which of the following statements is/are correct?

1. It revised India’s poverty estimation methodology.
2. It is one of the post-Alagh poverty-line exercises.
3. It launched Make in India in 2014.

Select the correct answer from the code given below:

A. Only 3
B. 1 and 2
C. 2 and 3
D. Only 1

<details>
<summary>Show answer</summary>

**Ans: B.** Statements 1 and 2 are correct.

**Logic:** Statement 3 is false.

</details>

**Q27.** Given below are two statements, one labelled as Assertion (A) and the other as Reason (R).

Assertion (A): HDI was created to go beyond income-only rankings.

Reason (R): Income alone does not capture health and education outcomes that shape human well-being.

Select the correct answer from the code given below:

A. Both (A) and (R) are true, but (R) is not the correct explanation of (A)
B. (A) is false, but (R) is true
C. (A) is true, but (R) is false
D. Both (A) and (R) are true and (R) is the correct explanation of (A)

<details>
<summary>Show answer</summary>

**Ans: D.** Both true; insufficiency of income explains HDI’s design.

**A/R logic:** Haq/Sen purpose.

</details>

**Q28.** National MPI for India is associated mainly with

A. Only RBI note issue
B. NITI Aayog’s multidimensional poverty monitoring using survey indicators
C. Only Article 360
D. Only Tobin tax

<details>
<summary>Show answer</summary>

**Ans: B.** NITI National MPI.

**Logic:** Domestic MPI product.

</details>

---

## Common Traps


* MPI **2010** ≠ $1/day line; HPI **1997** → MPI **2010**.
* HDI = **life + education + income** — not environment.
* First HDR **1990** (Haq); Priesner co-lead tag fails.
* Social indicators: **not size of house**.
* Alagh = nutrition-exclusive line; Naoroji = jail cost of living.
* Demographic dividend = **15–59** in UPPCS key (not 0–6 / 60+ / 14–50).
* Disguised ≠ seasonal; LFPR ≠ unemployment rate.
* Human capital returns via **productivity** (education + health).
