---
hide:
 - toc
---

<div class="fact-lock-hero" markdown="0">
<p class="fact-lock-hero__eyebrow">Geography · Part 1</p>
<h1 class="fact-lock-hero__title">Fundamentals of Physical Geography</h1>
<div class="fact-fact-meta">
<span class="fact-fact-pill fact-fact-pill--high">🔴 Very high priority</span>
<span class="fact-fact-pill">Prelims ~8.5/10</span>
</div>
<p class="fact-lock-hero__lead">Direct factual questions from <strong>Earth movements, latitude–longitude, interior of Earth, rocks/minerals, plate tectonics, earthquakes, volcanoes and physiographic features</strong>. UPPSC Prelims papers from 2017–2025 are the core PYQ window for this part.</p>
<p class="fact-lock-hero__rule"><strong>Booklet rule:</strong> Don't try to understand everything here. <strong>Memorise the highlighted associations, tables and traps.</strong></p>
</div>

<nav class="fact-lock-jump" markdown="0" aria-label="Jump to section">
<span class="fact-lock-jump__label">Jump</span>
<a href="#1-earth-basic-facts">1–4 Earth</a>
<a href="#5-latitude">5–8 Lat/Long</a>
<a href="#12-heat-zones">9–14 Zones</a>
<a href="#15-earths-interior">15–20 Interior</a>
<a href="#21-rocks">21–27 Rocks</a>
<a href="#28-continental-drift">28–37 Plates</a>
<a href="#38-major-mountain-types">38–41 Landforms</a>
<a href="#42-volcano-basic-terminology">42–47 Volcanoes</a>
<a href="#48-earthquakes">48–52 Earthquakes</a>
<a href="#56-one-line-geography-associations">56 Facts</a>
<a href="#confused-pairs">Pairs</a>
<a href="#uppcs-trap-statements">Traps</a>
<a href="#self-test-20-mcqs">Self test</a>
</nav>

<div class="fact-lock-sheet" markdown="1">

## 1. 🌍 Earth — Basic Facts

### 🔴 Must ratta

| Fact | Answer |
| --- | --- |
| Shape of Earth | Oblate spheroid / Geoid |
| Equatorial diameter | ~12,756 km |
| Polar diameter | ~12,714 km |
| Difference | ~42 km |
| Mean radius | ~6,371 km |
| Equatorial circumference | ~40,075 km |
| Polar circumference | ~40,008 km |
| Earth's age | ~4.54 billion years |
| Earth's only natural satellite | Moon |
| Earth's axis inclination | **23½°** from perpendicular to ecliptic |
| Earth's axis inclination to orbital plane | **66½°** |
| Mean distance from Sun | ~149.6 million km |
| Revolution period | ~365¼ days |
| Rotation period | ~24 hours |
| Rotation direction | **West → East** |
| Revolution direction | **West → East** |
| Earth's orbital path | Elliptical |
| Sun's apparent annual path | Ecliptic |

### 🧠 Important distinction

> **23½° → tilt of Earth's axis from perpendicular to ecliptic**

> **66½° → tilt of Earth's axis with orbital plane**


## 2. 🔄 Rotation of Earth

### Rotation = Earth spins on its own axis

**Direction:** West → East

Therefore:

> **Sun appears to rise in East and set in West.**

### Effects of rotation

| Effect | Ratta |
| --- | --- |
| Day & night | Rotation |
| Apparent daily movement of Sun | Rotation |
| Difference in local time | Rotation + longitude |
| Coriolis force | Rotation |
| Flattening at poles | Rotation |
| Bulging at equator | Rotation |

### 🔴 Coriolis Force

| Hemisphere | Deflection |
| --- | --- |
| Northern Hemisphere | **Right** |
| Southern Hemisphere | **Left** |
| Equator | Zero |

**Mnemonic:**

> **N → Right | S → Left**

### Coriolis force is maximum

> **At the poles**

### Coriolis force is zero

> **At the Equator**

!!! trap "Coriolis trap"
 Coriolis force **does not** create wind or determine wind speed — it only **deflects** the direction of moving air and water.


## 3. ☀️ Revolution of Earth

Earth revolves around the Sun in an **elliptical orbit**.

### Important dates

| Date | Event | Sun overhead |
| --- | --- | --- |
| **21 March** | March Equinox | Equator |
| **21 June** | Summer Solstice | Tropic of Cancer |
| **23 September** | September Equinox | Equator |
| **22 December** | Winter Solstice | Tropic of Capricorn |

Dates can vary by a day depending on the year.

### 🔴 Equinox

**March + September**

> Day ≈ Night throughout the Earth.

### 🔴 Solstice

**June + December**

> Maximum seasonal contrast between hemispheres.


## 4. 🌞 Perihelion & Aphelion

| Term | Meaning | Approx. date |
| --- | --- | --- |
| **Perihelion** | Earth closest to Sun | **3 January** |
| **Aphelion** | Earth farthest from Sun | **4 July** |

!!! trap "Seasons trap"
 **Seasons are NOT caused by Earth's changing distance from the Sun.**

 Main cause: **23½° axial tilt + revolution around Sun**


## 5. 🌐 Latitude

Latitude = angular distance **north or south of Equator**.

Measured:

> **0°–90° N/S**

### Important parallels

| Latitude | Name |
| ---: | --- |
| 0° | Equator |
| 23½° N | **Tropic of Cancer** |
| 23½° S | **Tropic of Capricorn** |
| 66½° N | **Arctic Circle** |
| 66½° S | **Antarctic Circle** |
| 90° N | North Pole |
| 90° S | South Pole |

### 🔴 Indian relevance

**Tropic of Cancer passes through 8 Indian states:**

> Gujarat → Rajasthan → Madhya Pradesh → Chhattisgarh → Jharkhand → West Bengal → Tripura → Mizoram

!!! mnemonic "Tropic of Cancer — 8 states"
 **Guj-Raj-MP-Chha-Jha-WB-Tri-Mizo**

 Gujarat → Rajasthan → Madhya Pradesh → Chhattisgarh → Jharkhand → West Bengal → Tripura → Mizoram


## 6. 🌍 Longitude

Longitude = angular distance **east or west of Prime Meridian**.

Range:

> **0°–180° E/W**

### Prime Meridian

> **0° longitude**

Passes through:

> **Greenwich, London**

### International Date Line

Approximately:

> **180° longitude**

But it is **not a perfectly straight line**; it deviates to avoid dividing countries/islands.


## 7. ⏰ Time & Longitude

Earth rotates:

> **360° in 24 hours**

Therefore:

> **15° = 1 hour**

> **1° = 4 minutes**

> **1′ longitude = 4 seconds**

### 🔴 Golden Formula

> **Time difference = Longitude difference × 4 minutes**

### Example

Delhi and a place differ by 10° longitude.

> 10 × 4 = **40 minutes**


## 8. 🇮🇳 Indian Standard Time

Indian Standard Meridian:

> **82°30′ E**

Passes near:

> **Mirzapur, Uttar Pradesh**

IST:

> **UTC/GMT + 5 hours 30 minutes**

!!! prelims "IST fact"
 **82°30′ E → IST → Mirzapur, UP** — NOT 82° E.


## 9. 🌐 Important Latitude Facts

### Equator

* 0°
* divides Earth into Northern and Southern Hemispheres
* longest latitude
* great circle

### All parallels of latitude

* run east-west
* are parallel to one another
* never meet
* decrease in length toward poles.

### 🔴 Important

> **Equator is the only latitude that is also a great circle among the parallels.**


## 10. 🌍 Great Circle

A great circle:

> Divides Earth into two equal halves.

Examples:

* Equator
* Any meridian + its opposite meridian

### ⚠️ Trap

**All meridians individually are not complete great circles.**

A meridian combined with its opposite meridian forms a great circle.


## 11. 🧭 Meridians vs Parallels

| Feature | Latitude | Longitude |
| --- | --- | --- |
| Direction | East–West | North–South |
| Reference | Equator | Prime Meridian |
| Range | 0–90° N/S | 0–180° E/W |
| Lines | Parallel | Meet at poles |
| Length | Varies | Equal |
| Major use | Climate zones | Time calculation |

### 🔴 One-line ratta

> **Latitude → Climate**

> **Longitude → Time**


## 12. 🌎 Heat Zones

| Zone | Latitude |
| --- | --- |
| Torrid Zone | Tropic of Cancer → Tropic of Capricorn |
| North Temperate | Tropic of Cancer → Arctic Circle |
| South Temperate | Tropic of Capricorn → Antarctic Circle |
| North Frigid | Arctic Circle → North Pole |
| South Frigid | Antarctic Circle → South Pole |

### 🔴 Torrid Zone

> **23½°N → 23½°S**

Sun can be directly overhead somewhere within this zone.

### Important

> **Sun can never be overhead at 30° latitude.**

Because direct overhead sun is limited to:

> **23½°N to 23½°S**


## 13. 🌞 Solstice — Exact Ratta

### 21 June

> Sun overhead at **Tropic of Cancer**

Northern Hemisphere:

> **Summer**

Southern Hemisphere:

> **Winter**

### 22 December

> Sun overhead at **Tropic of Capricorn**

Northern Hemisphere:

> **Winter**

Southern Hemisphere:

> **Summer**


## 14. 🌗 Equinox

### Around 21 March

> Sun overhead at Equator

Northern Hemisphere:

> Spring

Southern Hemisphere:

> Autumn

### Around 23 September

> Sun overhead at Equator

Northern Hemisphere:

> Autumn

Southern Hemisphere:

> Spring


## 15. 🌍 Earth's Interior

Earth is broadly divided into:

> **Crust → Mantle → Core**

Evidence about Earth's interior comes especially from **earthquake waves, gravity, heat flow, magnetic field and laboratory studies**.


## 16. 🥚 Crust

Outermost layer.

### Two major types

| Type | Oceanic crust | Continental crust |
| --- | --- | --- |
| Thickness | Thinner | Thicker |
| Composition | Basaltic | More granitic |
| Density | Higher | Lower |
| Age | Generally younger | Generally older |
| Main rocks | Basalt | Granite |

### 🔴 Ratta

> **Oceanic → thin + dense + basaltic**

> **Continental → thick + less dense + granitic**


## 17. 🌋 Mantle

Extends from:

> Moho → ~2,900 km depth

Composition dominated by:

> **Silicate minerals rich in Mg and Fe**

Mantle is predominantly solid but can flow extremely slowly over geological time.


## 18. 🔥 Core

Mainly:

> **Iron + Nickel**

Two parts:

| Layer | State |
| --- | --- |
| Outer core | **Liquid** |
| Inner core | **Solid** |

### 🔴 Why outer core is liquid?

S-waves do not pass through it.

### 🔴 Why inner core is solid?

Extremely high pressure maintains solid state despite high temperature.

### 🧠 Golden association

> **S-wave → cannot pass through liquid**

> **Outer core → liquid**


## 19. ⚡ Discontinuities

| Discontinuity | Separates |
| --- | --- |
| **Conrad** | Upper & lower continental crust |
| **Moho / Mohorovičić** | Crust & mantle |
| **Gutenberg** | Mantle & outer core |
| **Lehmann** | Outer core & inner core |

### 🔴 Most important

> **Moho → Crust/Mantle**

> **Gutenberg → Mantle/Core**

> **Lehmann → Outer Core/Inner Core**


## 20. 🪨 Lithosphere & Asthenosphere

### Lithosphere

> **Crust + uppermost rigid mantle**

Characteristics:

* rigid
* brittle
* broken into tectonic plates

### Asthenosphere

> Weak/ductile zone below lithosphere

Characteristics:

* hotter
* plastic/ductile
* allows plate movement

### 🔴 Trap

> **Lithosphere ≠ crust**

Lithosphere includes:

> **Crust + uppermost mantle**


## 21. 🪨 Rocks

Three major rock types:

> **Igneous → Sedimentary → Metamorphic**


## 22. 🔥 Igneous Rocks

Formed by:

> **Cooling and solidification of magma/lava**

### Intrusive

Magma cools:

> **Below Earth's surface**

Cooling:

> Slow

Crystals:

> Large

Examples:

> **Granite, Gabbro**

### Extrusive

Lava cools:

> **At/on Earth's surface**

Cooling:

> Rapid

Crystals:

> Small

Examples:

> **Basalt, Pumice, Obsidian**

### 🔴 Ratta

> **Granite → Intrusive**

> **Basalt → Extrusive**


## 23. 🪨 Sedimentary Rocks

Form through:

> Deposition → compaction → cementation

Characteristics:

* generally layered
* fossils commonly found
* often porous

Examples:

> **Sandstone, Limestone, Shale, Conglomerate**

### 🔴 Fossils

> **Most commonly associated with sedimentary rocks**

### ⚠️ Trap

Metamorphism can destroy fossils.


## 24. 🔥 Metamorphic Rocks

Pre-existing rocks altered by:

> **Heat + Pressure**

without complete melting.

| Original | Metamorphic |
| --- | --- |
| Limestone | **Marble** |
| Shale | **Slate** |
| Granite | **Gneiss** |
| Sandstone | **Quartzite** |
| Coal | **Graphite** |

### 🔴 Must ratta

> **Limestone → Marble**

> **Shale → Slate**

> **Sandstone → Quartzite**

> **Granite → Gneiss**


## 25. 🔄 Rock Cycle

> Magma
> ↓ cooling
> **Igneous rock**
> ↓ weathering/erosion/deposition
> **Sedimentary rock**
> ↓ heat + pressure
> **Metamorphic rock**
> ↓ melting
> **Magma**

### ⚠️ Important

Any rock can potentially transform into another rock type under appropriate geological conditions.


## 26. 💎 Minerals

A mineral is naturally occurring, inorganic and has:

* definite chemical composition
* ordered crystalline structure.

### Important mineral associations

| Mineral | Important association |
| --- | --- |
| Quartz | Silicon |
| Feldspar | Major rock-forming mineral |
| Mica | Sheets/flaky |
| Calcite | Limestone |
| Hematite | Iron ore |
| Magnetite | Iron ore |
| Bauxite | Aluminium ore |
| Galena | Lead ore |
| Chalcopyrite | Copper ore |
| Cinnabar | Mercury ore |


## 27. 🧠 Rock-Forming Minerals

Major rock-forming minerals:

> **Feldspar + Quartz + Mica + Pyroxene + Amphibole + Olivine**

### 🔴 Most abundant mineral group in Earth's crust

> **Feldspar**

### 🔴 Most abundant element in Earth's crust

> **Oxygen**

### Second:

> **Silicon**

### Ratta:

> **O > Si > Al > Fe**


## 28. 🌍 Continental Drift

Proposed by:

> **Alfred Wegener**

Year:

> **1912**

Theory:

> Continents were once joined in a supercontinent.

Supercontinent:

> **Pangaea**

Surrounding ocean:

> **Panthalassa**

### Pangaea divided into:

> **Laurasia + Gondwanaland**


## 29. 🧩 Wegner's Evidences

### 🔴 Major evidence

**1. Jigsaw fit**

> South America ↔ Africa

**2. Fossils**

> Mesosaurus fossils in South America + Africa

**3. Geological similarity**

Similar rock formations across continents.

**4. Glacial evidence**

Ancient glacial deposits found in present tropical regions.

**5. Coal deposits**

Coal deposits in Antarctica indicate past different climatic conditions.

### ⚠️ Main weakness

Wegener could not satisfactorily explain:

> **What force moved the continents?**


## 30. 🌊 Sea-Floor Spreading

Associated strongly with:

> **Harry Hess**

Process:

> Magma rises at mid-ocean ridges → cools → creates new oceanic crust → older crust moves away.

### 🔴 Key association

> **Mid-Ocean Ridge → Divergent boundary → New crust**


## 31. 🧲 Paleomagnetism

Magnetic minerals preserve information about Earth's magnetic field when rocks form.

### Important evidence

Symmetrical magnetic stripes on either side of mid-ocean ridges:

> **Evidence for sea-floor spreading**


## 32. 🌎 Plate Tectonics

Modern theory combines:

> Continental drift + Sea-floor spreading + movement of lithospheric plates

Earth's lithosphere is divided into moving plates. Earthquakes, volcanoes and mountain belts are strongly concentrated around plate boundaries.


## 33. 🔴 Three Types of Plate Boundaries

| Boundary | Movement | Main features |
| --- | --- | --- |
| **Divergent** | Apart | Mid-ocean ridges, rift valleys |
| **Convergent** | Towards | Trenches, volcanoes, mountains |
| **Transform** | Past each other | Faults, earthquakes |


## 34. ↔️ Divergent Boundary

Movement:

> **→ ←**

Actually plates move:

> **← →**

Creates:

* new crust
* mid-ocean ridges
* rift valleys

### Examples

> **Mid-Atlantic Ridge**

> **East African Rift**

### 🔴

> **Divergent → Constructional boundary**

Because new crust is produced.


## 35. 💥 Convergent Boundary

Movement:

> **→ ←**

Three important combinations:

### Oceanic + Continental

> Oceanic plate subducts

Produces:

> **Volcanic mountains**

Example:

> Andes

### Oceanic + Oceanic

Produces:

> **Island arc + trench**

Example:

> Japan / Aleutians

### Continental + Continental

Neither easily subducts.

Produces:

> **Fold mountains**

Example:

> **Himalayas**

### 🔴

> **India + Eurasia → Himalayas**


## 36. 🔄 Transform Boundary

Movement:

> **⇆**

Plates slide horizontally past one another.

Produces:

> **Earthquakes**

Classic example:

> **San Andreas Fault**

### ⚠️ Trap

Transform boundary:

> **Does NOT normally create or destroy crust.**


## 37. 🌋 Hotspots

Volcanic activity can occur:

> **Away from plate boundaries**

Examples:

> **Hawaii**

### 🔴

> **Hawaii → Hotspot / intraplate volcanism**


## 38. 🏔️ Major Mountain Types

### Fold mountains

Formed mainly by:

> **Compression**

Examples:

> Himalayas · Alps · Andes · Rockies

### Block mountains

Formed mainly by:

> **Faulting**

Example:

> Vosges · Black Forest

### Volcanic mountains

Formed by:

> **Volcanic activity**

Examples:

> Fuji · Kilimanjaro · Mauna Loa

### Residual mountains

Formed by:

> **Long-term erosion**

Examples:

> Aravalli


## 39. 🇮🇳 Important Indian Mountain Associations

| Mountain | Ratta |
| --- | --- |
| Himalayas | Young fold |
| Aravalli | Very old fold/residual |
| Western Ghats | Faulted/escarpment-related |
| Eastern Ghats | Discontinuous, highly eroded |
| Nilgiri | Meeting region of Western & Eastern Ghats |
| Vindhya | Block/structural highland association |
| Satpura | Block mountain association |

### ⚠️ Important

> **Himalayas → Young fold**

> **Aravalli → Among the oldest fold mountain systems**


## 40. 🏔️ Plateaus

Plateau:

> **Elevated relatively flat-topped landform**

### Important types

| Type | Example |
| --- | --- |
| Intermontane | Tibetan Plateau |
| Piedmont | Appalachian Plateau |
| Continental | Peninsular Plateau |
| Volcanic | Deccan Plateau |

### 🔴

> **Tibetan Plateau → Highest and largest plateau association**

> **Deccan Plateau → Volcanic/basaltic association**


## 41. 🌾 Plains

Plain:

> Broad area of relatively low relief.

### Important types

| Type | Formation |
| --- | --- |
| Alluvial plain | River deposition |
| Floodplain | River flooding/deposition |
| Coastal plain | Marine/coastal processes |
| Loess plain | Wind deposition |
| Glacial plain | Glacial deposition |


## 42. 🌋 Volcano — Basic Terminology

| Term | Meaning |
| --- | --- |
| Magma | Molten material below surface |
| Lava | Magma reaching surface |
| Vent | Opening through which magma/lava emerges |
| Crater | Depression/opening around volcanic vent |
| Caldera | Large volcanic depression formed by collapse/explosion |
| Ash | Fine volcanic particles |
| Tephra | General term for airborne volcanic fragments |

### 🔴

> **Magma underground → Lava at surface**


## 43. 🌋 Volcano Types

### Shield volcano

Characteristics:

> Broad + gently sloping

Lava:

> Low-viscosity basaltic lava

Example:

> **Mauna Loa**

### Composite volcano / Stratovolcano

Characteristics:

> Steep + layered

Associated with:

> Lava + ash/pyroclastic materials

Example:

> **Mount Fuji**

### Cinder cone

> Small, steep volcanic cone formed mainly from pyroclastic fragments.


## 44. 🌋 Volcanic Terms — Must Ratta

> **Mauna Loa → Shield volcano**

> **Fuji → Composite volcano**

> **Hawaii → Hotspot**

> **Ring of Fire → Pacific Ocean margins**


## 45. 🌋 Active / Dormant / Extinct

| Type | Meaning |
| --- | --- |
| Active | Currently erupting or likely to erupt |
| Dormant | Currently inactive but may erupt again |
| Extinct | Not expected to erupt again |

### ⚠️ Trap

**Dormant ≠ extinct**

Dormant volcano can become active.


## 46. 🇮🇳 Important Indian Volcanoes

| Volcano | Location | Ratta |
| --- | --- | --- |
| Barren Island | Andaman & Nicobar | **India's only confirmed active volcano** |
| Narcondam | Andaman & Nicobar | Volcanic island |
| Deccan Traps | Peninsular India | Ancient basaltic lava |

### 🔴

> **Barren Island → Active volcano**

> **Deccan Traps → Basaltic lava flows**


## 47. 🌋 Deccan Traps

Formed due to:

> Massive ancient volcanic eruptions

Rock:

> **Basalt**

Type:

> **Flood basalt / large igneous province**

### Trap

> **Deccan Traps are NOT a mountain range.**

They are a vast:

> **Basaltic lava plateau**


## 48. 🌐 Earthquakes

Earthquake:

> Sudden release of energy within Earth's crust/lithosphere producing seismic waves.

Main cause:

> **Movement along faults / tectonic activity**

### 🔴

> **Focus/Hypocentre → Point inside Earth where earthquake originates**

> **Epicentre → Point on surface directly above focus**


## 49. 🌊 Earthquake Waves

### P-waves

> **Primary waves**

Characteristics:

* fastest
* compressional
* travel through solids + liquids + gases

### S-waves

> **Secondary waves**

Characteristics:

* slower than P
* transverse/shear
* travel through solids
* **cannot travel through liquids**

### Surface waves

> Travel along Earth's surface.

Generally:

> **Most destructive**


## 50. 🧠 Earthquake Wave Ratta

> **P → Primary → fastest → solid + liquid + gas**

> **S → Secondary → shear → solid only**

> **Surface → surface → highly destructive**


## 51. 📏 Earthquake Magnitude vs Intensity

### Magnitude

Measures:

> **Energy released at source**

Common modern measure:

> **Moment Magnitude Scale (Mw)**

### Intensity

Measures:

> **Observed effects/damage at a particular location**

Traditionally:

> **Modified Mercalli Intensity (MMI)**

### 🔴

> **Magnitude → earthquake itself**

> **Intensity → effect at a place**


## 52. 🌊 Tsunami

Usually caused by:

> **Large undersea earthquake involving sudden vertical displacement of seafloor**

Other possible causes:

* volcanic eruption
* submarine landslide
* rarely meteorite impact.

### ⚠️ Trap

> **Every undersea earthquake does NOT produce a tsunami.**

A major tsunami generally requires significant displacement of the seafloor/water column.


## 53. 🏔️ Mountain — High-Yield Associations

| Feature | Association |
| --- | --- |
| Himalayas | Young fold |
| Andes | Oceanic–continental convergence |
| Alps | Young fold |
| Rockies | Fold mountains |
| Mid-Atlantic Ridge | Divergent |
| Himalayas | Continental–continental convergence |
| San Andreas | Transform |
| Japan | Island arc/subduction |
| Andes | Volcanic mountain chain |
| Hawaii | Hotspot |


## 54. 🔥 Internal vs External Forces

### Endogenic forces

Originate:

> **Inside Earth**

Examples:

* earthquakes
* volcanism
* folding
* faulting
* mountain building

### Exogenic forces

Operate:

> **At/near Earth's surface**

Examples:

* weathering
* erosion
* deposition
* rivers
* glaciers
* wind
* waves

### 🔴

> **Endogenic → Construction/uplift**

> **Exogenic → Weathering/erosion/deposition**


## 55. 🧠 Important Geographical Terms

| Term | Meaning |
| --- | --- |
| Lithosphere | Rigid outer shell |
| Asthenosphere | Weak/ductile zone below lithosphere |
| Magma | Molten material below surface |
| Lava | Magma at surface |
| Epicentre | Surface point above focus |
| Focus | Underground origin of earthquake |
| Rift | Long narrow zone of crustal stretching |
| Trench | Deep oceanic depression |
| Ridge | Elevated elongated submarine feature |
| Fault | Fracture along which movement occurs |
| Fold | Bending of rock layers |
| Delta | Depositional landform at river mouth |
| Estuary | Tidal river mouth, generally funnel-shaped |
| Basin | Depression/area drained by a river system |
| Watershed | Boundary separating drainage basins |
| Gorge | Deep narrow valley |
| Canyon | Deep valley with steep sides |
| Gorge vs canyon | Both deep valleys; usage often reflects regional terminology |


## 56. 🔴 One-Line Geography Associations

Memorise these as **X → Y**:

<div class="fact-fact-chips" markdown="1">

> **Moho → Crust–Mantle**

> **Gutenberg → Mantle–Outer Core**

> **Lehmann → Outer Core–Inner Core**

> **P-wave → Liquid possible**

> **S-wave → No liquid**

> **Lithosphere → Rigid**

> **Asthenosphere → Ductile**

> **Pangaea → Wegener**

> **Sea-floor spreading → Harry Hess**

> **Mid-ocean ridge → New crust**

> **Divergent → Plates move apart**

> **Convergent → Plates move together**

> **Transform → Plates slide past**

> **Himalayas → Continental–continental**

> **Andes → Oceanic–continental**

> **Hawaii → Hotspot**

> **San Andreas → Transform**

> **Ring of Fire → Pacific**

> **Barren Island → Active volcano**

> **Deccan Traps → Basalt**

> **Limestone → Marble**

> **Shale → Slate**

> **Sandstone → Quartzite**

> **Granite → Gneiss**

> **Latitude → Climate**

> **Longitude → Time**

> **82°30′E → IST**

> **23½°N → Tropic of Cancer**

> **23½°S → Tropic of Capricorn**

> **66½°N → Arctic Circle**

> **66½°S → Antarctic Circle**

</div>


## Confused Pairs {#confused-pairs}

One row = one distinction. Memorise the **fact** column.

| Pair | Fact |
| --- | --- |
| **23½° vs 66½°** | 23½° → Tropics · 66½° → Polar Circles |
| **Rotation vs Revolution** | Rotation → day/night · Revolution + tilt → seasons |
| **Perihelion vs Aphelion** | Perihelion → nearest → **January** · Aphelion → farthest → **July** |
| **Focus vs Epicentre** | Focus → underground · Epicentre → surface |
| **Magnitude vs Intensity** | Magnitude → energy released · Intensity → damage at a place |
| **Magma vs Lava** | Magma → below surface · Lava → at surface |
| **Lithosphere vs Crust** | Crust → one layer · Lithosphere → crust + uppermost mantle |
| **Weathering vs Erosion** | Weathering → breakdown in place · Erosion → removal/transport |
| **Delta vs Estuary** | Delta → deposition · Estuary → tidal/funnel river mouth |
| **Fold vs Fault** | Fold → bending · Fault → fracture + displacement |

## UPPCS Trap Statements {#uppcs-trap-statements}

| # | False statement (do not pick) | Correct fact |
| ---: | --- | --- |
| 1 | Seasons are caused because Earth is closer to the Sun in summer | **Axial tilt + revolution** |
| 2 | All longitudes are parallel | **Longitudes meet at the poles** |
| 3 | Latitude determines local time | **Longitude determines local time** |
| 4 | S-waves travel through liquids | **S-waves → solids only** |
| 5 | Oceanic crust is thicker than continental crust | **Oceanic → thinner but denser** |
| 6 | Himalayas formed at oceanic–continental convergence | **Continental–continental (C–C)** |
| 7 | New crust is produced at transform boundaries | **Divergent → new crust** |
| 8 | San Andreas Fault is a convergent boundary | **Transform boundary** |
| 9 | Hawaii lies on a typical plate boundary | **Hotspot / intraplate volcanism** |
| 10 | Barren Island is a dormant volcano (to remember) | **India's only confirmed active volcano** |

## Common Traps — Quick Scan {#common-traps-quick-scan}

| Wrong link | Correct |
| --- | --- |
| Seasons → distance from Sun | Tilt + revolution |
| Latitude → time | Longitude |
| Longitude → climate | Latitude |
| S-wave → passes liquid | Solids only |
| Oceanic crust → thick | Thin + dense |
| Himalaya → O–C | C–C |
| San Andreas → convergent | Transform |
| Hawaii → plate boundary | Hotspot |
| Lithosphere → crust only | Crust + uppermost mantle |
| Magnitude → damage at one place | Intensity |

## Top 20 Ratta Facts {#top-20-ratta-facts}

1. **Earth axis → 23½°**
2. **Perpendicular to ecliptic → 23½°**
3. **Orbital plane → 66½°**
4. **Rotation → West to East**
5. **Rotation → Day/night**
6. **Revolution + tilt → Seasons**
7. **21 June → Tropic of Cancer**
8. **22 December → Tropic of Capricorn**
9. **Equinox → Equator**
10. **Perihelion → January**
11. **Aphelion → July**
12. **1° longitude → 4 minutes**
13. **IST → 82°30′E**
14. **Moho → Crust/Mantle**
15. **Gutenberg → Mantle/Core**
16. **S-wave → Cannot pass liquid**
17. **Pangaea → Wegener**
18. **Mid-ocean ridge → New crust**
19. **Himalayas → C–C convergence**
20. **San Andreas → Transform**

## Rapid Revision Chips {#rapid-revision-chips}

<div class="fact-fact-chips" markdown="1">

> **23½ → Tilt**

> **66½ → Orbital plane**

> **15° → 1 hour**

> **1° → 4 minutes**

> **82½°E → IST**

> **Moho → Crust/Mantle**

> **Gutenberg → Mantle/Core**

> **Lehmann → Outer/Inner Core**

> **P → Fast + liquid**

> **S → Solid only**

> **Wegener → Continental Drift**

> **Hess → Sea-floor spreading**

> **Divergent → New crust**

> **Convergent → Mountains/Trenches**

> **Transform → Earthquakes**

> **Himalaya → C–C**

> **Andes → O–C**

> **Hawaii → Hotspot**

> **Barren → Active**

> **Deccan → Basalt**

</div>

## Must Ratta — Master Block {#must-ratta-master-block}

<div class="fact-fact-review" markdown="1">

**Earth** — Rotation W→E · Revolution W→E · Axis 23½° · Perihelion Jan · Aphelion Jul · Seasons = tilt + revolution

**Latitude/Longitude** — Equator 0° · Tropic Cancer 23½°N · Tropic Capricorn 23½°S · Arctic 66½°N · Antarctic 66½°S · Prime Meridian 0° · 1° long = 4 min · IST 82°30′E Mirzapur

**Interior** — Moho Crust/Mantle · Gutenberg Mantle/Core · Lehmann Outer/Inner Core · Outer core liquid · Inner core solid · Lithosphere rigid · Asthenosphere ductile

**Rocks** — Granite intrusive · Basalt extrusive · Limestone→Marble · Shale→Slate · Sandstone→Quartzite · Granite→Gneiss · Fossils sedimentary

**Plate tectonics** — Wegener drift · Hess sea-floor spreading · Divergent new crust · Convergent subduction/collision · Transform neither create nor destroy · Himalayas C–C · Andes O–C · San Andreas transform · Hawaii hotspot

</div>

## PYQ Connection {#pyq-connection}

!!! prelims "High-value formats"
 1. **Match the following** — discontinuity, rock, mountain, volcano.
 2. **Statement-based** — Earth movements and seasons.
 3. **Assertion–Reason** — plate tectonics / earthquakes.
 4. **Location association** — volcano, fault, mountain.
 5. **Conceptual elimination** — P vs S waves.
 6. **Numerical factual** — longitude / time.
 7. **Sequence / order** — Earth's internal layers.
 8. **Cause–effect** — plate movement → landform.
 9. **Rock association** — parent rock → metamorphic rock.
 10. **Latitude association** — tropics / polar circles.

## Self Test — 20 MCQs {#self-test-20-mcqs}

**Q1.** The angle between Earth's axis and the perpendicular to its orbital plane is:

A. 66½°
B. 23½°
C. 90°
D. 45°

**Q2.** Which movement of Earth primarily causes day and night?

A. Revolution
B. Rotation
C. Precession
D. Nutation

**Q3.** The Tropic of Cancer is located at:

A. 23½°S
B. 66½°N
C. 23½°N
D. 0°

**Q4.** If two places differ by 20° longitude, their approximate local-time difference is:

A. 20 minutes
B. 40 minutes
C. 80 minutes
D. 120 minutes

**Q5.** Indian Standard Meridian is:

A. 75°E
B. 82°30′E
C. 90°E
D. 82°E

**Q6.** The Moho discontinuity separates:

A. Mantle and core
B. Outer and inner core
C. Crust and mantle
D. Lithosphere and asthenosphere

**Q7.** Which seismic wave cannot travel through liquids?

A. P-wave
B. S-wave
C. Surface wave
D. Rayleigh wave

**Q8.** Which is an intrusive igneous rock?

A. Basalt
B. Pumice
C. Granite
D. Obsidian

**Q9.** Limestone changes into which metamorphic rock?

A. Slate
B. Marble
C. Gneiss
D. Quartzite

**Q10.** Who proposed the continental drift theory?

A. Harry Hess
B. Alfred Wegener
C. Arthur Holmes
D. Charles Lyell

**Q11.** Sea-floor spreading is strongly associated with:

A. Mid-ocean ridges
B. Ocean trenches only
C. Transform faults only
D. Fold mountains

**Q12.** The Himalayas primarily resulted from:

A. Oceanic–oceanic convergence
B. Oceanic–continental convergence
C. Continental–continental convergence
D. Transform movement

**Q13.** San Andreas Fault represents:

A. Divergent boundary
B. Convergent boundary
C. Transform boundary
D. Subduction boundary

**Q14.** Hawaii is best associated with:

A. Continental collision
B. Hotspot volcanism
C. Mid-ocean ridge
D. Transform fault

**Q15.** Barren Island is associated with:

A. Active volcanism
B. Fold mountains
C. Coral reef formation only
D. Rift valley

**Q16.** Which statement is correct?

A. Lithosphere consists only of crust.
B. Asthenosphere is more rigid than lithosphere.
C. Lithosphere includes crust and uppermost rigid mantle.
D. Asthenosphere is the Earth's outermost layer.

**Q17.** The point on Earth's surface directly above an earthquake's focus is called:

A. Focus
B. Epicentre
C. Fault
D. Seismic gap

**Q18.** Perihelion refers to:

A. Earth's maximum distance from Sun
B. Earth's minimum distance from Sun
C. Earth's maximum speed of rotation
D. Earth's minimum axial tilt

**Q19.** Which one is correctly matched?

A. Shale — Marble
B. Limestone — Slate
C. Sandstone — Quartzite
D. Granite — Marble

**Q20.** Which plate boundary is generally associated with the formation of new crust?

A. Transform
B. Convergent
C. Divergent
D. Collision

## Answer Key {#answer-key}

| Q | Ans | Core fact |
| ---: | :---: | --- |
| 1 | **B** | Axis tilt = 23½° |
| 2 | **B** | Rotation → day/night |
| 3 | **C** | Tropic Cancer = 23½°N |
| 4 | **C** | 20 × 4 = 80 min |
| 5 | **B** | 82°30′E |
| 6 | **C** | Moho = crust/mantle |
| 7 | **B** | S-waves don't travel through liquids |
| 8 | **C** | Granite = intrusive |
| 9 | **B** | Limestone → marble |
| 10 | **B** | Wegener |
| 11 | **A** | Mid-ocean ridge |
| 12 | **C** | Continental–continental |
| 13 | **C** | Transform |
| 14 | **B** | Hotspot |
| 15 | **A** | Active volcano |
| 16 | **C** | Crust + uppermost mantle |
| 17 | **B** | Epicentre |
| 18 | **B** | Closest to Sun |
| 19 | **C** | Sandstone → quartzite |
| 20 | **C** | Divergent |

!!! revise "Final revision order"
 **Latitude/Longitude → Earth movements → Interior → Rocks → Plates → Earthquakes → Volcanoes → Landforms.**

</div>
